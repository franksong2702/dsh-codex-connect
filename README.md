...
1. **Detect** looks for a candidate in `HTTPS_PROXY`, `https_proxy`, `HTTP_PROXY`, `http_proxy`, `ALL_PROXY`, then `all_proxy`. It copies a valid credential-free HTTP(S) candidate into the draft and reports its source, but does not test, save, or enable it.
