...
- **Plan-aware quota bars** sit beside the model selector. Go and Plus plans show both `5h` and `7d`; Pro and other plans omit `5h` and show only the applicable weekly row. Each color moves from green through yellow/orange to red as its remaining amount falls. Hover or focus the control to see exact remaining percentages and server-provided reset times. The whole control is hidden for non-GPT models or when no usable quota data exists.
...
- **GPT-5.3-Codex-Spark · 5-hour** appears only for Go and Plus; **GPT-5.3-Codex-Spark · Weekly** remains the Spark weekly bucket.

Each bar shows the remaining percentage and its local reset time. The `plan_type` returned by the usage endpoint gates the 5-hour row, so Pro, Business, Team, and unknown plans do not display a stale 5-hour-shaped window. OpenAI controls the returned windows, eligibility, and reset values; missing usage data is treated as unavailable rather than guessed.
...
- OAuth accounts are stored separately at `$DSH_HOME/.openai-codex-auth.json` (`~/.dsh` by default). Legacy single-account version-1 documents are read without data loss and migrate to the multi-account version on the next credential write. `~/.codex/auth.json` is never copied or modified. The parent directory and file use owner-only permissions where supported, writes are atomic, and account switches and refresh writes use a cross-process file lock. When the active account reports an explicit terminal credit/quota exhaustion error, the model stream selects the next untried saved account and continues the same text-only turn with repeated-prefix removal. Attempts are bounded by the saved-account count. Generic HTTP 429, authentication, cancellation, timeout, DNS, and transport failures never rotate accounts; once a tool call has started, the turn also fails instead of replaying side effects.
...
This report covers only the standalone route, not active profile routing, search/image tools, browser compatibility, account fallback, provider retry behavior, or session recovery. Saved-account quota fallback is a runtime model-stream feature and is not exercised by `capabilities --probe`. Cross-provider failover remains `rejected`; select a different provider explicitly. WebSocket-to-SSE fallback is inactive with the finite SSE default. Native `contextManagement`, compaction, and WebSocket reuse remain outside the current integration policy. No diagnostic result enables these capabilities or changes Harness history. Exit codes cover runtime, OAuth, selected model, Responses, and SSE only: `0` means all five were supported, `1` means at least one was rejected, and `2` means unknown evidence, invalid options, or an inspection failure. Rejected optional capabilities do not change that exit code.
