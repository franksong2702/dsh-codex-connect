...
import { assertThinkMatrix, inspectThinkTestReport, THINK_IDENTITY_CASE, THINK_RUNTIME_PACKAGES } from './think-matrix-contract.mjs'
...
export async function readThinkHostIdentity(host, version, names) {
  ...
  if (resolved.error || resolved.cleanupError || resolved.status !== 0) throw new Error(`Think ESM resolution failed: ${resolved.error?.message ?? resolved.cleanupError?.message ?? resolved.stderr}`)
  ...
  for (const name of names) {
    ...
    assert.ok(inside(root, entry), `Think dependency escaped the selected host: ${name}`)
    ...
    for (let depth = 0; depth < 16 && inside(root, directory); depth++) {
      try {
        ...
        assert.ok(inside(root, file), `Think manifest escaped the selected host: ${name}`)
    ...
    assert.ok(manifest, `Missing exact Think manifest: ${name}`)
    if (name.startsWith('@deepseek-ai/dsh')) assert.equal(manifest.version, version, `Mixed Think host: ${name}`)
...
async function main() {
  ...
  assert.ok(argv.length === 0 || (argv.length === 2 && argv[0] === '--report'), 'usage: check-think-matrix [--report docs/experiments/name.json]')
  ...
  const root = await mkdtemp(join(tmpdir(), 'dsh-think-matrix-'))
  const reports = []
  try {
    ...
    for (const version of versions) {
      console.error(`Think ${version}: resolving exact runtime`)
      ...
      for (const name of THINK_RUNTIME_PACKAGES) assert.equal(overrides[name], version, `Missing Think root: ${name}`)
      ...
      const env = { ...scrubCanaryEnvironment(process.env), HOME: join(host, 'home'), DSH_HOME: join(host, 'synthetic-home'),
        ...
        npm_config_cache: join(ROOT, 'node_modules/.cache/think-matrix/npm'), THINK_MATRIX_ROOT: host, THINK_MATRIX_VERSION: version }
      ...
      console.error(`Think ${version}: installing ${Object.keys(overrides).length} pinned DSH packages`)
      ...
      if (install.error || install.cleanupError || install.status !== 0) throw new Error(`Think ${version} install failed: ${install.error?.message ?? install.cleanupError?.message ?? install.stderr.slice(-2000)}`)
      const identity = await readThinkHostIdentity(host, version, [...THINK_RUNTIME_PACKAGES, '@earendil-works/pi-ai'])
      ...
      console.error(`Think ${version}: running identical native-admission cases`)
      ...
      if (test.error || test.cleanupError || test.status !== 0) {
        ...
        try {
          ...
          const cache = join(ROOT, 'node_modules/.cache/think-matrix')
        ...
        throw new Error(`Think ${version} tests failed:\n${diagnostics.slice(-24000)}\n${test.error?.message ?? test.cleanupError?.message ?? ''}\n${test.stdout.slice(-1000)}\n${test.stderr.slice(-2000)}`)
      }
      const result = inspectThinkTestReport(JSON.parse(await readFile(join(host, 'tests.json'), 'utf8')))
      ...
      console.error(`Think ${version}: passed ${result.cases.length} functional cases and runtime identity`)
      await rm(host, { recursive: true, force: true })
    }
    const report = assertThinkMatrix({ schemaVersion: 1, kind: 'think-native-host-matrix', bundleDigest,
      syntheticOnly: true, productionEntryEnabled: false, realProviderDispatches: 0, reports }, versions, bundleDigest)
