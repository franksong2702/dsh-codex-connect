...
try {
  ...
  const adapter = createOpenAICodexAdapter(credentials, () => undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, governor, undefined, dispatch)
  ...
  await ctx.plugin(function TaskFixture(host: Context) {
    runtime = new AdaptiveTaskRuntime(host, { store: new AdaptiveTaskStore(join(directory, 'tasks')), models: async () => catalog, legacyThinkEnabled: () => false })
  })
  ...
  if (phase === 'write') {
    ...
    await runtime.command({ sessionId: id, action: 'start', operationId: randomUUID(), revision: before.revision,
      maximumRequests: 8, models: [...ADAPTIVE_TASK_MODELS] }, owner)
  } else {
