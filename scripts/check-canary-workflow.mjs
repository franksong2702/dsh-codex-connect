...
assertContract('the job budget covers two candidate timeouts plus ten minutes', (() => {
  const jobMinutes = Number(workflow.match(/timeout-minutes:\s*(\d+)/u)?.[1])
...
assertContract('candidate check writes a channel-specific first report', /check:dsh-next -- --channel "\$\{\{ matrix\.channel \}\}" \$\{\{ matrix\.dedupe_args \}\} --report \.canary\/first\.json/.test(workflow))
assertContract('candidate failure is retried once', /steps\.first-canary\.outcome == 'failure'[\s\S]*?check:dsh-next -- --channel "\$\{\{ matrix\.channel \}\}" \$\{\{ matrix\.dedupe_args \}\} --report \.canary\/second\.json/.test(workflow))
