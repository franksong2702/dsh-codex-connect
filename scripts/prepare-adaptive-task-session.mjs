...
import { resolveExactDshOverrides, readDshRegistryManifest, exactDshFixtureManifest } from './exact-dsh-fixture.mjs'
...
export async function prepareTaskSession(repo, version = '0.1.2-rc.1') {
  const directory = await mkdtemp(join(tmpdir(), 'codex-task-session-'))
  ...
  const [pack] = JSON.parse(execFileSync('npm', ['pack', '--json', '--ignore-scripts', '--pack-destination', directory], { cwd: source, env, encoding: 'utf8' }))
  ...
  execFileSync('npm', ['install', '--prefix', install, '--ignore-scripts', '--no-audit', '--no-fund', '--package-lock=false'], {
