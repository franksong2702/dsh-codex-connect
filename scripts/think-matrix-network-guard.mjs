...
const key = Symbol.for('codex-connect.think-matrix-network-tripwire')
if (globalThis[key] === undefined) {
  const state = { attempts: 0 }
  globalThis[key] = state
  const blocked = () => { state.attempts++; throw new Error('Network dispatch forbidden in synthetic Think host tests') }
...
afterAll(() => {
  ...
  writeFileSync(join(process.env.THINK_MATRIX_ROOT, 'network.json'), JSON.stringify({ pid: process.pid, attempts }))
