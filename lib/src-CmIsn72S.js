...
function openAICodexBackoffDelay(attempt, options) {
	if (!Number.isSafeInteger(attempt) || attempt < 0) throw new TypeError("attempt must be a non-negative safe integer");
	if (!Number.isFinite(options.baseMs) || options.baseMs < 0 || !Number.isFinite(options.maxMs) || options.maxMs < options.baseMs) throw new TypeError("backoff bounds are invalid");
	...
	return Math.max(jittered, retryAfter === void 0 || !Number.isFinite(retryAfter) ? 0 : retryAfter);
}
/** Abort-aware wait used by routes that explicitly opt into retries. */
async function waitForOpenAICodexBackoff(delayMs, signal) {
...
const OPENAI_CODEX_BACKEND_USER_AGENT = "dsh-codex-connect";
const DIRECT_ORIGINATOR = {
...
function backendUrl(input) {
	...
	if (url.origin !== "https://chatgpt.com" || url.username !== "" || url.password !== "" || !url.pathname.startsWith("/backend-api/")) throw new TypeError("OpenAI Codex backend request must target https://chatgpt.com/backend-api/");
	return url;
}
/** Central route identity policy. It intentionally does not impersonate first-party Codex clients. */
function openAICodexBackendHeaders(route, initial, clientRequestId = randomUUID()) {
	...
	if (route !== "model") {
		...
		if (originator === void 0) headers.delete("originator");
...
function openAICodexBackendDeadline(signal, timeoutMs) {
	...
	timer.unref?.();
	return {
		signal: signal === void 0 ? timeout.signal : AbortSignal.any([signal, timeout.signal]),
...
function wrapResponse(response, diagnostic, releaseGate) {
	...
	const observer = response.headers.get("content-type")?.toLowerCase().includes("text/event-stream") === true ? new ErrorFrameObserver(diagnostic) : void 0;
...
var OpenAICodexBackendRequestLayer = class {
	proxyManager;
	resolveProxyUrl;
	gate;
	...
	wrapFetch(route, baseFetch = globalThis.fetch, onDiagnostic) {
		return async (input, init) => {
			backendUrl(input);
			const sourceSignal = init?.signal ?? (input instanceof Request ? input.signal : void 0);
			const release = await this.gate.acquire(sourceSignal ?? void 0);
			const diagnostic = {
				route,
				clientRequestId: randomUUID()
			};
			onDiagnostic?.(diagnostic);
			const headers = openAICodexBackendHeaders(route, init?.headers ?? (input instanceof Request ? input.headers : void 0), diagnostic.clientRequestId);
			try {
				return wrapResponse(await baseFetch(input, {
					...init,
					headers
				}), diagnostic, release);
			} catch (error) {
	...
	fetch(route, input, init, baseFetch, onDiagnostic) {
...
function formatOpenAICodexBackendDiagnostic(diagnostic) {
...
async function requestNativeCompaction(model, context, options, scope, backendRequests) {
	...
	for (const [key, value] of Object.entries(options?.headers ?? {})) if (value === null) rawHeaders.delete(key);
	else rawHeaders.set(key, value);
	rawHeaders.set("authorization", `Bearer ${access}`);
	rawHeaders.set("chatgpt-account-id", accountIdFromToken$1(access));
	...
	for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
		...
		try {
			try {
				response = await networkFetch(OPENAI_CODEX_NATIVE_COMPACTION_URL, {
					...
					...deadline === void 0 ? options?.signal === void 0 ? {} : { signal: options.signal } : { signal: deadline.signal }
				});
			} catch (error) {
				if (options?.signal?.aborted === true || attempt === maxRetries) throw error;
				lastError = error;
				delay = openAICodexBackoffDelay(attempt, {
					baseMs: 500,
					maxMs: 4e3
				});
			}
		...
		if (delay === Infinity) throw lastError instanceof Error ? lastError : /* @__PURE__ */ new Error("OpenAI Codex native compaction retry delay is not bounded");
		if (delay !== void 0) await waitForRetry(delay, options?.signal);
	}
...
function nativeCompactionStream(provider, model, context, options, scope, backendRequests) {
...
function withOpenAICodexNativeCompaction(provider, backendRequests = new OpenAICodexBackendRequestLayer()) {
	return {
		...provider,
		streamSimple(model, context, options) {
			const scope = nativeCompactionScope.getStore();
			if (scope?.useNativeCompaction === true) return nativeCompactionStream(provider, model, context, options, scope, backendRequests);
...
const fallbackBackendRequests$1 = new OpenAICodexBackendRequestLayer();
/** Inject the shared backend request policy into pi-ai while preserving its transport hooks. */
function withCodexDiagnosticFetch(options, backendRequests = fallbackBackendRequests$1) {
...
function requestProvider(provider, fastMode, proxyManager, resolveProxyUrl, backendRequests = new OpenAICodexBackendRequestLayer(proxyManager, resolveProxyUrl)) {
	const configured = withOpenAICodexFastMode(withOpenAICodexNativeCompaction({
		...provider,
		streamSimple(model, context, options) {
			return provider.streamSimple(model, context, withCodexDiagnosticFetch(options, backendRequests));
		}
	}, backendRequests), fastMode);
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits, nativeCompactionEnabled, backendRequests) {
...
const transportBackendRequestBindings = /* @__PURE__ */ new WeakMap();
...
function retryAfterSeconds(response) {
	...
	return delay === void 0 || !Number.isFinite(delay) ? void 0 : Math.ceil(delay / 1e3);
}
...
var OpenAICodexTransport = class extends Service {
	...
	constructor(ctx, credentials, proxyManager, resolveProxyUrl = () => void 0, resolveImageModelHint = () => "") {
		...
		if (!transportBackendRequestBindings.has(credentials)) transportBackendRequestBindings.set(credentials, new OpenAICodexBackendRequestLayer(proxyManager, resolveProxyUrl));
	}
	...
	async generateImagesWithoutProxy(input, context, deadline) {
...
function bindOpenAICodexTransportBackendRequests(credentials, backendRequests) {
...
async function readOpenAICodexRateLimits(store) {
	const deadline = openAICodexBackendDeadline(void 0, USAGE_REQUEST_TIMEOUT_MS);
	try {
		return parseOpenAICodexUsage(await readOpenAICodexUsageResponse(await readOpenAICodexRequestAuth(store, deadline.signal).catch((error) => {
			if (error instanceof OpenAICodexRequestAuthError && error.code === "REAUTH_REQUIRED") throw new OpenAICodexReauthRequiredError();
			throw error;
		}), deadline.signal, false));
	} finally {
		deadline.dispose();
	}
}
/** Read one bounded account response; public quota and Reserve decisions share these exact bytes. */
async function readOpenAICodexUsageResponse(auth, signal, supportsReserve, backendRequests = fallbackBackendRequests) {
...
var OpenAICodexProxyManager = class {
	...
	async probe(proxyUrl) {
		...
		const deadline = openAICodexBackendDeadline(void 0, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS);
		try {
...
const searchBackendRequestBindings = /* @__PURE__ */ new WeakMap();
/** Internal plugin assembly hook; not part of the package-level API surface. */
function bindOpenAICodexSearchBackendRequests(options, backendRequests) {
	searchBackendRequestBindings.set(options, backendRequests);
	return options;
}
function searchBackendRequests(options) {
	let backendRequests = searchBackendRequestBindings.get(options);
	if (backendRequests === void 0) {
...
var OpenAICodexSearchProvider = class {
	...
	*/
	...
	async searchWithoutProxy(request, signal) {
		...
		let response;
		try {
			response = await abortable$1(searchBackendRequests(this.options).fetch("search", OPENAI_CODEX_SEARCH_URL, {
				method: "POST",
				redirect: "error",
				headers: {
					authorization: `Bearer ${access}`,
...
async function probeCodexAutoReview(request, createDispatcher = (proxyUrl) => proxyUrl === void 0 ? new Agent() : new ProxyAgent(proxyUrl)) {
	...
	try {
		...
		return {
			outcome: request.signal?.aborted === true ? "cancelled" : deadline.timedOut() ? "timeout" : "network-error",
...
function failureDelay(error, failures) {
	...
	return openAICodexBackoffDelay(failures - 1, {
		baseMs: 6e4,
		maxMs: 9e5,
		...error instanceof OpenAICodexUsageHttpError && error.retryAfterMs !== void 0 ? { retryAfterMs: error.retryAfterMs } : {},
		jitterRatio: .1
	});
}
...
var OpenAICodexQuotaState = class {
	...
	async readInternal(credentials, enabled, proxy, epoch, model) {
		...
		const fetch = async (authoritySignal = epoch) => {
			const value = await this.backendRequests.run(() => readOpenAICodexUsageResponse(auth, authoritySignal, enabled && identity !== void 0, this.backendRequests), proxy);
			epoch.throwIfAborted();
			return {
