...
function withOpenAICodexContextWindowOverrides(provider, overrides) {
	const replaced = provider.getModels().map((model) => {
...
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides) {
	...
	const profiles = /* @__PURE__ */ new Map([[OPENAI_CODEX_PROVIDER, createOpenAICodexProfile(provider, fastMode, proxyManager, resolveProxyUrl, contextWindowOverrides?.())]]);
...
function isValidOpenAICodexContextWindowOverrides(value) {
	...
	return entries.every(([modelId, window]) => typeof modelId === "string" && modelId.length > 0 && typeof window === "number" && Number.isSafeInteger(window) && window > 0);
}
...
function decodeOpenAICodexSettings(value) {
	...
	if (contextWindowOverrides !== void 0 && !isValidOpenAICodexContextWindowOverrides(contextWindowOverrides)) return void 0;
	...
	return {
		...
		contextWindowOverrides: contextWindowOverrides === void 0 ? void 0 : Object.freeze({ ...contextWindowOverrides }),
		enableSearch,
		enableImageTool,
...
const Config = z.object({
	...
	contextWindowOverrides: z.union([z.const(void 0), z.dict(z.number().step(1).min(1))]),
