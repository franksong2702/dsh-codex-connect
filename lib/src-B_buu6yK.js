...
*/
function withOpenAICodexAccountFallback(provider, credentials, resolveAccessToken) {
				...
				while (true) {
					...
					for await (const event of inner) {
						...
						if (event.type === "done" || event.type === "error") {
							...
							const failedAccountId = event.type === "error" && event.reason === "error" && isOpenAICodexAccountQuotaExhausted(event.error.errorMessage ?? "") && options?.signal?.aborted !== true && !hasToolCall(raw) && ![...openBlocks.values()].includes("toolCall") && access !== void 0 ? await credentials.accountIdForAccessToken(access) : void 0;
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxy, _resolveProxyUrl, contextWindowOverrides) {
	...
	const currentProfiles = () => {
		...
		if (profiles === void 0 || !deepEqualJson(previousOverrides, overrides)) {
			...
			profiles = /* @__PURE__ */ new Map([[OPENAI_CODEX_PROVIDER, {
				...profile,
				piProvider: withOpenAICodexAccountFallback(profile.piProvider, credentials, resolveAccessToken)
			}]]);
		}
		return profiles;
	};
...
function apply(ctx, config) {
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).models, proxy, void 0, () => resolveOpenAICodexSettings(current()).contextWindowOverrides));
