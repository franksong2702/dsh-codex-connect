...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits, nativeCompactionEnabled, backendRequests, requestReplay) {
	...
	class OpenAICodexAdapter extends PiAiAdapter {
		streamPrepared(stream, options) {
			const delegate = () => streamWithCodexRequestDiagnostics((next) => streamWithNativeCompactionScope(stream, next, nativeCompactionEnabled?.() === true), options);
...
var OpenAICodexBackendRequests = class {
	...
	disposed = false;
	constructor(proxyManager, resolveProxyUrl = () => void 0, maxConcurrent = 8) {
		this.proxyManager = proxyManager;
		this.resolveProxyUrl = resolveProxyUrl;
