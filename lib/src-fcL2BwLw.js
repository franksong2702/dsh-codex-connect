...
/** Build the pi-ai profile with the model-error index required by DSH 0.1.7-alpha.2. */
...
const SUPPORTED_DSH_PLUGIN_API_VERSION = "0.1.7-alpha.2";
...
function evaluateCompatibility(input = {}) {
	...
	const matchedPair = dshVersion === packages["@deepseek-ai/dsh-llm-pi-ai"].installed && dshVersion === packages["@deepseek-ai/dsh-compaction"].installed && dshVersion === "0.1.7-alpha.2" && piVersion === "0.85.1";
	return {
		schemaVersion: 1,
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.43";
//#endregion
...
var AdaptiveTaskDelegation = class {
	...
	async execute(root, args, execution) {
		...
		execution.signal.throwIfAborted();
		const request = input$1(args), call = this.call(root, execution.callId);
		if (JSON.stringify(input$1(JSON.parse(call.data.arguments))) !== JSON.stringify(request)) taskFailure("TASK_OPERATION_CONFLICT");
	...
	async reconcile(parent, requireAll = false) {
		...
		for (const run of doc.delegation.runs) {
			...
			if (taskIdentity(JSON.stringify(input$1(JSON.parse(call.data.arguments)))) !== run.argumentDigest) taskFailure("TASK_OPERATION_CONFLICT");
