	...
	*/
	...
	async readDocument() {
		...
		try {
			...
			assertOwnerOnly$1(this.filename, info.mode);
			if (info.size > 524288) throw new Error(`openai-codex: ${this.filename} exceeds ${String(OPENAI_CODEX_AUTH_DOCUMENT_LIMIT)} bytes`);
			return parseDocument$2(await handle.readFile("utf8"), this.filename);
		} finally {
			await handle.close();
		}
	}
