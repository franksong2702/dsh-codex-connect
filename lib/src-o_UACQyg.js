...
function assertOpenAICodexBackendUrl(input) {
	...
	if (url.origin !== "https://chatgpt.com" || !url.pathname.startsWith("/backend-api/")) throw new TypeError("OpenAI Codex backend request must target chatgpt.com/backend-api");
}
...
function withCodexDiagnosticFetch(options, requests) {
	...
	return {
		...
		async fetch(input, init) {
			...
			const { headers, clientRequestId } = prepareOpenAICodexBackendHeaders(init?.headers ?? (input instanceof Request ? input.headers : void 0), "preserve");
...
function wrapResponseLifecycle(response, release) {
...
var OpenAICodexBackendRequests = class {
	...
	combinedSignal(signal, timeoutMs) {
		const signals = [this.lifecycle.signal, ...signal === void 0 ? [] : [signal]];
		if (timeoutMs !== void 0) {
	...
	async beforeRequest(lane, signal) {
		await waitUntil(this.cooldowns.get(lane) ?? 0, signal);
	}
	recordResponse(lane, meta) {
		if (meta.httpStatus !== 429 && meta.httpStatus !== 503 || meta.retryAfterMs === void 0 || meta.retryAfterMs <= 0) return;
	...
	wrapFetch(options) {
		return async (input, init) => {
			const inputSignal = init?.signal ?? (input instanceof Request ? input.signal : void 0);
			const signal = this.combinedSignal(inputSignal ?? void 0);
			await this.beforeRequest(options.lane, signal);
			const release = await this.acquire(signal);
			try {
				return wrapResponseLifecycle(await this.fetchAttempt(options.lane, signal, input, init, options), release);
			} catch (error) {
				release();
				throw error;
			}
