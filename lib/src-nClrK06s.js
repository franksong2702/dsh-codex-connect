...
var ErrorFrameObserver = class {
	...
	feed(chunk) {
		for (let offset = 0; offset < chunk.byteLength; offset += 4096) this.text(this.decoder.decode(chunk.subarray(offset, offset + 4096), { stream: true }));
	}
	finish() {
	...
	observe(data) {
		if (this.diagnostic.eventType !== void 0) return;
		let event;
		try {
		...
		if (!record$3(event) || event["type"] !== "error" && event["type"] !== "response.failed") return;
...
var OpenAICodexQuotaState = class {
	...
	schedule(at) {
		...
		this.timer = setTimeout(() => {
			this.timer = void 0;
			this.timerAt = Infinity;
			for (const entry of this.cache.values()) if (Date.now() >= entry.refreshAt) entry.authority?.abort();
