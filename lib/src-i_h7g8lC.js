...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits, nativeCompactionEnabled, requestReplay) {
...
async function readOpenAICodexUsageResponse(auth, signal, supportsReserve) {
...
var OpenAICodexSearchProvider = class {
	...
	*/
	...
	async searchWithoutProxy(request, signal) {
		...
		let response;
		try {
			response = await abortable$1(fetch(OPENAI_CODEX_SEARCH_URL, {
				method: "POST",
				redirect: "error",
				headers: {
					authorization: `Bearer ${access}`,
...
function prepareReasoningReplay(options, session) {
	...
	if (options.purpose !== void 0 || session.surface.replaceGeneration !== 0) reasoningUpdateError("Compaction and auxiliary requests are not supported by this Think replay mechanism.");
...
function registerThinkHostIntegration(ctx) {
	...
	ctx.on("agent/pre-step", async ({ agent, turn, step, signal }, next) => {
		...
		const base = history.map(readReasoningUpdate).find((value) => value !== void 0)?.baseEffort;
	...
	ctx.on("agent/request", async ({ agent, turn, step, signal }, next) => {
		...
		const first = messages.map(readReasoningUpdate).find((value) => value !== void 0);
		if (first === void 0) return config;
		assertOwner(agent);
		...
		if (current?.pending !== void 0 && !ids.has(current.pending.notice.id) && (!enabled || current.pending.signal.aborted || pending.get(agent) !== current.pending)) reasoningUpdateError("The pending Think approval was canceled before admission.");
	...
	function replaySession(options) {
		...
		if (captured !== void 0 && !captured.signal.aborted) for (const expected of captured.messages.filter((message) => readReasoningUpdate(message) !== void 0)) {
	...
	async function change(agent, effort, reason, signal) {
		...
		try {
			...
			if (after.baseEffort !== before.baseEffort || after.effectiveEffort !== before.effectiveEffort || after.revision !== before.revision) reasoningUpdateError("The conversation changed during the decision; this approval was not applied.");
			pending.set(agent, {
