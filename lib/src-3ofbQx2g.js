...
import { createAssistantMessageEventStream, createModels, defaultProviderAuthContext } from "@earendil-works/pi-ai";
import { openaiCodexProvider } from "@earendil-works/pi-ai/providers/openai-codex";
import { createUserMessage, isQuotaExceededError, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
import { Dispatcher, ProxyAgent, getGlobalDispatcher, request, setGlobalDispatcher } from "undici";
...
const OPENAI_CODEX_ACCOUNT_PROFILES_FILENAME = ".openai-codex-account-profiles.json";
const PROFILE_CLAIM = "https://api.openai.com/profile";
const MAX_PROFILE_FILE_BYTES = 65536;
...
function decodeOauthProfile(access) {
	...
	if (payload === void 0 || payload.length === 0 || payload.length > MAX_PROFILE_FILE_BYTES) return {};
	try {
...
function parseOverride(value) {
	if (typeof value === "string") {
		const name = boundedText$1(value, MAX_LABEL_LENGTH);
		return name === void 0 ? void 0 : { name };
	}
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const record = value;
	if (Object.keys(record).some((key) => key !== "name" && key !== "email")) return void 0;
	const name = boundedText$1(record["name"], MAX_LABEL_LENGTH);
	const email = boundedText$1(record["email"], MAX_EMAIL_LENGTH);
	return name === void 0 && email === void 0 ? void 0 : {
		...name === void 0 ? {} : { name },
		...email === void 0 ? {} : { email }
	};
}
function parseProfilesDocument(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const record = value;
	if (record["version"] !== 1 || Object.keys(record).some((key) => ![
		"version",
		"localUsername",
		"accounts"
	].includes(key))) return;
	const localUsername = boundedText$1(record["localUsername"], MAX_LABEL_LENGTH);
	const rawAccounts = record["accounts"];
	if (rawAccounts !== void 0 && (typeof rawAccounts !== "object" || rawAccounts === null || Array.isArray(rawAccounts))) return;
	const accounts = {};
	for (const [key, raw] of Object.entries(rawAccounts ?? {})) {
		const normalizedKey = boundedText$1(key, MAX_EMAIL_LENGTH);
		const override = parseOverride(raw);
		if (normalizedKey === void 0 || override === void 0) return void 0;
		accounts[normalizedKey] = override;
	}
	return {
		...localUsername === void 0 ? {} : { localUsername },
		accounts
	};
}
async function readProfilesDocument(filename) {
	try {
		const metadata = await stat(filename);
		if (!metadata.isFile() || metadata.size > MAX_PROFILE_FILE_BYTES) return void 0;
		return parseProfilesDocument(JSON.parse(await readFile(filename, "utf8")));
	} catch {
		return;
	}
}
/** Resolve the conventional profile file next to an OAuth auth document. */
function openAICodexAccountProfilesPath(authFilename) {
	return resolve(join(dirname(authFilename), OPENAI_CODEX_ACCOUNT_PROFILES_FILENAME));
}
function localUsername(document) {
	let operatingSystemUsername;
	try {
		operatingSystemUsername = boundedText$1(userInfo().username, MAX_LABEL_LENGTH);
	} catch {
		operatingSystemUsername = void 0;
	}
	return document?.localUsername ?? boundedText$1(process.env["OPENAI_CODEX_LOCAL_USERNAME"], MAX_LABEL_LENGTH) ?? operatingSystemUsername ?? boundedText$1(process.env["USERNAME"], MAX_LABEL_LENGTH) ?? boundedText$1(process.env["USER"], MAX_LABEL_LENGTH) ?? boundedText$1(process.env["LOGNAME"], MAX_LABEL_LENGTH);
}
/**
...
*/
async function resolveOpenAICodexAccountProfiles(credentials, profileFilename) {
	...
	return credentials.map((credential, index) => {
		...
		const override = document?.accounts[accountId] ?? (oauth.email === void 0 ? void 0 : document?.accounts[oauth.email]);
		const email = override?.email ?? oauth.email;
		const displayName = override?.name ?? (override?.email === void 0 ? oauth.name : void 0) ?? email ?? (username === void 0 ? void 0 : credentials.length > 1 ? `${username} · ${String(index + 1)}` : username) ?? `Account ${String(index + 1)}`;
		return {
			displayName,
			...email === void 0 || email === displayName ? {} : { email },
			source: override !== void 0 ? "file" : oauth.name !== void 0 || oauth.email !== void 0 ? "oauth" : username !== void 0 ? "local" : "generated"
		};
	});
}
//#endregion
...
function parseCredential(raw, filename, location = "credential") {
	if (typeof raw !== "object" || raw === null || Array.isArray(raw)) throw new Error(`openai-codex: ${filename} ${location} must be an object`);
	...
	if (Object.keys(credential).some((key) => ![
		...
		"expires",
		"accountId"
	].includes(key))) throw new Error(`openai-codex: ${filename} ${location} contains an unknown field`);
	if (credential["type"] !== "oauth") throw new Error(`openai-codex: ${filename} ${location} type must be oauth`);
	for (const key of [
		"access",
		"refresh",
		"accountId"
	]) if (typeof credential[key] !== "string" || credential[key].length === 0) throw new Error(`openai-codex: ${filename} ${location} ${key} must be a non-empty string`);
	if (typeof credential["expires"] !== "number" || !Number.isFinite(credential["expires"]) || credential["expires"] <= 0) throw new Error(`openai-codex: ${filename} ${location} expires must be a positive finite number`);
	return credential;
}
...
function parseDocument$2(text, filename) {
	...
	if (typeof activeAccountId !== "string" || activeAccountId.length === 0) throw new Error(`openai-codex: ${filename} activeAccountId must be a non-empty string`);
	if (!Array.isArray(document["accounts"]) || document["accounts"].length === 0) throw new Error(`openai-codex: ${filename} accounts must be a non-empty array`);
	const accounts = document["accounts"].map((account, index) => parseCredential(account, filename, `accounts[${String(index)}]`));
	const accountIds = accounts.map((account) => account.accountId);
	if (new Set(accountIds).size !== accountIds.length) throw new Error(`openai-codex: ${filename} contains duplicate account ids`);
	if (!accountIds.includes(activeAccountId)) throw new Error(`openai-codex: ${filename} activeAccountId does not reference a stored account`);
	return {
		version: AUTH_FORMAT_VERSION,
		activeAccountId,
		accounts
...
var OpenAICodexCredentialStore = class {
	...
	accountProfilesFilename;
	/**
	* @param filename - explicit document path, defaulting under `$DSH_HOME`.
	*/
	constructor(filename = openAICodexAuthPath(), accountProfilesFilename = openAICodexAccountProfilesPath(filename)) {
	...
	async readDocument() {
		...
		return cloneDocument(parseDocument$2(text, this.filename));
	}
	/** Persist a validated version-2 document while the caller owns the file lock. */
	async writeDocument(document) {
		const validated = parseDocument$2(JSON.stringify(document), this.filename);
	...
	async accountIdForAccessToken(access) {
		if (access.length === 0) return void 0;
		return (await this.readDocument())?.accounts.find((account) => account.access === access)?.accountId;
	}
	/**
	...
	*/
	async activateNext(failedAccountId, attemptedAccountIds = []) {
		if (failedAccountId.length === 0 || failedAccountId.length > 256) throw new TypeError("openai-codex: account id is invalid");
		...
		return withFileLock(this.filename, async () => {
			const document = await this.readDocument();
			if (document === void 0 || document.accounts.length < 2) return void 0;
			const failedIndex = document.accounts.findIndex((account) => account.accountId === failedAccountId);
			const start = failedIndex === -1 ? document.accounts.findIndex((account) => account.accountId === document.activeAccountId) : failedIndex;
			for (let offset = 1; offset <= document.accounts.length; offset += 1) {
				...
				if (document.activeAccountId !== accountId) await this.writeDocument({
					...document,
	...
	async activate(accountId) {
		...
		return withFileLock(this.filename, async () => {
			...
			if (document === void 0 || account === void 0) throw new Error("openai-codex: account was not found");
			if (document.activeAccountId !== accountId) await this.writeDocument({
				...document,
				activeAccountId: accountId
			});
	...
	/** @inheritdoc */
	async list() {
		return await this.read("openai-codex") === void 0 ? [] : [{
	...
	async modify(providerId, fn) {
		...
		return withFileLock(this.filename, async () => {
			...
			return active === void 0 ? void 0 : structuredClone(active);
		});
	}
...
//#endregion
//#region src/account-fallback.ts
...
const DEDUPLICATION_LIMIT = 4096;
/** Narrow terminal quota detection; transient 429/network failures do not rotate accounts. */
function isOpenAICodexAccountQuotaExhausted(detail) {
	return isQuotaExceededError(detail) || /\b(?:GoUsageLimitError|FreeUsageLimitError)\b/u.test(detail) || /\b(?:usage_limit_reached|usage_not_included)\b/iu.test(detail) || /\bhit\s+(?:(?:your|the)\s+)?(?:ChatGPT\s+)?usage\s+limit\b/iu.test(detail) || /\bMonthly usage limit reached\b/iu.test(detail) || /\bavailable balance\b/iu.test(detail);
}
function emptyUsage() {
	return {
		input: 0,
...
function continuationContext(context, model, accumulated) {
...
function combinedMessage(previous, current, currentContent = current.content) {
...
function longestSuffixPrefix(prefix, candidate) {
	const limit = Math.min(prefix.length, candidate.length);
	for (let length = limit; length > 0; length -= 1) if (prefix.endsWith(candidate.slice(0, length))) return length;
	return 0;
}
function canStillBeRepeatedSuffix(prefix, candidate) {
...
var ContinuationDeduplicator = class {
	prefix;
	buffered = "";
	decided = false;
	constructor(previousText) {
		this.prefix = previousText.slice(-4096);
	}
	write(delta) {
		if (this.decided || this.prefix.length === 0) return delta;
		this.buffered += delta;
		if (this.buffered.length < DEDUPLICATION_LIMIT && canStillBeRepeatedSuffix(this.prefix, this.buffered)) return "";
		return this.release();
	}
	finish() {
...
function withCombinedPartial(previous, partial, firstTextIndex, firstText) {
	return combinedMessage(previous, partial, partial.content.map((block, index) => index === firstTextIndex && block.type === "text" ? {
		...block,
		text: firstText
	} : block));
}
function remapEvent(event, offset, previous, firstTextIndex, firstText) {
	return {
		...event,
...
function setupFailure(model, error) {
	return {
		role: "assistant",
		content: [],
		api: model.api,
...
*/
function withOpenAICodexAccountFallback(provider, credentials, resolveAccessToken, enabled = () => true) {
	const streamSimple = provider.streamSimple;
	return {
		...provider,
		streamSimple(model, originalContext, options) {
			...
			const pump = async () => {
				...
				while (true) {
					...
					const deduplicator = new ContinuationDeduplicator(previous === void 0 ? "" : visibleText(previous));
					...
					const inner = streamSimple.call(provider, model, context, {
						...options,
						...access === void 0 ? {} : { apiKey: access }
					});
					for await (const event of inner) {
						if (event.type === "start") {
							if (previous === void 0) output.push(event);
							continue;
						}
						if (event.type === "done" || event.type === "error") {
							sawTerminal = true;
							const released = deduplicator.finish();
							if (released.length > 0 && firstTextIndex !== void 0) {
								firstText += released;
								output.push({
									type: "text_delta",
							...
							const failedAccountId = event.type === "error" && event.reason === "error" && isOpenAICodexAccountQuotaExhausted(event.error.errorMessage ?? "") && enabled() && options?.signal?.aborted !== true && !hasToolCall(raw) && ![...openBlocks.values()].includes("toolCall") && access !== void 0 ? await credentials.accountIdForAccessToken(access) : void 0;
							if (failedAccountId !== void 0) {
								...
								if (next !== void 0) {
									...
									const nextAccountId = nextAccess === void 0 ? void 0 : await credentials.accountIdForAccessToken(nextAccess);
									if (nextAccess !== void 0 && nextAccountId === next.accountId && !attempted.includes(nextAccountId)) {
										for (const [index, type] of openBlocks) {
											const block = currentContent[index];
											if (type === "text" && block?.type === "text") output.push({
												type: "text_end",
												contentIndex: index + offset,
												content: block.text,
												partial: combined
											});
											else if (type === "thinking" && block?.type === "thinking") output.push({
												type: "thinking_end",
												contentIndex: index + offset,
							...
							if (event.type === "done") output.push({
								...event,
								message: combined
							});
							else output.push({
						...
						if (event.type === "text_delta") {
							...
							if (delta.length > 0) output.push(remapEvent({
								...event,
								delta
...
const OPENAI_CODEX_PROXY_TEST_URL = "https://chatgpt.com/backend-api/codex";
/** Bound the explicit user-triggered proxy probe. */
const OPENAI_CODEX_PROXY_TEST_TIMEOUT_MS = 1e4;
...
const OPENAI_CODEX_CONNECTIVITY_TIMEOUT_MS = 2500;
const OPENAI_CODEX_CONNECTIVITY_TARGETS = [
	{
		id: "codex-api",
...
const dispatcherScope = new AsyncLocalStorage();
const attachedControllers = /* @__PURE__ */ new Map();
...
function detach(controller) {
	...
	const current = getGlobalDispatcher();
	if (installedDispatcher !== void 0 && current === installedDispatcher) setGlobalDispatcher(installedDispatcher.fallback);
	installedDispatcher = void 0;
}
function proxyAgentState(proxyUrl) {
...
*/
var OpenAICodexProxyController = class {
	state;
	...
	configure(proxyUrl) {
		...
		this.url = proxyUrl;
		this.state = proxyUrl === void 0 ? void 0 : proxyAgentState(proxyUrl);
		if (this.state !== void 0) this.states.add(this.state);
		if (previous !== void 0) this.retire(previous);
	}
	run(operation) {
		const current = this.state;
		if (current === void 0 || this.disposed) return operation();
		current.uses += 1;
		attach(this);
		const release = () => {
			detach(this);
	...
	closeIfRetired(state) {
		if (!state.retired || state.uses !== 0 || state.closing) return;
		state.closing = true;
		this.closeTail = this.closeTail.then(() => state.agent.close()).then(() => void 0, () => void 0).finally(() => {
...
function withOpenAICodexProxy(proxy, operation) {
	return proxy === void 0 ? operation() : proxy.run(operation);
}
...
function classifyProxyResponse(statusCode) {
...
function safeConnectivityError(error) {
	const record = typeof error === "object" && error !== null ? error : void 0;
	return `${typeof record?.code === "string" ? `${record.code}: ` : ""}${error instanceof Error ? error.message : String(error)}`.replace(/https?:\/\/[^\s/@]+:[^\s/@]+@/giu, "[redacted proxy]").slice(0, 300);
}
/** Check the small set of first-party domains used by Codex Connect. */
async function checkOpenAICodexConnectivity(proxy, timeoutMs = OPENAI_CODEX_CONNECTIVITY_TIMEOUT_MS) {
	const targets = await Promise.all(OPENAI_CODEX_CONNECTIVITY_TARGETS.map(async (target) => {
		const startedAt = performance.now();
		const controller = new AbortController();
		const timer = setTimeout(() => {
			controller.abort(/* @__PURE__ */ new Error(`connection timed out after ${String(timeoutMs)}ms`));
		}, timeoutMs);
		timer.unref();
		try {
	...
	return {
		checkedAt: Date.now(),
		mode: proxy?.activeUrl === void 0 ? "direct" : "proxy",
		targets
	};
}
/**
...
*/
async function testOpenAICodexProxy(proxyUrl, timeoutMs = OPENAI_CODEX_PROXY_TEST_TIMEOUT_MS) {
	const agent = new ProxyAgent(proxyUrl);
	const controller = new AbortController();
	const timer = setTimeout(() => {
		controller.abort(/* @__PURE__ */ new Error("proxy test timed out"));
	}, timeoutMs);
	timer.unref();
	try {
		...
		return {
			ok: false,
			error: (error instanceof Error ? error.message : String(error)).replace(/https?:\/\/[^\s/@]+:[^\s/@]+@/giu, "[redacted proxy]").slice(0, 300),
			classification: classifyProxyError(error)
		};
	} finally {
		clearTimeout(timer);
...
function requestProvider(provider, fastMode, proxy) {
	...
	return {
		...
		streamSimple(model, context, options) {
			...
			return withOpenAICodexProxy(proxy, () => streamSimple.call(configured, model, context, proxyUrl === void 0 ? options : {
				...options,
				transport: "sse"
			}));
		},
...
function createOpenAICodexProfile(provider, fastMode, proxy, _resolveProxyUrl, contextWindowOverrides) {
	...
	return {
		...
		piProvider: requestProvider(effectiveProvider, fastMode, proxy)
	};
}
/** Replace only the advertised context window for configured catalog models. */
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxy, _resolveProxyUrl, contextWindowOverrides, enableAccountFallback) {
	const provider = openaiCodexProvider();
	const models = createModels({ credentials });
	models.setProvider(provider);
	const resolveAccessToken = async () => withOpenAICodexProxy(proxy, async () => (await models.getAuth(OPENAI_CODEX_PROVIDER))?.auth.apiKey);
	let profiles;
	...
	const currentProfiles = () => {
		...
		if (profiles === void 0 || !deepEqualJson(previousOverrides, overrides)) {
			const profile = createOpenAICodexProfile(provider, fastMode, proxy, void 0, overrides);
			previousOverrides = overrides === void 0 ? void 0 : { ...overrides };
			profiles = /* @__PURE__ */ new Map([[OPENAI_CODEX_PROVIDER, {
				...profile,
				piProvider: withOpenAICodexAccountFallback(profile.piProvider, credentials, resolveAccessToken, enableAccountFallback)
			}]]);
		}
		return profiles;
	};
...
*/
async function loginOpenAICodex(interaction, store = new OpenAICodexCredentialStore(), proxy) {
	const models = createModels({ credentials: store });
	models.setProvider(openaiCodexProvider());
	await withOpenAICodexProxy(proxy, () => models.login(OPENAI_CODEX_PROVIDER, "oauth", interaction));
}
/**
...
const FIVE_HOUR_PLAN_TYPES = /* @__PURE__ */ new Set(["go", "plus"]);
...
function hasFiveHourLimit(planType) {
	return typeof planType === "string" && FIVE_HOUR_PLAN_TYPES.has(planType.trim().toLowerCase());
}
function parseLimit(id, name, value, includeFiveHour) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$2(value)) throw new Error("OpenAI Codex returned malformed rate-limit details");
	const windows = [parseWindow(value["primary_window"]), parseWindow(value["secondary_window"])].filter((window) => window !== void 0).filter((window) => includeFiveHour || window.windowSeconds !== FIVE_HOUR_WINDOW_SECONDS);
...
function parseOpenAICodexUsage(value) {
	...
	for (const item of additional ?? []) {
		...
		const limit = parseLimit(id, typeof name === "string" && name.length > 0 ? name : void 0, item["rate_limit"], includeFiveHour);
...
*/
async function readOpenAICodexRateLimits(store, proxy) {
...
var OpenAICodexWebAuth = class {
	...
	start() {
		...
		}, this.store, this.proxy).then(async () => {
...
function header$1(req, name) {
	const value = req.headers[name];
	if (Array.isArray(value)) return value[0];
	return value;
}
function contentLength(req) {
	const raw = header$1(req, "content-length");
...
function accountSwitchBody(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const record = value;
	if (Object.keys(record).length !== 1) return void 0;
	const accountId = record["accountId"];
	return typeof accountId === "string" && accountId.length > 0 && accountId.length <= 256 ? { accountId } : void 0;
}
/** Register the plugin-owned OAuth routes when the Web server is composed. */
function registerOpenAICodexAuthRoutes(ctx, store, trustedOriginsOverride, fastModeOverride, proxy) {
	const auth = new OpenAICodexWebAuth(store, { ...proxy === void 0 ? {} : { proxy } });
		...
		const routes = [
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					const type = header$1(req, "content-type");
					...
					try {
						...
						return json$4(res, 200, { accounts: await store.accounts() });
					} catch (error) {
						...
						if (error instanceof Error && error.message === "openai-codex: account was not found") return json$4(res, 404, { error: "account not found" });
						return json$4(res, 500, { error: safeMessage(error) });
					}
				}
			}),
			ctx.webServer.register({
...
function resolveOpenAICodexSettings(value) {
	...
	if (resolved.enableProxy && !isValidOpenAICodexProxyUrl(resolved.proxyUrl)) throw new TypeError("OpenAI Codex proxy URL must be a credential-free HTTP(S) origin");
	return {
		...resolved,
		proxyUrl: resolved.enableProxy ? normalizeOpenAICodexProxyUrl(resolved.proxyUrl) : resolved.proxyUrl,
...
const OPENAI_CODEX_PROXY_ENV_KEYS = [
	"HTTPS_PROXY",
	"https_proxy",
	"HTTP_PROXY",
	"http_proxy",
...
function detectOpenAICodexProxyEnvironment(environment = process.env) {
...
*/
function listOpenAICodexProxyCandidates(environment = process.env) {
	const candidates = [];
	const append = (source, raw) => {
		const value = raw?.trim();
		if (value === void 0 || value.length === 0 || !isValidOpenAICodexProxyUrl(value)) return;
...
const CONNECTIVITY_CACHE_MS = 2500;
...
async function readJson(req) {
	...
	if (declared !== void 0 && (!/^\d+$/u.test(declared.trim()) || Number(declared) > BODY_LIMIT)) throw new RangeError("request body too large");
	const chunks = [];
	let total = 0;
	const iterable = req;
	if (typeof req[Symbol.asyncIterator] === "function") for await (const chunk of iterable) {
...
function proxyUrlBody(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const record = value;
	if (Object.keys(record).length !== 1 || typeof record["proxyUrl"] !== "string") return void 0;
	const proxyUrl = record["proxyUrl"].trim();
	return isValidOpenAICodexProxyUrl(proxyUrl) ? proxyUrl : void 0;
}
/** Register read-only Detect and draft-only Test routes. Activation uses SettingsScope. */
function registerOpenAICodexProxyRoutes(ctx, trustedOrigins, options = {}) {
	const environment = options.environment ?? process.env;
	const testProxy = options.testProxy ?? testOpenAICodexProxy;
	const detectProxy = options.testProxy ?? ((proxyUrl) => testOpenAICodexProxy(proxyUrl, 3e3));
	const checkConnectivity = options.checkConnectivity ?? (() => checkOpenAICodexConnectivity(options.proxy));
	ctx.effect(() => {
		let cachedConnectivity;
		let connectivityInFlight;
		const connectivity = () => {
			const proxyUrl = options.proxy?.activeUrl;
			if (cachedConnectivity !== void 0 && cachedConnectivity.proxyUrl === proxyUrl && Date.now() - cachedConnectivity.report.checkedAt < CONNECTIVITY_CACHE_MS) return Promise.resolve(cachedConnectivity.report);
			if (connectivityInFlight !== void 0 && connectivityInFlight.proxyUrl === proxyUrl) return connectivityInFlight.promise;
			const promise = checkConnectivity().then((report) => {
				cachedConnectivity = {
					proxyUrl,
					report
				};
				...
				if (connectivityInFlight?.promise === promise) connectivityInFlight = void 0;
			});
			connectivityInFlight = {
				proxyUrl,
				promise
			};
			return promise;
		...
		const routes = [
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$3(res, 200, await connectivity());
					} catch {
						json$3(res, 503, { error: "connectivity check failed" });
					}
				}
			}),
			ctx.webServer.register({
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (type === void 0 || !/^application\/json(?:\s*;|$)/iu.test(type.trim())) return json$3(res, 415, { error: "unsupported content type" });
					try {
						const proxyUrl = proxyUrlBody(await readJson(req));
						if (proxyUrl === void 0) return json$3(res, 400, { error: "invalid proxy URL" });
						json$3(res, 200, await testProxy(proxyUrl));
					} catch (error) {
						json$3(res, error instanceof RangeError ? 413 : 400, { error: error instanceof RangeError ? "request body too large" : "invalid input" });
					}
				}
			})
		];
...
async function requestPinned(url, address, maxBytes, signal) {
	...
	return new Promise((resolve, reject) => {
		...
		const request = (url.protocol === "https:" ? request$2 : request$1)(url, {
			method: "GET",
			agent: false,
...
var OpenAICodexTransport = class extends Service {
	...
	apiVersion = 1;
	models;
	constructor(ctx, credentials, proxy) {
	...
	async generateImages(input, context) {
		...
		let auth;
		try {
			auth = await withOpenAICodexProxy(this.proxy, () => this.models.getAuth(OPENAI_CODEX_PROVIDER));
		} catch {
...
var OpenAICodexSearchProvider = class {
	...
	*/
	...
	async search(request, signal) {
		...
		let auth;
		try {
			auth = await abortable(withOpenAICodexProxy(this.options.proxy, () => this.models.getAuth(OPENAI_CODEX_PROVIDER)), signal);
		} catch (error) {
			throwIfSearchAborted(signal);
...
function apply(ctx, config) {
	...
	let current = () => config;
	const resolveProviderProxyUrl = () => resolveOpenAICodexProxyUrl(current());
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).models, proxy, void 0, () => resolveOpenAICodexSettings(current()).contextWindowOverrides, () => resolveOpenAICodexSettings(current()).enableAccountFallback));
...
//#endregion
export { OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as $, openAICodexConflictMessage as A, OPENAI_CODEX_CONNECTIVITY_TIMEOUT_MS as At, DSH_PLUGIN_API_PACKAGES as B, openAICodexAuthPath as Bt, OpenAICodexTransport as C, parseOpenAICodexUsage as Ct, IMAGE_GENERATE_TOOL_NAME as D, openAICodexAuthStatus as Dt, VIEW_IMAGE_TOOL_NAME as E, logoutOpenAICodex as Et, parseOpenAICodexUpdateResult as F, checkOpenAICodexConnectivity as Ft, assessCompatibility as G, SUPPORTED_DSH_PLUGIN_API_VERSION as H, openAICodexAccountProfilesPath as Ht, parseOpenAICodexVersion as I, testOpenAICodexProxy as It, readInstalledPackageVersion as J, detectCompatibility as K, COMPATIBILITY_CONTRACT as L, OPENAI_CODEX_AUTH_FILENAME as Lt, OPENAI_CODEX_UPDATE_PATH as M, OPENAI_CODEX_PROXY_TEST_TIMEOUT_MS as Mt, checkForOpenAICodexUpdate as N, OPENAI_CODEX_PROXY_TEST_URL as Nt, assertNoOpenAICodexProviderConflict as O, openAICodexModelCatalog as Ot, compareOpenAICodexVersions as P, OpenAICodexProxyController as Pt, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as Q, COMPATIBILITY_PACKAGES as R, OPENAI_CODEX_PROVIDER as Rt, OPENAI_CODEX_TRANSPORT_SERVICE as S, OPENAI_CODEX_USAGE_URL as St, isOpenAICodexTransportError as T, loginOpenAICodex as Tt, SUPPORTED_NODE_RANGE as U, PI_AI_PACKAGE as V, OPENAI_CODEX_ACCOUNT_PROFILES_FILENAME as Vt, SUPPORTED_PI_AI_VERSION as W, OPENAI_CODEX_PROXY_DETECT_PATH as X, OPENAI_CODEX_CONNECTIVITY_PATH as Y, OPENAI_CODEX_PROXY_TEST_PATH as Z, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as _, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as _t, name as a, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as at, OPENAI_CODEX_TRANSPORT_API_VERSION as b, OpenAICodexTrustedOriginsStore as bt, migrateOpenAICodexSearchHistory as c, DEFAULT_OPENAI_CODEX_SETTINGS as ct, OPENAI_CODEX_SEARCH_URL as d, isValidOpenAICodexContextWindowOverrides as dt, OPENAI_CODEX_PROXY_ENV_KEYS as et, OpenAICodexSearchProvider as f, isValidOpenAICodexProxyUrl as ft, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as g, FastModeRegistry as gt, OPENAI_CODEX_IMAGE_MAX_COUNT as h, OPENAI_CODEX_FAST_MODE_PATH as ht, inject as i, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as it, CODEX_CONNECT_VERSION as j, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as jt, diagnoseOpenAICodex as k, OPENAI_CODEX_CONNECTIVITY_TARGETS as kt, OPENAI_CODEX_BASE_URL as l, OPENAI_CODEX_SETTINGS_NAMESPACE as lt, OPENAI_CODEX_IMAGE_GENERATION_URL as m, resolveOpenAICodexSettings as mt, OPENAI_CODEX_SETTINGS_NS as n, listOpenAICodexProxyCandidates as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, DEFAULT_OPENAI_CODEX_SEARCH_MODE as ot, mapOpenAICodexSearchResponse as p, resolveOpenAICodexProxyUrl as pt, evaluateCompatibility as q, apply as r, DEFAULT_OPENAI_CODEX_PROXY_URL as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as st, Config as t, detectOpenAICodexProxyEnvironment as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, decodeOpenAICodexSettings as ut, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as v, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as vt, OpenAICodexTransportError as w, readOpenAICodexRateLimits as wt, OPENAI_CODEX_TRANSPORT_ERROR_CODES as x, normalizeTrustedOrigin as xt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as y, isFastModeSessionId as yt, COMPATIBILITY_SCHEMA_VERSION as z, OpenAICodexCredentialStore as zt };
