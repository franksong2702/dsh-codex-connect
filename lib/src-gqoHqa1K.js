...
async function compactResponse(response, retained) {
	...
	const consumeEvent = (raw) => {
		...
		if (!isRecord$4(terminal)) throw new Error("OpenAI Codex native compaction returned a malformed terminal event");
		const id = terminal["id"];
		if (id !== void 0 && typeof id !== "string") throw new Error("OpenAI Codex native compaction returned a malformed response id");
		responseId = id;
...
function nativeCompactionStream(provider, model, context, options, scope) {
	...
	requestNativeCompaction(model, context, options, scope).then((response) => {
		const source = markerStream(model, response);
		(async () => {
			for await (const event of source) target.push(event);
		})();
	}, (error) => {
		const source = options?.signal?.aborted === true ? failedStream(model, error, options.signal) : standardStream(provider, model, context, options, scope);
		(async () => {
			for await (const event of source) target.push(event);
		})();
	});
