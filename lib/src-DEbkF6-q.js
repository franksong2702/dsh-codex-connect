...
function releaseHighlightsBetween(highlights, currentVersion, latestVersion) {
	return highlights.filter((highlight) => compareOpenAICodexVersions(highlight.version, currentVersion) > 0).filter((highlight) => compareOpenAICodexVersions(highlight.version, latestVersion) <= 0).map((highlight) => ({ ...highlight }));
}
async function releaseHighlights(currentVersion, latestVersion, fetchImpl, timeoutMs) {
	try {
		...
		return parsed === void 0 ? [] : releaseHighlightsBetween(parsed, currentVersion, latestVersion);
	} catch {
		return [];
	}
}
...
async function checkForOpenAICodexUpdate(options) {
	...
	const [highlights, details] = await Promise.all([releaseHighlights(currentVersion, latestVersion, fetchImpl, timeoutMs), releaseDetails(latestVersion, fetchImpl, timeoutMs)]);
	return {
...
//#endregion
export { compareOpenAICodexVersions as $, OPENAI_CODEX_TRANSPORT_SERVICE as A, COMPATIBILITY_PACKAGES as B, OPENAI_CODEX_IMAGE_MAX_COUNT as C, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as D, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as E, IMAGE_GENERATE_TOOL_NAME as F, SUPPORTED_NODE_RANGE as G, DSH_PLUGIN_API_PACKAGES as H, assertNoOpenAICodexProviderConflict as I, detectCompatibility as J, SUPPORTED_PI_AI_VERSION as K, diagnoseOpenAICodex as L, OpenAICodexTransportError as M, isOpenAICodexTransportError as N, OPENAI_CODEX_TRANSPORT_API_VERSION as O, VIEW_IMAGE_TOOL_NAME as P, checkForOpenAICodexUpdate as Q, openAICodexConflictMessage as R, OPENAI_CODEX_IMAGE_GENERATION_URL as S, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as T, PI_AI_PACKAGE as U, COMPATIBILITY_SCHEMA_VERSION as V, SUPPORTED_DSH_PLUGIN_API_VERSION as W, CODEX_CONNECT_VERSION as X, evaluateCompatibility as Y, OPENAI_CODEX_UPDATE_PATH as Z, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as _, OpenAICodexCredentialStore as _t, name as a, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as at, decodeOpenAICodexSettings as b, migrateOpenAICodexSearchHistory as c, normalizeTrustedOrigin as ct, OPENAI_CODEX_SEARCH_URL as d, readOpenAICodexRateLimits as dt, parseOpenAICodexUpdateResult as et, OpenAICodexSearchProvider as f, loginOpenAICodex as ft, DEFAULT_OPENAI_CODEX_SEARCH_MODE as g, OPENAI_CODEX_PROVIDER as gt, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as h, OPENAI_CODEX_AUTH_FILENAME as ht, inject as i, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as it, OpenAICodexTransport as j, OPENAI_CODEX_TRANSPORT_ERROR_CODES as k, OPENAI_CODEX_BASE_URL as l, OPENAI_CODEX_USAGE_URL as lt, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as m, openAICodexAuthStatus as mt, OPENAI_CODEX_SETTINGS_NS as n, OPENAI_CODEX_FAST_MODE_PATH as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, isFastModeSessionId as ot, mapOpenAICodexSearchResponse as p, logoutOpenAICodex as pt, assessCompatibility as q, apply as r, FastModeRegistry as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, OpenAICodexTrustedOriginsStore as st, Config as t, parseOpenAICodexVersion as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, parseOpenAICodexUsage as ut, DEFAULT_OPENAI_CODEX_SETTINGS as v, openAICodexAuthPath as vt, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as w, resolveOpenAICodexSettings as x, OPENAI_CODEX_SETTINGS_NAMESPACE as y, COMPATIBILITY_CONTRACT as z };
