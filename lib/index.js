...
const IMAGE_INPUT_SCHEMA = {
	...
	description: "Select exactly one: assetId for an original result, attachmentId from this session, or a complete attachment reference. usePreview=true is only for an explicitly chosen lower-resolution preview.",
	properties: {
		assetId: { type: "string" },
...
function imageGenerateTool(ctx, assets) {
	...
	return defineTool({
		...
		description: "Generate a new image, or edit an explicitly selected session image. For edits use operation=edit and target; optional ordered references have separate purposes. Use exact assetId/attachmentId handles from the conversation, never guess or use global recent images. Ask when the target is ambiguous. An unavailable edit target must not become text-only generation. Results preserve originals and can be edited again; output size and style are service-controlled.",
		parameters: {
			prompt: {
				type: "string",
				required: true,
