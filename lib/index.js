import { a as setGlobalDispatcher, i as getGlobalDispatcher, n as ProxyAgent, r as fetch$1, t as Dispatcher } from "./undici-runtime-Ctml-NLj.js";
...
function boundedText$2(value, maximum) {
...
function decodeOauthProfile(access) {
	...
	try {
		...
		const name = boundedText$2(record["name"], MAX_LABEL_LENGTH);
		const email = boundedText$2(record["email"], MAX_EMAIL_LENGTH);
		return {
			...name === void 0 ? {} : { name },
			...email === void 0 ? {} : { email }
		};
...
function isCompactCheckpointSource$1(message) {
	return isCompactCheckpointSourceValue(message.source);
}
function isCompactCheckpointSourceValue(source) {
	return isCompactCheckpointSource(source);
}
function isCompactionInstructionMessage(message) {
	if (message === void 0 || message.role !== "user" || "id" in message) return false;
	return message.content.length === 1 && isTextBlock(message.content[0]) && message.content[0].text.startsWith("You are now acting as a compaction engine for this AI coding assistant.");
}
...
function decodeNativeCompactionCheckpoint(message) {
	if (!isCompactCheckpointSource$1(message)) return void 0;
...
function prepareScopedRequest(options, enabled) {
	...
	const messages = options.messages.map((message) => {
		const items = "id" in message && message.id !== void 0 ? decodeNativeCompactionCheckpoint(message) : void 0;
...
var ErrorFrameObserver = class {
	...
	observe(data) {
		...
		this.stop();
		const response = record$2(event["response"]) ? event["response"] : void 0;
		const nested = record$2(event["error"]) ? event["error"] : response !== void 0 && record$2(response["error"]) ? response["error"] : void 0;
...
function decodeTaskCommand(value) {
	...
	if (value.action === "delegate-enable") {
		if (keys !== "action,disclose,files,maxChildRequests,operationId,revision,routes,sessionId,timeoutMs" || value.disclose !== true || !Array.isArray(value.files) || value.files.length < 1 || value.files.length > 32 || new Set(value.files).size !== value.files.length || value.files.some((file) => typeof file !== "string" || !file.trim() || file.length > 512) || !Array.isArray(value.routes) || value.routes.length < 1 || value.routes.length > 64 || value.routes.some((route) => !taskRoute(route)) || new Set(value.routes.map((route) => `${route.model}/${route.effort}`)).size !== value.routes.length || !Number.isSafeInteger(value.maxChildRequests) || Number(value.maxChildRequests) < 1 || Number(value.maxChildRequests) > 6 || !Number.isSafeInteger(value.timeoutMs) || Number(value.timeoutMs) < 1e3 || Number(value.timeoutMs) > 9e4) return void 0;
...
function parseTaskDocument(value, sessionId) {
...
var AtomicTaskDocumentStore = class {
	directory;
	parse;
	constructor(directory, parse) {
	...
	async read(sessionId) {
		...
		try {
			...
			try {
				...
				if (!stat.isFile() || stat.nlink !== 1 || stat.size > MAX_BYTES$1 || process.platform !== "win32" && (stat.mode & 63) !== 0) taskFailure("TASK_STATE_UNSAFE");
				const bytes = Buffer.alloc(65537);
				const { bytesRead } = await file.read(bytes, 0, bytes.length, 0);
				if (bytesRead > MAX_BYTES$1 || bytesRead !== stat.size) taskFailure("TASK_STATE_INVALID");
	...
	async update(sessionId, change) {
		...
		return withFileLock(filename, async () => {
			...
			if (Buffer.byteLength(text) > MAX_BYTES$1) taskFailure("TASK_STATE_TOO_LARGE");
...
var AdaptiveTaskStore = class extends AtomicTaskDocumentStore {
	constructor(directory) {
		super(directory, parseTaskDocument);
	}
};
//#endregion
...
function imageInputObject(value) {
...
function decodeImageInputAttachment(value) {
	const ref = imageInputObject(value);
	if (ref === void 0 || !only(ref, [
		"attachmentId",
		"mediaType",
		"width",
		"height",
		...
		"image/jpeg",
		"image/webp"
	].includes(String(ref.mediaType)) || !positive(ref.width) || !positive(ref.height) || !positive(ref.bytes) || ref.name !== void 0 && (typeof ref.name !== "string" || ref.name.length < 1 || ref.name.length > 512)) return void 0;
	const dimensions = ref.originalDimensions === void 0 ? void 0 : imageInputObject(ref.originalDimensions);
	if (ref.originalDimensions !== void 0 && (dimensions === void 0 || !only(dimensions, ["width", "height"]) || !positive(dimensions.width) || !positive(dimensions.height))) return void 0;
	return {
		...
		...dimensions === void 0 ? {} : { originalDimensions: {
...
function decodeImageInputRef(value) {
	const ref = imageInputObject(value);
	if (ref === void 0) return void 0;
	if (typeof ref.assetId === "string" && OPENAI_CODEX_IMAGE_ASSET_ID_PATTERN.test(ref.assetId) && only(ref, ["assetId"])) return { assetId: ref.assetId };
	const preview = ref.usePreview === true ? { usePreview: true } : {};
	if (ref.usePreview !== void 0 && ref.usePreview !== true) return void 0;
	if (identifier(ref.attachmentId) && only(ref, ["attachmentId", "usePreview"])) return {
		attachmentId: ref.attachmentId,
		...preview
	};
	const attachment = decodeImageInputAttachment(ref.attachment);
	if (attachment !== void 0 && only(ref, ["attachment", "usePreview"])) return {
		attachment,
		...preview
	};
}
function decodeImageEditSources(value) {
	const candidate = imageInputObject(value);
	if (candidate === void 0 || !only(candidate, ["target", "references"])) return void 0;
	const target = decodeImageInputRef(candidate.target);
	if (target === void 0 || !Array.isArray(candidate.references) || candidate.references.length >= 5) return void 0;
	const references = [];
	for (const item of candidate.references) {
		const ref = imageInputObject(item);
		if (ref === void 0 || !only(ref, ["image", "purpose"]) || typeof ref.purpose !== "string" || ref.purpose.trim().length === 0 || ref.purpose.length > 512) return void 0;
		const image = decodeImageInputRef(ref.image);
		if (image === void 0) return void 0;
		references.push({
			image,
			purpose: ref.purpose.trim()
		});
	}
	return {
		target,
...
function decodeImageToolRequest(value) {
	const request = imageInputObject(value);
	if (request === void 0 || typeof request.prompt !== "string") return void 0;
	const prompt = request.prompt.trim();
	if (prompt.length < 1 || prompt.length > 32e3) return void 0;
	if ((request.operation === void 0 || request.operation === "generate") && only(request, ["prompt", "operation"])) return {
		operation: "generate",
		prompt
	};
	if (request.operation !== "edit" || !only(request, [
		"operation",
		"prompt",
		"target",
		"references"
	])) return void 0;
	const edit = decodeImageEditSources({
		target: request.target,
		references: request.references === void 0 ? [] : request.references
	});
	return edit === void 0 ? void 0 : {
		operation: "edit",
		prompt,
		edit
	};
...
const IMAGE_REFERENCES_SCHEMA = {
	type: "array",
	description: "Reference images only, kept in the same order the user specified; never place the edit target here.",
	items: {
		type: "object",
		additionalProperties: false,
		properties: {
			...
			purpose: {
				type: "string",
				required: true,
				description: "What to borrow, such as colors or texture, without replacing the target."
			}
		}
	}
};
const IMAGE_EDIT_SOURCES_SCHEMA = {
	type: "object",
	additionalProperties: false,
	properties: {
		target: {
...
function withOpenAICodexImageSelectionHandles(options) {
	let changed = false;
	const messages = options.messages.map((message) => {
		let imageIndex = 0;
		...
		for (const block of message.content) {
			...
			imageIndex += 1;
			const name = block.attachment.name === void 0 ? "" : ` ${JSON.stringify(block.attachment.name)}`;
			const attachment = decodeImageInputAttachment(block.attachment);
			const exactSelection = attachment === void 0 ? "" : ` To edit this uploaded copy, use attachment=${JSON.stringify(attachment)}. For a generated original, use its result assetId instead.`;
			content.push({
				type: "text",
...
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits, nativeCompactionEnabled, backendRequests, taskDispatch, imageEditingEnabled) {
	...
	class OpenAICodexAdapter extends PiAiAdapter {
		streamPrepared(stream, options) {
			const dispatch = (request) => {
				return streamWithCodexRequestDiagnostics((next) => streamWithNativeCompactionScope(stream, next, nativeCompactionEnabled?.() === true), imageEditingEnabled?.() === true ? withOpenAICodexImageSelectionHandles(request) : request);
			};
...
function resolveOpenAICodexSettings(value) {
	...
	if (typeof resolved.enableNewSessionFastMode !== "boolean" || typeof resolved.enableNewSubagentFastMode !== "boolean") throw new TypeError("OpenAI Codex new-session Fast Mode defaults must be booleans");
...
function decodeOpenAICodexSettings(value) {
	...
	if (enableNewSessionFastMode !== void 0 && typeof enableNewSessionFastMode !== "boolean") return void 0;
	if (enableNewSubagentFastMode !== void 0 && typeof enableNewSubagentFastMode !== "boolean") return void 0;
...
*/
function snapshotImageEditRequest(input) {
	if (typeof input !== "object" || input === null || Array.isArray(input) || Object.keys(input).some((key) => key !== "prompt" && key !== "images") || typeof input.prompt !== "string" || input.prompt.trim().length === 0 || input.prompt.length > 32e3 || !Array.isArray(input.images) || input.images.length < 1 || input.images.length > 5) invalid();
	let total = 0;
	for (const image of input.images) {
		if (typeof image !== "object" || image === null || Array.isArray(image) || Object.keys(image).some((key) => key !== "data" && key !== "mediaType") || !(image.data instanceof Uint8Array) || image.data.byteLength < 1 || image.data.byteLength > 10485760) invalid();
		total += image.data.byteLength;
		if (total > 20971520) invalid();
		const actual = detectEncodedImage(image.data);
		if (actual === void 0 || actual.mediaType !== image.mediaType || actual.width > 32768 || actual.height > 32768 || actual.width * actual.height > 5e7) invalid();
	}
	return {
		prompt: input.prompt,
		images: input.images.map((image) => ({ image_url: `data:${image.mediaType};base64,${Buffer.from(image.data).toString("base64")}` }))
	};
}
//#endregion
...
const OPENAI_CODEX_IMAGE_EDIT_URL = "https://chatgpt.com/backend-api/codex/images/edits";
...
var OpenAICodexTransport = class extends Service {
	...
	async editImages(input, context) {
	...
	async requestImages(input, context, images) {
	...
	async generateImagesWithoutProxy(input, context, requestFetch = globalThis.fetch, images) {
		...
		if (images !== void 0 && Buffer.byteLength(body, "utf8") > 33554432) throw new OpenAICodexTransportError(OPENAI_CODEX_TRANSPORT_ERROR_CODES.invalidRequest);
		...
		try {
			...
			const response = await requestFetch(images === void 0 ? OPENAI_CODEX_IMAGE_GENERATION_URL : OPENAI_CODEX_IMAGE_EDIT_URL, {
				method: "POST",
				redirect: "manual",
...
let installedFetch;
let previousFetch;
const scopedUndiciFetch = fetch$1;
function installScopedFetch(fallback) {
	previousFetch = fallback;
	installedFetch = (input, init) => {
		if (proxyScope.getStore() === void 0) return fallback(input, init);
...
function removeOwner(owner) {
	...
	if (globalThis.fetch === installedFetch && previousFetch !== void 0) globalThis.fetch = previousFetch;
	installedLegacy = void 0;
	previousLegacy = void 0;
	installedFetch = void 0;
	previousFetch = void 0;
}
...
const SUPPORTED_DSH_PLUGIN_API_VERSION = "0.1.7-rc.1";
const SUPPORTED_DSH_PLUGIN_API_VERSIONS = [SUPPORTED_DSH_PLUGIN_API_VERSION, "0.1.7-rc.2"];
...
function evaluateCompatibility(input = {}) {
	...
	const matchedPair = dshVersion === packages["@deepseek-ai/dsh-llm-pi-ai"].installed && dshVersion === packages["@deepseek-ai/dsh-compaction"].installed && typeof dshVersion === "string" && isSupportedDshPluginApiVersion(dshVersion) && piVersion === "0.85.1";
	return {
		schemaVersion: 1,
...
const IMAGE_EDIT_RESULT_PREFIX = "codex-connect-image-result-v2:";
...
function decodeImagePresentationMeta(value) {
	...
	const edit = candidate.schemaVersion === 2 ? decodeImageEditSources(candidate.edit) : void 0;
	if (candidate.schemaVersion === 2 && (candidate.operation !== "edit" || edit === void 0)) return void 0;
	const images = [];
...
function decodeImageResultContent(content, prompt) {
	...
	for (const value of content) {
		...
		if (block.type === "text" && typeof block.text === "string" && (block.text.startsWith("codex-connect-image-result-v1:") || block.text.startsWith("codex-connect-image-result-v2:"))) envelopes.push(block.text);
	}
	if (previews.length < 1 || previews.length > 4 || envelopes.length > 1) return void 0;
	const envelope = envelopes[0];
	if (envelope !== void 0) {
		if (envelope.length > (envelope.startsWith("codex-connect-image-result-v2:") ? 256e3 : 16e3)) return void 0;
		try {
			const isEdit = envelope.startsWith(IMAGE_EDIT_RESULT_PREFIX);
			const payload = JSON.parse(envelope.slice(isEdit ? 30 : 30));
			const decoded = decodeImagePresentationMeta(isEdit ? payload : {
				kind: IMAGE_PRESENTATION_KIND,
				schemaVersion: 1,
				prompt,
				images: payload
			});
			if (decoded === void 0 || decoded.prompt !== prompt || decoded.images.length !== previews.length || decoded.images.some((image, index) => JSON.stringify(image.preview) !== JSON.stringify(previews[index]))) return void 0;
			return decoded;
		} catch {
			return;
		}
...
function imagePresentationFromEvent(event) {
	const entry = imageInputObject(event);
	const data = imageInputObject(entry?.data);
	if (data === void 0) return void 0;
	if (entry?.type === "tool/result") {
		if (imageInputObject(data.message)?.isError !== false) return void 0;
	...
	return typeof args?.prompt === "string" ? decodeImageResultContent(data.content, args.prompt.trim()) : void 0;
}
/** Original access in a fork remains confined to the actual inherited event prefix. */
function inheritedImageOriginal(session, assetId) {
...
function sessionImageInventory(session) {
	const result = {
	...
	for (const event of session.snapshotEvents()) {
		...
		for (const image of presentation?.images ?? []) {
			if (image.original !== void 0) result.originals.set(image.original.assetId, image.original);
			const preview = addRef(image.preview);
			if (preview === void 0) continue;
			const id = String(preview.attachmentId);
			const occurrences = result.results.get(id) ?? /* @__PURE__ */ new Map();
...
function registerOpenAICodexOriginalImageRoute(ctx, trustedOrigins, assets) {
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			const stored = await assets.read(requested.sessionId, requested.assetId, inheritedImageOriginal(session, requested.assetId));
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.50";
//#endregion
//#region src/fast-mode-defaults.ts
/**
...
*/
function registerOpenAICodexFastModeDefaults(ctx, fastMode, settings) {
	const started = /* @__PURE__ */ new WeakSet();
	ctx.on("agent/created", ({ agent, source }) => {
		if (source !== "startup" || started.has(agent.session)) return void 0;
		const session = agent.session;
		started.add(session);
		if (!isFastModeSessionId(session.id)) return void 0;
		const defaults = settings();
		if (session.header.origin === "subagent" ? defaults.enableNewSubagentFastMode : defaults.enableNewSessionFastMode) fastMode.set(session.id, true);
	}, { global: true });
}
//#endregion
...
function imageInputFailureMessage(label, reason) {
	return `${label}: ${reason}`;
}
...
function imageTransportErrorMessage(code) {
	return typeof code === "string" && Object.hasOwn(TRANSPORT_MESSAGES, code) ? TRANSPORT_MESSAGES[code] : UNKNOWN_TRANSPORT_MESSAGE;
}
const failures = /* @__PURE__ */ new Map();
function register(message, retryable, recovery) {
...
for (const [code, message] of Object.entries(TRANSPORT_MESSAGES)) {
	...
	register(message, !account, account ? "account" : void 0);
}
...
//#endregion
//#region src/image-inputs.ts
var ImageInputError = class extends Error {};
function fail$1(label, reason) {
	throw new ImageInputError(imageInputFailureMessage(label, reason));
}
/** Includes no input IDs or raw metadata in error messages. */
function promptForImageEdit(prompt, sources) {
	const value = `${["Image 1 is the target to edit. Preserve aspects not requested to change.", ...sources.references.map((ref, index) => `Image ${String(index + 2)} is a reference only. Reference purpose: ${ref.purpose}`)].join("\n")}\n\nRequested edit:\n${prompt}`;
	if (value.length > 32e3) fail$1("Edit instructions", "shorten the instructions or reference descriptions.");
	return value;
}
async function resolveImageEditInputs(ctx, assets, sources, exec) {
	exec.signal.throwIfAborted();
	const snapshot = decodeImageEditSources(sources);
	if (snapshot === void 0) fail$1("Image editing", "requires valid, bounded image selections.");
	sources = snapshot;
	const session = exec.agent?.session;
	if (session === void 0 || typeof session.snapshotEvents !== "function" || session.id !== exec.agent?.id) fail$1("Image editing", "requires the current session and its image history.");
	...
	].every((value) => Number.isSafeInteger(value) && value > 0)) fail$1("Image editing", "requires complete host image limits.");
	const inputs = [sources.target, ...sources.references.map((ref) => ref.image)];
	if (inputs.length > Math.min(5, limits.maxImagesPerMessage)) fail$1("Image batch", "too many inputs for this deployment.");
	const candidates = inputs.map((source, index) => {
		const label = index === 0 ? "Target image" : `Reference image ${index}`;
		if ("assetId" in source) {
			const ref = inventory.originals.get(source.assetId);
			if (ref === void 0) fail$1(label, "is unavailable in this session. Select or attach the image again.");
			return {
				label,
				source: { assetId: ref.assetId },
				ref
			};
		}
		...
		const selectorKey = "attachment" in source ? JSON.stringify(source.attachment) : void 0;
		const ref = selectorKey === void 0 ? occurrences?.values().next().value : occurrences?.get(selectorKey);
		if (ref === void 0) fail$1(label, "is unavailable or does not match a reference in this session. Select or attach the image again.");
		const uploads = inventory.uploads.get(id);
		const allResults = [...inventory.results.get(id)?.values() ?? []];
		if (selectorKey === void 0 && source.usePreview !== true && uploads !== void 0 && allResults.length > 0) fail$1(label, "identifies both an upload and a generated result. Select the uploaded attachment explicitly or choose an exact original image.");
		const results = selectorKey !== void 0 && uploads?.has(selectorKey) === true ? [] : allResults.filter((result) => selectorKey === void 0 || JSON.stringify(result.preview) === selectorKey);
		const result = results[0];
		if (result !== void 0 && source.usePreview !== true) {
			if (new Set(results.map((item) => item.original?.assetId)).size > 1) fail$1(label, "matches several original results. Select one exact original image.");
			if (result.original === void 0) fail$1(label, "only a preview is available. Explicitly choose that preview or attach an original.");
			return {
				label,
				source: { assetId: result.original.assetId },
				ref: result.original
			};
		}
		const attachment = decodeImageInputAttachment(ref);
		if (attachment === void 0) fail$1(label, "has unsupported attachment metadata.");
		return {
			label,
			source: {
				attachment,
				...source.usePreview === true ? { usePreview: true } : {}
			},
			ref
		};
	});
	...
	for (const candidate of candidates) {
		...
		if (ref.bytes > Math.min(10485760, limits.maxImageBytes) || ref.width * ref.height > Math.min(5e7, limits.maxImagePixels) || Math.max(ref.width, ref.height) > Math.min(32768, limits.maxImageDimension) || !limits.mediaTypes.includes(ref.mediaType)) fail$1(candidate.label, "exceeds this deployment's image limits or uses an unsupported format.");
		total += ref.bytes;
		if (!Number.isSafeInteger(total) || total > Math.min(20971520, limits.maxMessageImageBytes)) fail$1("Image batch", "exceeds this deployment's total input limit.");
	}
	const images = [];
	for (const candidate of candidates) {
		exec.signal.throwIfAborted();
		...
		try {
			if ("assetId" in candidate.ref) {
				...
				if (stored === void 0 || JSON.stringify(stored.ref) !== JSON.stringify(candidate.ref)) fail$1(candidate.label, "the original is unavailable or failed integrity checks; no preview was substituted.");
				data = stored.data;
			} else data = (await ctx.attachments.readImage(candidate.ref, exec.signal)).data;
			const actual = detectEncodedImage(data);
			if (actual === void 0 || actual.mediaType !== candidate.ref.mediaType || actual.width !== candidate.ref.width || actual.height !== candidate.ref.height || data.byteLength !== candidate.ref.bytes) fail$1(candidate.label, "the stored bytes do not match the image reference.");
			...
			fail$1(candidate.label, "cannot be read or validated. No image request was sent.");
		}
	}
	const target = candidates[0]?.source;
	if (target === void 0) fail$1("Target image", "is required.");
	return {
		images,
		edit: {
			target,
			references: sources.references.map((ref, index) => ({
				image: candidates[index + 1].source,
				purpose: ref.purpose
			}))
		}
	};
...
function fixedTransportMessage(error) {
	return imageTransportErrorMessage(typeof error === "object" && error !== null && "code" in error ? error.code : void 0);
}
function extension(mediaType) {
	return mediaType === "image/jpeg" ? "jpg" : mediaType.slice(6);
}
function presentation(prompt, value) {
	return {
		kind: IMAGE_PRESENTATION_KIND,
		schemaVersion: value.edit === void 0 ? 1 : 2,
		prompt,
		images: value.images,
		...value.edit === void 0 ? {} : {
			operation: "edit",
...
function outputContent(value, prompt) {
	...
	return [
		{
			type: "text",
			text: `${value.edit === void 0 ? "Generated" : "Edited"} ${String(value.images.length)} image${value.images.length === 1 ? "" : "s"}:\n${lines.join("\n")}`
		},
		{
			type: "text",
			text: value.edit === void 0 ? IMAGE_RESULT_PREFIX + JSON.stringify(value.images) : IMAGE_EDIT_RESULT_PREFIX + JSON.stringify(presentation(prompt, value))
		},
...
async function generate(ctx, transport, assets, prompt, exec, edit) {
	let response;
	let resolvedEdit;
	try {
		if (edit === void 0) response = await transport.generateImages({ prompt }, { signal: exec.signal });
		else {
			if (transport.editImages === void 0) failure("This Codex Connect transport does not support editing. No generation was attempted.");
			...
			response = await transport.editImages({
				prompt: instructions,
	...
	try {
		return {
			...resolvedEdit === void 0 ? {} : { edit: resolvedEdit },
	...
	return defineTool({
		...
		parameters: {
			prompt: {
				type: "string",
				required: true,
				description: "The requested new image or changes to the selected target."
			},
			operation: {
				type: "string",
				enum: ["generate", "edit"],
				description: "Use edit whenever modifying a selected image. Omission is legacy text generation only."
			},
		...
		output: {
			...
			render: (args, value) => outputContent(value, args.prompt.trim()),
		...
		async execute(args, exec) {
			...
			if (request === void 0) failure("Invalid image request. Editing requires operation=edit, one explicit target and valid references; no generation was attempted.");
			...
			const pending = generate(ctx, transport, assets, prompt, exec, request.operation === "edit" ? request.edit : void 0).catch((error) => {
...
async function runPublicHttpHop(signal, operation) {
	...
	return new Promise((resolve, reject) => {
		...
		const onAbort = () => {
			const error = abortError$2(signal);
...
async function fetchPublicHttpResource(source, maxBytes, signal, runtime = NODE_PUBLIC_HTTP_RUNTIME) {
	...
	for (let redirects = 0;; redirects += 1) {
		...
		const hop = await runPublicHttpHop(signal, async (hopSignal) => {
			...
			if (hopSignal.aborted) throw abortError$2(hopSignal);
...
var OpenAICodexBackendRequests = class {
	...
	drain() {
		while (!this.disposed && this.active < this.maxConcurrent && this.waiters.length > 0) {
			...
			if (waiter.signal.aborted) {
				waiter.reject(abortError$1(waiter.signal));
				continue;
			}
			this.active += 1;
	...
	acquire(signal) {
		if (this.disposed || this.lifecycle.signal.aborted) return Promise.reject(abortError$1(this.lifecycle.signal));
	...
	dispose() {
		...
		for (const waiter of this.waiters.splice(0)) {
			waiter.signal.removeEventListener("abort", waiter.onAbort);
			waiter.reject(abortError$1(this.lifecycle.signal));
		}
	}
};
//#endregion
...
function taskCheckpointValidator(session, events) {
	...
	return (event) => {
		...
		if (!Array.isArray(summary.data.rawOutput) || !isDeepStrictEqual(summary.data.summary, summary.data.rawOutput.filter((block) => block.type === "text")) || !isDeepStrictEqual(source, compactCheckpointSource(start.data.compactionId, start.data.sourceCommandId)) || !isDeepStrictEqual(summary.data.shadowedSeqs, range.shadowedSeqs) || summary.data.shadowedRange.start !== range.start || summary.data.shadowedRange.end !== range.end || !isDeepStrictEqual(event.sourceEventSeqs, [
			start.seq,
			summary.seq,
			...range.shadowedSeqs
		])) return fail();
...
function portableTaskMessages(session, current, target) {
	...
	const add = (message, depth = 0) => {
		...
		if (message.role === "system") messages.push({
			...message,
			content,
			source: structuredClone(message.source)
		});
		else if (message.role === "assistant") messages.push({
			...message,
			content,
			source: {
				kind: "model",
...
function sameRoute$1(a, b) {
	return a.model === b.model && a.effort === b.effort;
}
function isFresh(agent) {
	return agent.status === "idle" && agent.inbox.nextTurn.length === 0 && agent.inbox.nextStep.length === 0 && !agent.session.header.isSeeded && !agent.session.header.parentSession && !agent.session.snapshotEvents().some((event) => event.type === "request/header" || event.type === "user/message" && event.data.source.kind === "user");
}
const TASK_MARKER = "dsh-codex-connect/task-grant";
function hasTaskMarker(agent) {
	const marked = (message) => message.source.kind === "dsh-codex-connect" && message.source.plugin === TASK_MARKER;
...
var AdaptiveTaskRuntime = class {
	...
	constructor(ctx, options) {
		...
		ctx.on("agent/request", async ({ agent, signal }, next) => {
			...
			if (selectionSeq$1(agent) !== task.selectionSeq) {
	...
	async load(agent) {
		if (this.options.ownsRoot !== void 0 && !await this.options.ownsRoot(agent)) {
			this.withdraw(agent);
			return;
		}
	...
	async command(command, principal) {
		...
		if (![
			...
			"stop",
			"resume"
		].includes(command.action)) taskFailure("TASK_COMMAND_INVALID");
			...
			await this.options.store.update(agent.id, (existing) => {
				if (existing !== void 0) {
					...
					if (command.action === "resume") {
						...
						value.selectionSeq = selectionSeq$1(agent);
					} else {
				...
				return {
					...
					reserved: 0,
					selectionSeq: selectionSeq$1(agent),
					portable: false,
					handoffSeq: -1,
					receipts: [{
						id: command.operationId,
		...
		const remove = agent.ctx.tools.register({
			...
			execute: async (args, execution) => {
				...
				const updated = await this.options.store.update(agent.id, (existing) => {
					...
					if (selectionSeq$1(agent) !== value.selectionSeq) taskFailure("TASK_MANUAL_SELECTION_CHANGED");
					...
					if (!sameRoute$1(value.route, target)) {
	...
	async reserve(agent, route, revision) {
		...
		if ((await this.options.store.update(agent.id, (existing) => {
			...
			if (selectionSeq$1(agent) !== value.selectionSeq) taskFailure("TASK_MANUAL_SELECTION_CHANGED");
			if (route !== void 0 && (!sameRoute$1(value.route, route) || value.revision !== revision)) taskFailure("TASK_REQUEST_STALE");
	...
	async *stream(options, delegate) {
		...
		if (options.provider !== "openai-codex" || !sameRoute$1(route, task.route)) taskFailure("TASK_ROUTE_CHANGED");
...
const ledgerHash = (value) => typeof value === "string" && /^[a-f0-9]{64}$/u.test(value);
const ledgerId = (value) => typeof value === "string" && /^[A-Za-z0-9_-]{16,80}$/u.test(value);
const ledgerInteger = (value, min = 0, max = Number.MAX_SAFE_INTEGER) => typeof value === "number" && Number.isSafeInteger(value) && !Object.is(value, -0) && value >= min && value <= max;
const exact$1 = (value, keys) => taskRecord(value) && Object.keys(value).sort().join(",") === keys.split(",").sort().join(",");
const ids = (value, max) => Array.isArray(value) && value.length <= max && value.every(ledgerId) && new Set(value).size === value.length;
const childTerminal = (run) => [
	"succeeded",
	"failed",
	"cancelled",
	"interrupted"
].includes(run.state);
const childUnresolved = (run) => !childTerminal(run) || run.cleanup !== "verified" || run.delivery === "pending" || run.delivery === "unknown";
function parseDelegationGrant(value, capabilities) {
	if (!exact$1(value, "routes,sourceManifest,sourceIds,maxRequests,timeoutMs") || !ledgerHash(value.sourceManifest) || !ids(value.sourceIds, 32) || value.sourceIds.length === 0 || !ledgerInteger(value.maxRequests, 1, 6) || !ledgerInteger(value.timeoutMs, 1e3, 9e4) || !Array.isArray(value.routes) || value.routes.length === 0 || value.routes.length > 64 || value.routes.some((route) => !taskRoute(route) || !allowsTaskRoute(capabilities, route)) || new Set(value.routes.map((route) => `${route.model}/${route.effort}`)).size !== value.routes.length) taskFailure("TASK_DELEGATION_GRANT_INVALID");
	return structuredClone(value);
}
function parseRun(value, document) {
	if (!exact$1(value, "id,callId,argumentDigest,grantRevision,revocationGeneration,epoch,childSessionId,childSessionKey,route,sourceIds,evidenceDigest,maxRequests,attempts,deadlineAt,state,outcome,cleanup,resultDigest,delivery") || !ledgerId(value.id) || !ledgerId(value.callId) || !ledgerHash(value.argumentDigest) || !ledgerHash(value.epoch) || !ledgerHash(value.evidenceDigest) || !ledgerInteger(value.grantRevision, 1, document.delegation.grantRevision) || !ledgerInteger(value.revocationGeneration, 0, document.delegation.revocationGeneration) || !taskRoute(value.route) || !allowsTaskRoute(document.capabilities, value.route) || !ids(value.sourceIds, 32) || value.sourceIds.length === 0 || !ledgerInteger(value.maxRequests, 1, 6) || !ids(value.attempts, Number(value.maxRequests)) || !ledgerInteger(value.deadlineAt, 1) || ![
		"prepared",
		"running",
		"settling",
		"succeeded",
		...
		"cancelled",
		"interrupted"
	].includes(String(value.state)) || !(value.outcome === null || [
		"succeeded",
		"failed",
		"cancelled",
		"interrupted"
		...
		"failed",
		"verified"
	].includes(String(value.cleanup)) || ![
		"none",
		"pending",
		"recorded",
		"unknown"
	].includes(String(value.delivery)) || !(value.resultDigest === null || ledgerHash(value.resultDigest)) || !(value.childSessionId === null && value.childSessionKey === null || validTaskSessionId(value.childSessionId) && value.childSessionId !== document.sessionId && ledgerHash(value.childSessionKey) && value.childSessionKey !== document.sessionKey)) taskFailure("TASK_LEDGER_INVALID");
	const run = value;
	...
	if (["prepared", "running"].includes(run.state)) {
		...
		if (document.mode !== "auto" || grant === null || run.epoch !== document.runtime || run.grantRevision !== document.delegation.grantRevision || run.revocationGeneration !== document.delegation.revocationGeneration || !grant.routes.some((route) => route.model === run.route.model && route.effort === run.route.effort) || run.sourceIds.some((id) => !grant.sourceIds.includes(id)) || run.maxRequests !== grant.maxRequests) taskFailure("TASK_LEDGER_INVALID");
	}
	return run;
}
/** V1 remains strict; V2 adds one root-owned extension rather than widening V1 parsing. */
function parseTaskLedger(value, sessionId) {
	...
	if (!exact$1(delegation, "grant,grantRevision,revocationGeneration,runs") || !ledgerInteger(delegation.grantRevision) || !ledgerInteger(delegation.revocationGeneration) || !Array.isArray(delegation.runs) || delegation.runs.length > 32) taskFailure("TASK_LEDGER_INVALID");
	const document = value;
	...
	if (attempts.length > document.reserved || new Set(attempts).size !== attempts.length || runs.filter((run) => !childTerminal(run) || run.cleanup !== "verified").length > 1) taskFailure("TASK_LEDGER_INVALID");
	return document;
}
//#endregion
//#region src/adaptive-task-delegation-ledger.ts
...
function identify$1(current, who) {
	if (current === void 0) taskFailure("TASK_STATE_MISSING");
...
function revision$1(current, expected) {
	if (!ledgerInteger(expected, 1) || current.revision !== expected) taskFailure("TASK_STALE_REVISION");
}
function epoch(current, expected) {
	if (!ledgerHash(expected) || current.runtime !== expected) taskFailure("TASK_STALE_EPOCH");
}
function active(current, fence) {
	epoch(current, fence.epoch);
	const d = current.delegation;
	if (current.mode !== "auto" || d.grant === null || d.grantRevision !== fence.grantRevision || d.revocationGeneration !== fence.revocationGeneration) taskFailure("TASK_DELEGATION_REVOKED");
	return d.grant;
}
function find(current, id) {
	const run = current.delegation.runs.find((item) => item.id === id);
	if (run === void 0) taskFailure("TASK_CHILD_UNKNOWN");
	return run;
}
function fenceRun(current, run, fence, now) {
	active(current, fence);
	if (run.epoch !== fence.epoch || run.grantRevision !== fence.grantRevision || run.revocationGeneration !== fence.revocationGeneration) taskFailure("TASK_CHILD_BINDING_MISMATCH");
...
function capacity(document) {
	...
	for (const run of worst.delegation.runs) if (!childTerminal(run) || run.cleanup !== "verified") {
		run.childSessionId = "x".repeat(160);
		run.childSessionKey = "a".repeat(64);
		run.attempts = Array.from({ length: run.maxRequests }, () => "x".repeat(80));
		run.resultDigest = "a".repeat(64);
		run.state = "interrupted";
		run.outcome = "interrupted";
		run.cleanup = "verified";
		run.delivery = "recorded";
	}
	if (Buffer.byteLength(JSON.stringify(worst) + "\n") > 65536) taskFailure("TASK_LEDGER_CAPACITY");
}
var AdaptiveTaskDelegationLedger = class {
	now;
	store;
	...
	async read(identity) {
		...
		return structuredClone(identify$1(await this.store.read(who.sessionId), who));
	}
	async update(identity, change) {
		const who = structuredClone(identity);
		return v2(await this.store.update(who.sessionId, (current) => {
			const doc = v2(identify$1(current, who));
			change(doc);
			capacity(doc);
			return doc;
		}));
	}
	async migrate(identity, expectedRevision) {
		const who = structuredClone(identity);
		return v2(await this.store.update(who.sessionId, (current) => {
			const doc = identify$1(current, who);
			revision$1(doc, expectedRevision);
			if (doc.version === 2) return doc;
			return {
				...doc,
				version: 2,
				revision: doc.revision + 1,
				mode: doc.mode === "auto" ? "interrupted" : doc.mode,
				delegation: {
					grant: null,
					grantRevision: 0,
	...
	control(doc, expectedRevision, operation, reducing = false) {
		if (operation !== void 0) {
	...
	async configure(identity, expectedRevision, requested, operation) {
		...
		return this.update(identity, (doc) => {
			...
			doc.delegation.grantRevision++;
			doc.delegation.revocationGeneration++;
			doc.revision++;
		});
	}
	/** Caller must additionally verify host-idle ownership before invoking this primitive. */
	async resume(identity, expectedRevision, expectedEpoch, operation) {
		return this.update(identity, (doc) => {
			if (this.control(doc, expectedRevision, operation)) return;
			epoch(doc, expectedEpoch);
			if (doc.mode !== "interrupted" || doc.reserved >= doc.maximumRequests || doc.delegation.runs.some(childUnresolved)) taskFailure("TASK_RESUME_UNAVAILABLE");
			doc.mode = "auto";
			doc.revision++;
		});
	}
	/** Host verifies portable history and current model catalog before selecting a new main route. */
	async selectRoot(identity, expectedRevision, expectedEpoch, requested, selectionSeq, handoffSeq) {
		...
		return this.update(identity, (doc) => {
			...
			if (!sameRoute(doc.route, route)) {
				doc.route = route;
				doc.portable = true;
				doc.handoffSeq = handoffSeq;
				doc.revision++;
			}
		});
	}
	async prepare(identity, captured, request) {
		...
		const result = await this.update(identity, (doc) => {
			if (Object.keys(input).sort().join(",") !== "argumentDigest,callId,evidenceDigest,route,sourceIds" || !ledgerId(input.callId) || !ledgerHash(input.argumentDigest) || !ledgerHash(input.evidenceDigest) || !taskRoute(input.route) || !Array.isArray(input.sourceIds) || input.sourceIds.length === 0 || input.sourceIds.length > 32 || input.sourceIds.some((id) => !ledgerId(id)) || new Set(input.sourceIds).size !== input.sourceIds.length) taskFailure("TASK_CHILD_ARGUMENTS_INVALID");
			const existing = doc.delegation.runs.find((run) => run.callId === input.callId);
			if (existing !== void 0) {
				if (existing.argumentDigest !== input.argumentDigest || !sameRoute(existing.route, input.route) || existing.evidenceDigest !== input.evidenceDigest || [...existing.sourceIds].sort().join(",") !== [...input.sourceIds].sort().join(",")) taskFailure("TASK_OPERATION_CONFLICT");
				runId = existing.id;
				return;
			}
			const grant = active(doc, fence);
			if (doc.delegation.runs.some(childUnresolved)) taskFailure("TASK_CHILD_UNRESOLVED");
			if (doc.delegation.runs.length >= 32) taskFailure("TASK_LEDGER_CAPACITY");
			if (!grant.routes.some((route) => sameRoute(route, input.route)) || input.sourceIds.some((id) => !grant.sourceIds.includes(id))) taskFailure("TASK_CHILD_SCOPE_DENIED");
			...
			runId = randomUUID();
			created = true;
			doc.delegation.runs.push({
				id: runId,
				...input,
				epoch: fence.epoch,
				grantRevision: fence.grantRevision,
				revocationGeneration: fence.revocationGeneration,
				childSessionId: null,
				childSessionKey: null,
				maxRequests: grant.maxRequests,
				attempts: [],
				deadlineAt: now + grant.timeoutMs,
				state: "prepared",
				outcome: null,
				cleanup: "pending",
				resultDigest: null,
		...
		return {
			created,
			run: structuredClone(find(result, runId))
		};
	}
	async publish(identity, runId, captured, child) {
		const fence = structuredClone(captured), binding = structuredClone(child);
		return this.update(identity, (doc) => {
			const run = find(doc, runId);
			fenceRun(doc, run, fence, this.now());
			if (run.state !== "prepared") taskFailure("TASK_CHILD_TRANSITION_INVALID");
			run.childSessionId = binding.sessionId;
			run.childSessionKey = binding.sessionKey;
			run.state = "running";
			doc.revision++;
		});
	}
	async reserveChild(identity, runId, captured, binding, attemptId) {
		const fence = structuredClone(captured), child = structuredClone(binding);
		return this.update(identity, (doc) => {
			const run = find(doc, runId);
			fenceRun(doc, run, fence, this.now());
			if (run.state !== "running" || run.childSessionId !== child.sessionId || run.childSessionKey !== child.sessionKey || !sameRoute(run.route, child.route)) taskFailure("TASK_CHILD_BINDING_MISMATCH");
			...
			if (doc.maximumRequests - doc.reserved < 2) taskFailure("TASK_PARENT_RESERVE_REQUIRED");
			run.attempts.push(attemptId);
			doc.reserved++;
			doc.revision++;
		});
	}
	/** Read-only reconciliation; a found debit does not say transport occurred or authorize replay. */
	async lookupReservation(identity, attemptId) {
		const run = v2(await this.read(identity)).delegation.runs.find((item) => item.attempts.includes(attemptId));
		return run && {
			runId: run.id,
			epoch: run.epoch,
			debited: true,
	...
	async reserveRoot(identity, expectedEpoch, route, kind, expectedSelectionSeq) {
		...
		return this.update(identity, (doc) => {
			...
			if (kind === "main" && !sameRoute(doc.route, selected) || expectedSelectionSeq !== void 0 && doc.selectionSeq !== expectedSelectionSeq) taskFailure("TASK_REQUEST_STALE");
	...
	async settle(identity, runId, captured, outcome, resultDigest) {
		const fence = structuredClone(captured);
		return this.update(identity, (doc) => {
			const run = find(doc, runId);
			epoch(doc, fence.epoch);
			if (run.epoch !== fence.epoch || run.grantRevision !== fence.grantRevision || run.revocationGeneration !== fence.revocationGeneration || !["prepared", "running"].includes(run.state)) taskFailure("TASK_CHILD_TRANSITION_INVALID");
			if (![
				"succeeded",
				"failed",
				"cancelled"
			].includes(outcome) || (outcome === "succeeded" ? !ledgerHash(resultDigest) || run.state !== "running" : resultDigest !== null)) taskFailure("TASK_CHILD_RESULT_INVALID");
	...
	async cleanup(identity, runId, expectedEpoch, verified) {
		return this.update(identity, (doc) => {
			epoch(doc, expectedEpoch);
			const run = find(doc, runId);
			...
			if (verified && run.state === "settling") {
	...
	async delivery(identity, runId, expectedEpoch, expected, next) {
		return this.update(identity, (doc) => {
			epoch(doc, expectedEpoch);
			const run = find(doc, runId);
			if (!["pending", "unknown"].includes(expected) || !["recorded", "unknown"].includes(next) || !childTerminal(run) || run.cleanup !== "verified" || run.delivery !== expected || expected === "unknown" && next !== "recorded") taskFailure("TASK_CHILD_TRANSITION_INVALID");
			run.delivery = next;
			doc.revision++;
		});
	}
	async revoke(identity, expectedRevision, mode, handoffSeq, operation) {
		return this.update(identity, (doc) => {
...
//#endregion
//#region src/adaptive-task-delegation-host.ts
/** Optional host capabilities remain optional for ordinary standalone adapters. */
function taskHostServices(ctx) {
	const agents = ctx.get("agents"), sessions = ctx.get("sessions");
	if (agents === void 0 || sessions === void 0) taskFailure("TASK_LIVE_ROOT_REQUIRED");
	return {
		agents,
		sessions
	};
}
const allowedTools = /* @__PURE__ */ new Set(["read_task_evidence", "submit_task_findings"]);
function abortError() {
	const error = /* @__PURE__ */ new Error("TASK_CHILD_ABORTED");
	error.name = "AbortError";
	return error;
}
function definition(name, description, parameters, invoke) {
	return {
		name,
		description,
		parameters,
...
const readEvidence = (controls) => definition("read_task_evidence", "Read one approved source line range.", {
	type: "object",
	additionalProperties: false,
	required: [
		"sourceId",
	...
	controls.check();
	return controls.execute("read_task_evidence", args);
});
const submitFindings = (controls, markSubmitted) => definition("submit_task_findings", "Submit exactly one bounded findings report with evidence references.", {
	type: "object",
	additionalProperties: false,
...
*/
var TaskDelegationHost = class {
	ctx;
	constructor(ctx) {
		this.ctx = ctx;
	}
	get agents() {
		return taskHostServices(this.ctx).agents;
	}
	get sessions() {
		return taskHostServices(this.ctx).sessions;
	}
	assertRoot(parent, identity) {
		const header = parent.session.header;
		const expectedKey = taskIdentity(JSON.stringify([
			header.id,
			header.createdAt,
			header.cwd ?? "",
			header.isSeeded,
		...
		if (parent.id !== identity.sessionId || identity.sessionKey !== expectedKey || this.agents.get(parent.id) !== parent || this.sessions.get(parent.session.id) !== parent.session || !this.agents.roots().includes(parent) || header.parentSession !== void 0) throw new Error("TASK_PARENT_NOT_LIVE");
	}
	...
	async create(parent, childRun, signal, controls) {
		...
		const childOptions = {
			model: childRun.route.model,
			reasoningEffort: childRun.route.effort,
			provider: "openai-codex"
		};
		...
		const creation = {
			...
			setup: (agentCtx) => {
				...
				agentCtx.tools.guard((exec) => {
					if (submitted) {
						violation = true;
						return "TASK_CHILD_SUBMITTED";
					}
					if (!allowedTools.has(exec.name)) {
						violation = true;
						return "TASK_CHILD_TOOL_DENIED";
					}
					controls.check();
				});
				agentCtx.tools.register({
					...readEvidence(controls),
					isConcurrencySafe: () => false
				});
				agentCtx.tools.register({
					...submitFindings(controls, () => {
						submitted = true;
					}),
					isConcurrencySafe: () => false
				});
		...
		const executeRun = async (brief, runSignal) => {
		...
		return {
			agent,
			sessionId,
			sessionKey: taskIdentity(JSON.stringify([
				header.id,
				header.createdAt,
				header.cwd ?? "",
				header.isSeeded,
			...
			run: executeRun,
			dispose,
			isLive: () => !disposed && !disposing && this.isLive(agent, sessionId, parent)
		};
	}
	isLive(agent, sessionId, parent) {
		return agent.id === sessionId && this.agents.get(SessionId(sessionId)) === agent && this.sessions.get(agent.session.id) === agent.session && this.agents.isOwnedBy(agent.id, parent);
	}
};
const exact = (value, keys) => taskRecord(value) && Object.keys(value).sort().join(",") === keys.split(",").sort().join(",");
const boundedText = (value, max) => typeof value === "string" && value.trim().length > 0 && Buffer.byteLength(value) <= max && !value.includes("\0");
...
async function taskSafeDirectories(path) {
...
function safeRelative(path) {
	return path.length > 0 && path.length <= 4096 && !isAbsolute(path) && !path.includes("\\") && path.split("/").every((part) => part !== "" && part !== "." && part !== ".." && !part.startsWith(".") && !/(?:secret|credential|password|token|auth|config|private[-_]?key)/iu.test(part) && !/\.(?:pem|key|p12|pfx|keystore)$/iu.test(part) && !/^id_(?:rsa|dsa|ecdsa|ed25519)(?:\.|$)/iu.test(part));
}
async function readApproved(root, path) {
	...
	if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1 || before.size > 65536) taskFailure("TASK_EVIDENCE_FILE_DENIED");
	const file = await open(filename, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
	try {
		if (!same(before, await file.stat())) taskFailure("TASK_EVIDENCE_CHANGED");
		const bytes = Buffer.alloc(65537);
		const { bytesRead } = await file.read(bytes, 0, bytes.length, 0);
		if (bytesRead !== before.size || bytesRead > 65536 || !same(before, await file.stat()) || !same(before, await lstat(filename))) taskFailure("TASK_EVIDENCE_CHANGED");
		...
		try {
			...
			taskFailure("TASK_EVIDENCE_NOT_TEXT");
		}
...
var TaskEvidenceManifest = class TaskEvidenceManifest {
	root;
	entries;
	digest;
	constructor(root, entries) {
		...
		this.digest = taskIdentity(JSON.stringify({
			root,
			entries
		}));
	}
	static async approve(root, paths) {
		const input = [...paths];
		if (!isAbsolute(root) || resolve(root) !== root || await realpath(root) !== root || input.length === 0 || input.length > 32 || new Set(input).size !== input.length) taskFailure("TASK_EVIDENCE_MANIFEST_INVALID");
		await taskSafeDirectories(root);
		const entries = [];
		let total = 0;
		for (const path of input) {
			total += Buffer.byteLength(await readApproved(root, path));
			if (total > 262144) taskFailure("TASK_EVIDENCE_TOO_LARGE");
			entries.push({
				id: randomUUID(),
				path
			});
	...
	static restore(value, digest) {
		if (!exact(value, "root,entries") || typeof value.root !== "string" || !isAbsolute(value.root) || resolve(value.root) !== value.root || !Array.isArray(value.entries) || value.entries.length === 0 || value.entries.length > 32 || value.entries.some((item) => !exact(item, "id,path") || !ledgerId(item.id) || typeof item.path !== "string" || !safeRelative(item.path)) || new Set(value.entries.map((item) => item.id)).size !== value.entries.length || new Set(value.entries.map((item) => item.path)).size !== value.entries.length) taskFailure("TASK_EVIDENCE_MANIFEST_INVALID");
	...
	async snapshot(ids) {
		...
		for (const id of selected) {
			...
			if (entry === void 0) taskFailure("TASK_EVIDENCE_SCOPE_DENIED");
			const text = await readApproved(this.root, entry.path);
			total += Buffer.byteLength(text);
			if (total > 262144) taskFailure("TASK_EVIDENCE_TOO_LARGE");
			files.push({
				id,
				digest: taskIdentity(text),
				text
			});
		}
		return {
...
function parseTaskEvidence(value) {
	if (!exact(value, "version,manifest,files") || value.version !== 1 || !ledgerHash(value.manifest) || !Array.isArray(value.files) || value.files.length === 0 || value.files.length > 32) taskFailure("TASK_EVIDENCE_INVALID");
	let total = 0;
	for (const item of value.files) {
		if (!exact(item, "id,digest,text") || !ledgerId(item.id) || !ledgerHash(item.digest) || typeof item.text !== "string" || item.text.includes("\0") || Buffer.byteLength(item.text) > 65536 || taskIdentity(item.text) !== item.digest) taskFailure("TASK_EVIDENCE_INVALID");
		total += Buffer.byteLength(item.text);
	}
	if (total > 262144 || new Set(value.files.map((item) => item.id)).size !== value.files.length) taskFailure("TASK_EVIDENCE_INVALID");
	return structuredClone(value);
}
function parseTaskFindings(value) {
	if (!exact(value, "summary,findings") || !boundedText(value.summary, 16384) || !Array.isArray(value.findings) || value.findings.length > 10 || Buffer.byteLength(JSON.stringify(value)) > 16384) taskFailure("TASK_FINDINGS_INVALID");
	for (const finding of value.findings) {
		if (!exact(finding, "text,references") || !boundedText(finding.text, 16384) || !Array.isArray(finding.references) || finding.references.length === 0 || finding.references.length > 8) taskFailure("TASK_FINDINGS_INVALID");
		for (const ref of finding.references) if (!exact(ref, "sourceId,start,end,digest") || !ledgerId(ref.sourceId) || !ledgerInteger(ref.start, 1) || !ledgerInteger(ref.end, Number(ref.start)) || !ledgerHash(ref.digest)) taskFailure("TASK_FINDINGS_INVALID");
	}
	return structuredClone(value);
}
/** Observations are per run. Exactly one submission; any further action invalidates the unit. */
var TaskEvidenceReader = class {
	snapshot;
	observed = /* @__PURE__ */ new Set();
	result;
	failed = false;
	constructor(snapshot) {
		this.snapshot = parseTaskEvidence(snapshot);
	}
	check() {
		if (this.failed || this.result !== void 0) {
			this.failed = true;
			taskFailure("TASK_CHILD_EXTRA_ACTION");
		}
	}
	read(value) {
		this.check();
		if (!exact(value, "sourceId,start,end") || !ledgerId(value.sourceId) || !ledgerInteger(value.start, 1) || !ledgerInteger(value.end, Number(value.start), Number(value.start) + 199)) taskFailure("TASK_EVIDENCE_RANGE_INVALID");
		const file = this.snapshot.files.find((item) => item.id === value.sourceId);
		if (file === void 0) taskFailure("TASK_EVIDENCE_SCOPE_DENIED");
		...
		if (Buffer.byteLength(text) > 16384) taskFailure("TASK_EVIDENCE_RANGE_TOO_LARGE");
		const ref = {
			sourceId: file.id,
	...
	finish() {
		if (this.failed || this.result === void 0) taskFailure("TASK_FINDINGS_MISSING_OR_INVALID");
		return structuredClone(this.result);
	}
};
//#endregion
//#region src/adaptive-task-delegation.ts
/** Opt-in internal v2 execution boundary. Not constructed by the product entry point. */
const TASK_DELEGATE_TOOL = "delegate_task";
function input(value) {
	if (!taskRecord(value) || Object.keys(value).sort().join(",") !== "effort,expectedOutput,goal,model,sourceIds" || !taskRoute({
		model: value.model,
		effort: value.effort
	}) || typeof value.goal !== "string" || !value.goal.trim() || typeof value.expectedOutput !== "string" || !value.expectedOutput.trim() || Buffer.byteLength(JSON.stringify(value)) > 16384 || !Array.isArray(value.sourceIds) || value.sourceIds.length === 0 || value.sourceIds.length > 32 || value.sourceIds.some((id) => typeof id !== "string") || new Set(value.sourceIds).size !== value.sourceIds.length) taskFailure("TASK_CHILD_ARGUMENTS_INVALID");
	return {
		goal: value.goal,
...
const capture = (doc) => ({
	epoch: doc.runtime,
	grantRevision: doc.delegation.grantRevision,
	revocationGeneration: doc.delegation.revocationGeneration
});
const selectionSeq = (parent) => parent.session.snapshotEvents().findLast((event) => event.type === "model/selection")?.seq ?? -1;
var AdaptiveTaskDelegation = class {
	ctx;
	options;
	get agents() {
		return taskHostServices(this.ctx).agents;
	...
	constructor(ctx, options) {
		...
		this.removeRequest = ctx.on("agent/request", async ({ agent, signal }, next) => {
			...
			return {
				...config,
				provider: "openai-codex",
				model: doc.route.model,
				reasoningEffort: ReasoningEffortId(doc.route.effort)
			};
		}, { prepend: true });
	...
	withdraw(root) {
		...
		delete root.remove;
		root.removeModel?.();
		delete root.removeModel;
		root.life.abort(/* @__PURE__ */ new Error("TASK_DELEGATION_REVOKED"));
		this.active.get(root.parent)?.life.abort(/* @__PURE__ */ new Error("TASK_DELEGATION_REVOKED"));
	}
	async available(route, signal) {
		const actual = await this.ctx.llm.resolveModelInfo("openai-codex", route.model, signal);
		if (actual.provider !== "openai-codex" || actual.id !== route.model || !actual.reasoning?.efforts.some((item) => item.id === route.effort)) taskFailure("TASK_MODEL_UNAVAILABLE");
	}
	async document(root) {
		...
		if (root.epoch !== void 0 && doc.runtime !== root.epoch) taskFailure("TASK_STALE_EPOCH");
		const end = root.parent.session.snapshotEvents().findLast((event) => event.type === "turn/end");
		if (doc.mode === "auto" && end?.type === "turn/end" && end.seq > root.stopThrough && end.data.reason.kind === "aborted" && end.data.reason.reason.kind === "user") {
	...
	async install(parent, identity, manifest) {
		if (this.stopped || this.roots.get(parent)?.removeModel !== void 0 || this.active.has(parent)) taskFailure("TASK_DELEGATION_UNAVAILABLE");
		const root = {
			parent,
		...
		if (doc.delegation.grant !== null) {
			if (manifest === void 0 || doc.delegation.grant.sourceManifest !== manifest.digest || doc.delegation.grant.sourceIds.some((id) => !manifest.sources().some((source) => source.id === id))) taskFailure("TASK_EVIDENCE_SCOPE_DENIED");
	...
	installTools(root, doc) {
		const { parent, manifest } = root;
		if (doc.mode !== "auto") taskFailure("TASK_REQUIRES_USER_RESUME");
		root.removeModel = parent.ctx.tools.register({
			name: ADAPTIVE_TASK_TOOL,
			...
			isConcurrencySafe: () => false,
			execute: async (args, execution) => {
				if (execution.agent !== parent || execution.parent !== void 0 || !taskRecord(args) || Object.keys(args).sort().join(",") !== "effort,model,reason" || !taskRoute({
					model: args.model,
					effort: args.effort
				}) || typeof args.reason !== "string" || !args.reason.trim() || args.reason.length > 1e3) taskFailure("TASK_ACTION_INVALID");
				execution.signal.throwIfAborted();
				const current = await this.document(root), route = {
					model: String(args.model),
					effort: String(args.effort)
				};
				...
				portableTaskMessages(parent.session, parent.session.deriveMessages(), {
					afterSeq: parent.session.seq,
					model: route.model
				});
				execution.signal.throwIfAborted();
		...
		if (manifest === void 0) {
			root.removeModel();
			delete root.removeModel;
			taskFailure("TASK_EVIDENCE_SCOPE_DENIED");
		}
		try {
			root.remove = parent.ctx.tools.register({
	...
	check(active) {
		...
		if (active.owned !== void 0 && !active.owned.isLive()) taskFailure("TASK_CHILD_BINDING_MISMATCH");
	}
	/** Follow both durable and live ownership, including descendants of already-disposed run IDs. */
	async descendant(agent) {
		const live = this.agents.list();
		const pending = [];
		const parents = (id) => {
			const header = this.sessions.get(id)?.header;
			if (header?.parentSession !== void 0) pending.push(header.parentSession);
			for (const owner of live) if (this.agents.isOwnedBy(id, owner)) pending.push(owner.id);
		};
		parents(agent.id);
		if (pending.length === 0) return false;
		const managed = new Set([...this.roots.keys()].map((root) => root.id));
		for (const root of this.roots.values()) {
			const doc = await this.options.ledger.read(root.identity);
			if (doc.version === 2) {
				for (const run of doc.delegation.runs) if (run.childSessionId !== null) managed.add(run.childSessionId);
			}
		}
	...
	call(root, callId) {
		...
		if (calls.length !== 1 || calls[0].type !== "tool/call" || calls[0].data.name !== "delegate_task") taskFailure("TASK_PARENT_CALL_REQUIRED");
		return calls[0];
	}
	async receipt(root, run, callId) {
		const receipt = {
			runId: run.id,
			callId,
			status: run.state,
		...
		if (run.state === "succeeded" && run.cleanup === "verified" && run.resultDigest !== null) receipt.findings = parseTaskFindings(await this.options.artifacts(root.identity).get(run.resultDigest));
		return receipt;
	}
	async execute(root, args, execution) {
		if (execution.agent !== root.parent || execution.parent !== void 0 || execution.rootCallId !== execution.callId) taskFailure("TASK_PARENT_CALL_REQUIRED");
		execution.signal.throwIfAborted();
		...
		if (root.manifest === void 0 || doc.mode !== "auto" || doc.delegation.grant?.sourceManifest !== root.manifest.digest || request.sourceIds.some((id) => !doc.delegation.grant.sourceIds.includes(id)) || !doc.delegation.grant.routes.some((route) => route.model === request.model && route.effort === request.effort)) taskFailure("TASK_EVIDENCE_SCOPE_DENIED");
		...
		if (Buffer.byteLength(brief) > 16384) taskFailure("TASK_CHILD_ARGUMENTS_INVALID");
		execution.signal.throwIfAborted();
		...
		const prepared = await this.options.ledger.prepare(root.identity, capture(doc), {
			callId,
			argumentDigest,
			route: {
				model: request.model,
		...
		const life = new AbortController(), timer = setTimeout(() => life.abort(/* @__PURE__ */ new Error("TASK_CHILD_DEADLINE")), Math.max(0, prepared.run.deadlineAt - Date.now()));
		let complete;
		...
		const active = {
			...
			fence: capture(doc),
			life,
			signal: AbortSignal.any([life.signal, execution.signal]),
			done
		};
		this.active.set(root.parent, active);
		let outcome = "failed", resultDigest = null;
		try {
			...
			active.owned = await this.options.host.create(root.parent, active.run, active.signal, {
				...
				execute: async (name, value) => {
					...
					if (current.mode !== "auto" || current.runtime !== active.fence.epoch || current.delegation.grantRevision !== active.fence.grantRevision || current.delegation.revocationGeneration !== active.fence.revocationGeneration) taskFailure("TASK_DELEGATION_REVOKED");
					this.check(active);
			...
			await this.options.ledger.publish(root.identity, active.run.id, active.fence, {
				sessionId: active.owned.sessionId,
				sessionKey: active.owned.sessionKey
			});
			this.check(active);
			...
			outcome = active.signal.aborted ? "cancelled" : "failed";
			resultDigest = null;
		} finally {
			life.abort(/* @__PURE__ */ new Error("TASK_CHILD_FINISHED"));
			clearTimeout(timer);
			try {
				const latest = await this.document(root), run = latest.delegation.runs.find((item) => item.id === active.run.id);
				if (latest.runtime === active.fence.epoch && ["prepared", "running"].includes(run.state)) try {
					await this.options.ledger.settle(root.identity, run.id, active.fence, outcome, resultDigest);
				} catch {
					const current = await this.document(root), state = current.delegation.runs.find((item) => item.id === run.id);
					if (current.runtime === active.fence.epoch && ["prepared", "running"].includes(state.state)) await this.options.ledger.settle(root.identity, run.id, active.fence, "cancelled", null);
				}
			} finally {
				let verified = false;
				try {
					await active.owned?.dispose();
					this.options.host.assertNoChildren(root.parent);
					verified = true;
				} catch {}
				try {
					const latest = await this.document(root), run = latest.delegation.runs.find((item) => item.id === active.run.id);
					if (latest.runtime === active.fence.epoch && ["settling", "interrupted"].includes(run.state) && run.cleanup !== "verified") await this.options.ledger.cleanup(root.identity, run.id, active.fence.epoch, verified);
				} finally {
					if (verified) this.active.delete(root.parent);
					complete();
				}
			}
		}
	...
	async reconcile(parent, requireAll = false) {
		const root = this.roots.get(parent);
		if (root === void 0) taskFailure("TASK_LIVE_ROOT_REQUIRED");
		const doc = await this.document(root);
		for (const run of doc.delegation.runs) {
			const missing = () => {
				if (requireAll) taskFailure("TASK_DELIVERY_UNCONFIRMED");
			};
			if (run.cleanup !== "verified" || ![
				"pending",
				"unknown",
				...requireAll ? ["recorded"] : []
			].includes(run.delivery)) {
				missing();
				continue;
			}
			const calls = parent.session.snapshotEvents().filter((event) => event.type === "tool/call" && taskIdentity(event.data.callId) === run.callId);
			if (calls.length !== 1 || calls[0].type !== "tool/call") {
				missing();
				continue;
			}
			...
			if (results.length !== 1 || results[0].type !== "tool/result") {
				missing();
				continue;
			}
			const result = results[0];
			...
			if (result.data.turn !== call.data.turn || result.data.step !== call.data.step || result.data.error !== void 0 || message.role !== "tool" || message.toolCallId !== call.data.callId || message.isError === true || block === void 0 || message.content.length !== 1 || block.type !== "text" || block.text !== expected) {
				missing();
				continue;
			}
			if (!await this.sessions.flush(parent.session)) {
				missing();
				continue;
			}
			if (run.delivery !== "recorded") await this.options.ledger.delivery(root.identity, run.id, doc.runtime, run.delivery, "recorded");
		}
	}
	...
	async *stream(options, delegate) {
		...
		if (active === void 0) {
			const agent = options.sessionId === void 0 ? void 0 : this.agents.get(options.sessionId);
			if (agent !== void 0 && await this.descendant(agent)) taskFailure("TASK_CHILD_BINDING_MISMATCH");
			const root = agent === void 0 ? void 0 : this.roots.get(agent);
			if (root !== void 0) {
				...
				if (doc.mode === "manual") {
					...
					yield* delegate(doc.portable ? {
						...options,
						messages: portableTaskMessages(root.parent.session, options.messages, {
							afterSeq: doc.handoffSeq,
							model: options.model
						})
					} : options);
					return;
				...
				const hostDefault = auxiliary && options.reasoningEffort === void 0;
				if (this.stopped || doc.mode !== "auto" || options.provider !== "openai-codex" || options.model !== doc.route.model || !hostDefault && options.reasoningEffort !== doc.route.effort || options.purpose !== void 0 && !auxiliary) taskFailure("TASK_ROOT_ROUTE_DENIED");
				const signal = options.signal === void 0 ? root.life.signal : AbortSignal.any([options.signal, root.life.signal]);
				signal.throwIfAborted();
				await this.available(doc.route, signal);
				const transformed = {
					...options,
					signal,
					...doc.portable ? { messages: portableTaskMessages(root.parent.session, options.messages, {
						afterSeq: doc.handoffSeq,
						model: options.model
					}) } : {}
				};
				const scope = {
					route: doc.route,
					cacheKey: taskIdentity(root.identity.sessionKey + doc.runtime),
					signal,
				...
				try {
					while (true) {
						const next = await inAdaptiveTaskDispatch(scope, () => iterator.next());
						if (next.done) break;
						yield next.value;
					}
				} finally {
		...
		this.check(active);
		if (options.provider !== "openai-codex" || options.model !== active.run.route.model || options.reasoningEffort !== active.run.route.effort || options.purpose !== void 0) taskFailure("TASK_CHILD_ROUTE_DENIED");
		const owned = active.owned, signal = options.signal === void 0 ? active.signal : AbortSignal.any([options.signal, active.signal]);
		const scope = {
			route: active.run.route,
			cacheKey: taskIdentity(active.run.id + owned.sessionKey),
			signal,
			reserve: async () => {
				this.check(active);
				signal.throwIfAborted();
		...
		const iterator = inAdaptiveTaskDispatch(scope, () => delegate({
			...options,
			signal
	...
	async reserveAuxiliary() {
		...
		const active = agent === void 0 ? void 0 : [...this.active.values()].find((value) => value.owned?.agent === agent);
		if (active !== void 0) {
			this.check(active);
			if (scope === void 0 || scope.cacheKey !== taskIdentity(active.run.id + active.owned.sessionKey)) taskFailure("TASK_CHILD_AUXILIARY_DENIED");
			return;
		}
		if (agent !== void 0 && await this.descendant(agent)) taskFailure("TASK_CHILD_AUXILIARY_DENIED");
	...
	async revoke(parent, expectedRevision, mode, operation) {
		const root = this.roots.get(parent);
		if (root === void 0) taskFailure("TASK_LIVE_ROOT_REQUIRED");
		await this.document(root);
	...
	async resume(parent, identity, expectedRevision, operation) {
		const root = this.roots.get(parent);
		if (root === void 0 || root.identity.owner !== identity.owner || root.identity.sessionKey !== identity.sessionKey || root.identity.sessionId !== identity.sessionId) taskFailure("TASK_OWNER_MISMATCH");
		await parent.runMaintenance(async (signal) => {
			...
			root.stopThrough = parent.session.seq;
			root.life = new AbortController();
			try {
				this.installTools(root, resumed);
			} catch (error) {
				this.withdraw(root);
	...
	async recover(parent, identity, expectedRevision, oldEpoch, newEpoch) {
		...
		if (doc.delegation.grant !== null) root.manifest = TaskEvidenceManifest.restore(await this.options.artifacts(identity).get(doc.delegation.grant.sourceManifest), doc.delegation.grant.sourceManifest);
		this.roots.set(parent, root);
	...
	async dispose() {
		...
		for (const active of this.active.values()) active.life.abort(/* @__PURE__ */ new Error("TASK_RUNTIME_DISPOSED"));
	...
	release(parent) {
		...
		this.options.host.assertNoChildren(parent);
		const root = this.roots.get(parent);
		if (root !== void 0) this.withdraw(root);
		this.roots.delete(parent);
	}
	async unresolved(parent) {
		const root = this.roots.get(parent);
		if (root === void 0) taskFailure("TASK_LIVE_ROOT_REQUIRED");
...
//#endregion
//#region src/adaptive-task-transitions.ts
/** Explicit idle transitions. Same root lock; immutable archives retain provenance, not authority. */
function identify(value, identity) {
	if (value === void 0) taskFailure("TASK_STATE_MISSING");
...
function duplicate(value, operation) {
	...
	if (prior !== void 0 && prior.digest !== operation.digest) taskFailure("TASK_OPERATION_CONFLICT");
	return prior !== void 0;
}
function append(value, operation) {
	if (value.receipts.length >= 64) taskFailure("TASK_RECEIPT_CAPACITY");
	value.receipts.push(structuredClone(operation));
}
function canArchive(doc) {
	if (doc.mode !== "manual" || doc.delegation.grant !== null || doc.delegation.runs.some((run) => !childTerminal(run) || run.cleanup !== "verified" || run.delivery !== "recorded")) taskFailure("TASK_DOWNGRADE_UNAVAILABLE");
}
async function verifyArtifacts(doc, artifacts) {
	for (const run of doc.delegation.runs) {
		await artifacts.get(run.evidenceDigest);
		if (run.resultDigest !== null) await artifacts.get(run.resultDigest);
	}
}
var AdaptiveTaskTransitions = class {
	store;
	constructor(directory) {
		this.store = new AtomicTaskDocumentStore(directory, parseTaskLedger);
	}
	async migrate(identity, expectedRevision, operation, guard, artifacts) {
		return this.store.update(identity.sessionId, async (current) => {
			...
			if (existing.mode === "manual" || existing.mode === "stopped") {
				let archive;
				try {
					archive = await artifacts.get(existing.runtime);
				} catch (error) {
					if (error.code === "ENOENT") taskFailure("TASK_ARCHIVE_MISSING");
					throw error;
				}
				if (!taskRecord(archive) || Object.keys(archive).sort().join(",") !== "document,kind,version" || archive.kind !== "task-v2-downgrade" || archive.version !== 1) taskFailure("TASK_ARCHIVE_INVALID");
				const document = parseTaskLedger(archive.document, identity.sessionId);
				if (document.version !== 2 || document.sessionKey !== existing.sessionKey || document.owner !== existing.owner || document.maximumRequests !== existing.maximumRequests || document.reserved > existing.reserved || !isDeepStrictEqual(document.capabilities, existing.capabilities)) taskFailure("TASK_ARCHIVE_INVALID");
				canArchive(document);
				await verifyArtifacts(document, artifacts);
				delegation = {
					...document.delegation,
					grant: null,
					revocationGeneration: document.delegation.revocationGeneration + 1
				};
			...
			const next = {
				...
				runtime: taskIdentity(randomUUID()),
				mode: "interrupted",
				delegation
			};
	...
	async downgrade(identity, expectedRevision, expectedEpoch, operation, guard, artifacts) {
		return this.store.update(identity.sessionId, async (current) => {
			guard();
			...
			canArchive(doc);
			await verifyArtifacts(doc, artifacts);
			if (doc.receipts.length < 64) append(doc, operation);
			const archive = {
				kind: "task-v2-downgrade",
				version: 1,
...
var AdaptiveTaskControlRuntime = class {
	...
	attaching = /* @__PURE__ */ new WeakMap();
	...
	constructor(ctx, options) {
		...
		this.legacy = new AdaptiveTaskRuntime(ctx, {
			store: new AdaptiveTaskStore(options.directory),
			models: options.models,
			ownsRoot: async (agent) => (await this.store.read(agent.id))?.version !== 2
		});
		this.execution = new AdaptiveTaskDelegation(ctx, {
			ledger: this.ledger,
			host: this.host,
			artifacts: options.artifacts
		});
		ctx.on("agent/request", async ({ agent }, next) => {
			await this.managed(agent);
			return next();
		}, { prepend: true });
		ctx.effect(() => () => {
			this.stopped = true;
		}, "Task consent composition lifecycle");
	}
	root(id) {
		const agent = this.agents.get(SessionId(id));
		if (agent === void 0 || !this.agents.roots().includes(agent)) taskFailure("TASK_LIVE_ROOT_REQUIRED");
		return agent;
	}
	...
	async ensure(agent) {
		if (this.bound.has(agent)) {
			await this.execution.reconcile(agent);
			return;
		}
	...
	async managed(agent) {
		for (const root of this.agents.roots()) {
			...
			while (!belongs && current?.session.header.parentSession !== void 0) {
			...
			if (belongs && (await this.store.read(root.id))?.version === 2) {
	...
	async *stream(options, delegate) {
		const agent = options.sessionId === void 0 ? void 0 : this.ctx.get("agents")?.get(options.sessionId);
		yield* (agent !== void 0 && await this.managed(agent) ? this.execution : this.legacy).stream(options, delegate);
	}
	async reserveAuxiliary() {
		const registry = this.ctx.get("agents");
		const agent = typeof registry?.currentInitiator === "function" && typeof registry.get === "function" ? registry.currentInitiator() : void 0;
		await (agent !== void 0 && await this.managed(agent) ? this.execution : this.legacy).reserveAuxiliary();
	}
	async state(sessionId, principal) {
		const parent = this.root(sessionId);
		if ((await this.authorized(parent, principal))?.version !== 2) {
			const state = await this.legacy.state(sessionId, principal);
			return {
				...state,
				...state.mode === "off" ? {} : { delegation: {
					version: 1,
					idle: parent.status === "idle",
					enabled: false,
					canDowngrade: false,
					files: [],
					routes: [],
		...
		const doc = await this.ledger.read(this.identity(parent, principal));
		const grant = doc.delegation.grant;
		const manifest = grant === null ? void 0 : TaskEvidenceManifest.restore(await this.options.artifacts(doc).get(grant.sourceManifest), grant.sourceManifest);
		const config = parent.session.requestHeader()?.config;
		const current = config?.provider === "openai-codex" && config.reasoningEffort !== void 0 ? {
			model: config.model,
		...
		return {
			...
			maximumRequests: doc.maximumRequests,
			...
			delegation: {
				...
				idle: parent.status === "idle",
				enabled: grant !== null,
				canDowngrade: doc.mode === "manual" && grant === null && parent.status === "idle" && doc.delegation.runs.every((run) => !childUnresolved(run) && run.delivery === "recorded"),
				files: manifest?.sources().map((source) => source.path) ?? [],
				routes: grant?.routes ?? [],
				maxRequests: grant?.maxRequests ?? 6,
				timeoutMs: grant?.timeoutMs ?? 9e4,
				runs: doc.delegation.runs.map((run) => ({
					id: run.id,
					state: run.state,
					cleanup: run.cleanup,
					delivery: run.delivery,
					reserved: run.attempts.length
				}))
			}
		};
	}
	async command(value, principal) {
		const command = structuredClone(value);
		if (decodeTaskCommand(command) === void 0) taskFailure("TASK_COMMAND_INVALID");
		const next = (this.commands.get(command.sessionId) ?? Promise.resolve()).catch(() => {}).then(() => this.apply(command, principal));
		this.commands.set(command.sessionId, next);
		try {
			return await next;
		} finally {
			if (this.commands.get(command.sessionId) === next) this.commands.delete(command.sessionId);
		}
	}
	async apply(command, principal) {
		if (this.stopped) taskFailure("TASK_RUNTIME_DISPOSED");
		...
		const operation = {
			id: command.operationId,
			digest: taskIdentity(JSON.stringify(command))
		};
		...
		if (doc?.version !== 2 && command.action !== "upgrade") {
			if (![
				...
				"stop",
				"manual"
			].includes(command.action)) taskFailure("TASK_LEDGER_MIGRATION_REQUIRED");
			await this.legacy.command(command, principal);
			return this.state(parent.id, principal);
		}
		if (doc === void 0) taskFailure("TASK_STATE_MISSING");
		if (doc.version === 2) {
			await this.ensure(parent);
			doc = await this.ledger.read(identity);
		}
		if (doc.revision !== command.revision) taskFailure("TASK_STALE_REVISION");
		if (command.action === "manual" || command.action === "stop") await this.execution.revoke(parent, command.revision, command.action === "manual" ? "manual" : "stopped", operation);
		else if (command.action === "resume") await this.execution.resume(parent, identity, command.revision, operation);
		else await parent.runMaintenance(async (signal) => {
			const guard = () => {
				signal.throwIfAborted();
				if (this.stopped) taskFailure("TASK_RUNTIME_DISPOSED");
				this.host.assertRoot(parent, identity);
				this.host.assertNoChildren(parent);
			};
			guard();
			if (!await this.sessions.flush(parent.session)) taskFailure("TASK_DURABLE_PARENT_REQUIRED");
			guard();
			...
			const current = await this.ledger.read(identity);
			if (current.version !== 2) taskFailure("TASK_LEDGER_MIGRATION_REQUIRED");
			if (command.action === "downgrade") {
				...
				this.execution.release(parent);
				this.bound.delete(parent);
				return;
			}
			...
			if (command.action === "delegate-enable") {
				...
				if (parent.session.header.cwd === void 0) taskFailure("TASK_EVIDENCE_MANIFEST_INVALID");
				manifest = await TaskEvidenceManifest.approve(parent.session.header.cwd, command.files);
				if (await this.options.artifacts(identity).put(manifest.serialize()) !== manifest.digest) taskFailure("TASK_EVIDENCE_MANIFEST_INVALID");
			}
			guard();
			const updated = await this.ledger.configure(identity, command.revision, manifest === void 0 ? null : {
				routes: [...command.routes],
				sourceManifest: manifest.digest,
				sourceIds: manifest.sources().map((source) => source.id),
				maxRequests: command.maxChildRequests,
				timeoutMs: command.timeoutMs
			}, operation);
			this.execution.release(parent);
			this.bound.delete(parent);
			if (updated.mode === "auto") try {
				await this.execution.install(parent, identity, manifest);
				this.bound.add(parent);
			} catch (error) {
				await this.ledger.revoke(identity, updated.revision, "stopped");
				throw error;
			}
...
var TaskDelegationArtifacts = class {
	...
	async safe(create) {
		...
		if (create) await mkdir(this.directory, { mode: 448 }).catch((error) => {
			if (error.code !== "EEXIST") throw error;
		});
		const stat = await lstat(this.directory);
		if (!stat.isDirectory() || stat.isSymbolicLink() || process.platform !== "win32" && (stat.mode & 63) !== 0) taskFailure("TASK_ARTIFACT_UNSAFE");
	}
	async put(value) {
	...
	async get(digest) {
		...
		try {
			const stat = await file.stat();
			if (!stat.isFile() || stat.nlink !== 1 || stat.size > MAX_BYTES || process.platform !== "win32" && (stat.mode & 63) !== 0) taskFailure("TASK_ARTIFACT_UNSAFE");
			const bytes = Buffer.alloc(2097153);
			const { bytesRead } = await file.read(bytes, 0, bytes.length, 0);
			const after = await file.stat();
			if (bytesRead !== stat.size || after.size !== stat.size || after.mtimeMs !== stat.mtimeMs || after.ctimeMs !== stat.ctimeMs || taskIdentity(bytes.subarray(0, bytesRead).toString("utf8")) !== digest) taskFailure("TASK_ARTIFACT_CORRUPT");
...
const ADAPTIVE_TASK_RELEASE_PAUSED = "TASK_PUBLIC_RELEASE_PAUSED";
/** Keep authenticated safety exits available, but admit no new or resumed automation. */
function publicTaskCommandAllowed(action, released = false) {
	switch (action) {
		case "manual":
		case "stop":
		...
		case "resume":
		case "upgrade":
		case "delegate-enable": return released === true;
		default: return false;
	}
}
/** Preserve historical grants, accounting and truthful lifecycle state during the pause. */
function publicTaskState(state, released = false) {
	return released === true ? state : {
		...state,
		canStart: false,
		unavailable: state.unavailable ?? "TASK_PUBLIC_RELEASE_PAUSED"
	};
}
//#endregion
...
function apply(ctx, config) {
	...
	const current = () => Object.fromEntries(Object.entries({
		...
		enableReserveFallback: configValue(config.enableReserveFallback),
		enableNativeCompaction: configValue(config.enableNativeCompaction),
	...
	taskRuntime = new AdaptiveTaskControlRuntime(ctx, {
		...
		artifacts: (identity) => new TaskDelegationArtifacts(join(taskDirectory, taskIdentity(JSON.stringify([identity.owner, identity.sessionKey])) + "-artifacts")),
		models: async () => {
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).models, proxyManager, resolveProviderProxyUrl, () => resolveOpenAICodexSettings(current()).contextWindowOverrides, reservePermits, () => resolveOpenAICodexSettings(current()).enableNativeCompaction, backendRequests, taskRuntime, () => resolveOpenAICodexSettings(current()).enableImageGeneration));
	ctx.inject(["webServer"], (webCtx) => {
		registerOpenAICodexAuthRoutes(webCtx, credentials, trustedOrigins, fastMode, proxyManager, resolveProviderProxyUrl, configValue(config.oauthTimeoutMs) ?? 6e5, quota);
