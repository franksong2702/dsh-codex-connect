...
function sessionImageInventory(session) {
	const result = {
		...
		ambiguousResults: /* @__PURE__ */ new Set()
	};
	const add = (content) => {
		...
		for (const block of content) {
			...
			if (ref !== void 0) result.attachments.set(String(ref.attachmentId), ref);
		}
	};
	for (const event of session.snapshotEvents()) {
		...
		for (const image of presentation?.images ?? []) {
			...
			if (existing !== void 0 && existing.original?.assetId !== image.original?.assetId) result.ambiguousResults.add(id);
...
async function resolveImageEditInputs(ctx, assets, sources, exec) {
	...
	const candidates = inputs.map((source, index) => {
		...
		if (ref === void 0) fail$1(label, "is unavailable in this session. Select or attach the image again.");
		if ("attachment" in source && JSON.stringify(decodeImageInputAttachment(ref)) !== JSON.stringify(source.attachment)) fail$1(label, "the attachment reference does not match the session record.");
		const result = inventory.results.get(id);
		if (result !== void 0 && source.usePreview !== true) {
			if (inventory.ambiguousResults.has(id)) fail$1(label, "matches several original results. Select one exact original image.");
