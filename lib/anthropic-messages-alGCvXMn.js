import { a as appendAssistantMessageDiagnostic, l as AssistantMessageEventStream, t as calculateCost } from "./models-DKCjz2BR.js";
import { _ as parseJsonWithRepair, c as transformMessages, f as getJsonSchemaToolParameters, g as getProviderEnvValue, h as sanitizeSurrogates, m as resolveJsonSchemaStrictSampling, n as buildBaseOptions, o as getPiUserAgent, r as clampMaxTokensToContext, s as headersToRecord, t as adjustMaxTokensForThinking, v as parseStreamingJson } from "./simple-options-BexO61P-.js";
import { t as Anthropic } from "./sdk-BrEcWBXQ.js";
...
//#region node_modules/.pnpm/@earendil-works+pi-ai@0.85.1_ws@8.21.3_zod@4.4.3/node_modules/@earendil-works/pi-ai/dist/api/anthropic-messages.js
/**
		...
		try {
			...
			for await (const event of iterateAnthropicEvents(response, options?.signal)) if (event.type === "message_start") {
				...
				if (event.content_block.type === "fallback") {
					if (output.content.length > 0) throw new Error("Anthropic performed an unsupported mid-output model fallback");
					continue;
				}
			...
			if (inputTransformations && inputTransformations.length > 0) appendAssistantMessageDiagnostic(output, {
				type: "anthropic_input_transformations",
				timestamp: Date.now(),
				details: { transformations: inputTransformations.map((transformation) => ({
...
function createClient(model, apiKey, optionsHeaders, fetch, dynamicHeaders, sessionId) {
	...
	const defaultHeaders = mergeClientHeaders({
		accept: "application/json",
		"anthropic-dangerous-direct-browser-access": "true"
	}, sessionId && getAnthropicCompat(model).sendSessionAffinityHeaders ? { "x-session-affinity": sessionId } : {}, model.headers, optionsHeaders);
	return {
		client: new Anthropic({
			apiKey: apiKey ?? null,
			authToken: null,
...
function getBetaFeatures(model, context, isOAuthToken, options) {
	...
	for (const headers of [model.headers, options?.headers]) for (const [name, value] of Object.entries(headers ?? {})) if (name.toLowerCase() === "anthropic-beta") configuredFeatures = value;
	if (configuredFeatures === null) return [];
	if (configuredFeatures !== void 0) return [...new Set(configuredFeatures.split(",").map((feature) => feature.trim()).filter((feature) => feature.length > 0))];
	const features = [];
	...
	if (model.reasoning && options?.thinkingEnabled === true && (options.interleavedThinking ?? true) && model.compat?.forceAdaptiveThinking !== true) features.push(INTERLEAVED_THINKING_BETA);
	if (shouldUseServerSideFallbackBeta(model)) features.push(SERVER_SIDE_FALLBACK_BETA);
	if (model.compat?.supportsMidConvoEffort === true) features.push(MID_CONVERSATION_OUTPUT_CONFIG_BETA, THINKING_BINDING_CONTROLS_BETA);
...
function buildParams(model, context, isOAuthToken, options) {
	...
	const converted = convertMessages(transformedMessages, isOAuthToken, cacheControl, compat.allowEmptySignature, deferredToolNames, normalizeToolName, model.compat?.supportsMidConvoEffort === true ? model.provider : void 0);
	...
	if (options?.temperature !== void 0 && !options?.thinkingEnabled && model.compat?.supportsMidConvoEffort !== true && compat.supportsTemperature) params.temperature = options.temperature;
	...
	if (allowedFallbackModels && allowedFallbackModels.length > 0) params.fallbacks = allowedFallbackModels.map((fallback) => ({ model: fallback.model }));
	return params;
}
...
function convertMessages(transformedMessages, isOAuthToken, cacheControl, allowEmptySignature = false, deferredToolNames = /* @__PURE__ */ new Set(), normalizeToolName = (name) => name, managedProvider) {
	const params = [];
	...
	for (let i = 0; i < transformedMessages.length; i++) {
		...
		if (msg.role === "user") {
			...
			if (managedProvider !== void 0 && msg.api === "anthropic-messages" && msg.provider === managedProvider && isAnthropicEffort(msg.providerThinkingLevel)) assistantLevels.set(messageIndex, msg.providerThinkingLevel);
...
function insertThinkingLevelMessages(converted, activeEffort) {
	...
	for (let index = 0; index < converted.messages.length; index++) {
		...
		if (historicalEffort !== void 0) messages.push({
			role: "system",
			content: [],
