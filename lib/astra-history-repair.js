...
async function repairAstraHistoryFile(input, output) {
	...
	const encoded = output.endsWith(".zstd") ? Buffer.concat([zstdCompressSync(outputBytes.subarray(0, headerEnd)), zstdCompressSync(outputBytes.subarray(headerEnd))]) : outputBytes;
