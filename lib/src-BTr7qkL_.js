...
function reserveIdentity(access) {
	...
	if (!isRecord$4(decoded)) return void 0;
	const claims = decoded["https://api.openai.com/auth"];
	if (!isRecord$4(claims)) return void 0;
...
function parseReserveUsage(value, identity) {
	...
	if (isRecord$4(banner) && banner["banner_type"] === "luna_reserve" && validBanner(banner)) {
		...
		const reserves = (additional ?? []).filter((limit) => isRecord$4(limit) && limit["limit_name"] === "gpt-reserve");
		if (reserves.length > 1) return { kind: "unavailable" };
		const reserveLimit = reserves[0]?.["rate_limit"];
		if (isRecord$4(reserveLimit) && (reserveLimit["allowed"] === false || reserveLimit["limit_reached"] === true)) return { kind: "exhausted" };
	...
	const hasCredits = isRecord$4(credits) && (credits["has_credits"] === true || credits["unlimited"] === true);
	if (isRecord$4(rateLimit) && typeof rateLimit["allowed"] === "boolean" && (rateLimit["allowed"] || hasCredits) && (spendControl === void 0 || spendControl === null || isRecord$4(spendControl) && spendControl["reached"] === false) && (value["rate_limit_reached_type"] === void 0 || value["rate_limit_reached_type"] === null)) return { kind: "ordinary" };
	const additional = value["additional_rate_limits"];
	if (isRecord$4(rateLimit) && rateLimit["allowed"] === false && Array.isArray(additional) && additional.some((limit) => isRecord$4(limit) && limit["limit_name"] === "gpt-reserve" && isRecord$4(limit["rate_limit"]) && limit["rate_limit"]["allowed"] === false)) return { kind: "exhausted" };
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.35";
//#endregion
