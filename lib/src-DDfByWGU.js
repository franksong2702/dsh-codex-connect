...
import { LlmError, QUOTA_EXCEEDED_CODE, ReasoningEffortId, createUserMessage, isAgentLoopRequest, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
var ErrorFrameObserver = class {
	...
	observe(data) {
		...
		this.stop();
		const response = record$5(event["response"]) ? event["response"] : void 0;
		const nested = record$5(event["error"]) ? event["error"] : response !== void 0 && record$5(response["error"]) ? response["error"] : void 0;
...
function assertOpen(scope) {
	scope.signal.throwIfAborted();
	if (scope.closed || scope.childId === void 0) throw new Error("SPLIT_DISPATCH_CLOSED");
}
/** Force SSE and zero provider retries; reserve every payload before it can reach fetch. */
function withSplitProviderBounds(provider) {
	return {
		...provider,
		streamSimple(model, context, options) {
			const scope = storage.getStore();
			if (scope === void 0) return provider.streamSimple(model, context, options);
			assertOpen(scope);
			if (options === void 0 || model.provider !== "openai-codex" || model.id !== "gpt-5.6-luna" || options.sessionId !== scope.childId) throw new Error("SPLIT_ROUTE_DENIED");
			...
			const bounded = {
				...
				transport: "sse",
				maxRetries: 0,
				signal: options.signal === void 0 ? scope.signal : AbortSignal.any([options.signal, scope.signal]),
				async onPayload(payload, payloadModel) {
					assertOpen(scope);
					const replacement = await onPayload?.(payload, payloadModel);
					assertOpen(scope);
					const next = replacement === void 0 ? payload : replacement;
					if (!record$4(next) || next["model"] !== "gpt-5.6-luna" || next["service_tier"] !== void 0 || next["stream"] !== true || next["store"] !== false || !record$4(next["reasoning"]) || next["reasoning"]["effort"] !== "low" || !Array.isArray(next["tools"]) || next["tools"].some((tool) => !record$4(tool) || tool["type"] !== "function" || !["split_read_evidence", "structured_output"].includes(String(tool["name"]))) || Buffer.byteLength(JSON.stringify(next)) > 512e3) throw new Error("SPLIT_PAYLOAD_DENIED");
...
function requestProvider(provider, fastMode, proxyManager, resolveProxyUrl, backendRequests, requestReplay) {
...
function createOpenAICodexProfile(provider, fastMode, proxyManager, resolveProxyUrl, contextWindowOverrides, backendRequests, requestReplay) {
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits, nativeCompactionEnabled, backendRequests, requestReplay, taskDispatch) {
	...
	class OpenAICodexAdapter extends PiAiAdapter {
		streamPrepared(stream, options) {
			const dispatch = (request) => {
				const delegate = () => streamWithCodexRequestDiagnostics((next) => streamWithNativeCompactionScope(stream, next, nativeCompactionEnabled?.() === true), request);
...
function resolveOpenAICodexSettings(value) {
	...
	if (typeof resolved.enableReasoningUpdates !== "boolean") throw new TypeError("OpenAI Codex enableReasoningUpdates must be a boolean");
...
function decodeOpenAICodexSettings(value) {
	...
	if (enableReasoningUpdates !== void 0 && typeof enableReasoningUpdates !== "boolean") return void 0;
...
function assessment(text) {
	...
	if (!record$3(value)) return false;
...
function completedResponse(value) {
	if (!record$3(value) || value["status"] !== "completed" || typeof value["model"] !== "string" || !Array.isArray(value["output"])) return void 0;
	return value["output"].flatMap((item) => record$3(item) && item["type"] === "message" && item["role"] === "assistant" && Array.isArray(item["content"]) ? item["content"].flatMap((part) => record$3(part) && part["type"] === "output_text" && typeof part["text"] === "string" ? [part["text"]] : []) : []);
}
...
function completedStream(text) {
	...
	for (const line of text.split(/\r\n|\r|\n/u)) if (line === "") {
		...
		if (!record$3(event) || event["type"] === "error" || event["type"] === "response.failed" || event["type"] === "response.incomplete") return false;
...
function parseAutoReviewAssessment(text) {
	...
	if (!record$2(value) || Object.keys(value).some((key) => ![
		"risk_level",
		"user_authorization",
		"outcome",
		"rationale"
...
function completedAssessment(value) {
	if (!record$2(value) || value["status"] !== "completed" || !Array.isArray(value["output"])) return void 0;
	const texts = value["output"].flatMap((item) => record$2(item) && item["type"] === "message" && Array.isArray(item["content"]) ? item["content"].flatMap((part) => record$2(part) && part["type"] === "output_text" && typeof part["text"] === "string" ? [part["text"]] : []) : []);
...
function canonicalJson(value) {
	...
	if (record$1(value)) return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${canonicalJson(value[key])}`).join(",")}}`;
...
function readReasoningUpdate(message) {
	...
	let value;
	if ("reasoningUpdate" in source || "reasoningSelectionSeq" in source) reasoningUpdateError("Legacy Astra reasoning metadata requires a separately reviewed migration; no history was changed.");
	...
	const update = value;
	if (message.role !== "user" || message.content.length !== 1 || message.content[0]?.type !== "text" || message.content[0].text !== reasoningUpdateText(update)) reasoningUpdateError("The saved Astra reasoning notice does not match its confirmed update.");
	return update;
}
/**
...
function applyReasoningUpdates(payload, plan) {
	...
	if (payload.context_management !== void 0 || payload.truncation !== void 0 && payload.truncation !== "disabled" || payload.previous_response_id !== void 0 || payload.agents !== void 0 || payload.agent !== void 0 || payload.input.some((item) => record(item) && item.type === "configuration_update")) reasoningUpdateError("Astra reasoning updates require source-validated, single-agent request history.");
	...
	for (let i = 0; i < payload.input.length; i += 1) {
		...
		if (checkpoint?.userIndex === userIndex) {
			const expected = checkpoint.nativeItems ?? [{
				role: "user",
				content: [{
					type: "input_text",
					text: checkpoint.content.map((block) => {
			...
			if (isDeepStrictEqual(payload.input.slice(i, i + expected.length), expected)) {
				...
				if (checkpoint.effort !== void 0) input.push({
					type: "configuration_update",
					reasoning: { effort: checkpoint.effort }
				});
				checkpointIndex += 1;
				userIndex += 1;
				i += expected.length - 1;
			...
			if (record(item) && (item.role === "user" || item.type === "compaction")) reasoningUpdateError("Think checkpoint projection differs from its verified host source.");
		}
		if (record(item) && (item.type === "compaction" || item.type === "compaction_trigger")) {
			if (native && i === payload.input.length - 1 && item.type === "compaction_trigger") {
				input.push(item);
				continue;
			}
	...
	if (userIndex !== expectedUsers || updateIndex !== plan.updates.length || checkpointIndex !== checkpoints.length) reasoningUpdateError("The Astra request conversion did not preserve its user-message positions and checkpoints.");
	return {
		...payload,
		input,
...
//#endregion
//#region src/reasoning-update-checkpoint.ts
...
function recordedReasoningBase(session) {
	for (const event of session.snapshotEvents()) if (event.type === "user/message") {
		const update = readReasoningUpdate(event.data);
		if (update !== void 0) return update.baseEffort;
	}
}
function checkpointCount(events, event, shadowed, noticeIndices, range) {
	if (event.type !== "user/message" || event.data.source.kind !== "plugin" || event.data.source.plugin !== "compact") fail();
	const source = event.data.source;
	if (typeof source.compactionId !== "string" || event.surfaceOp === void 0 || event.surfaceOp === "append") fail();
	...
	const end = ends[0];
	if (starts.length !== 1 || ends.length !== 1 || start?.type !== "compaction/start" || end?.type !== "compaction/end" || start.seq >= event.seq || end.seq <= event.seq || end.data.error !== void 0 || end.data.turn !== start.data.turn || summary?.type !== "compaction/summary" || summary.data.compactionId !== source.compactionId || summary.data.provider !== "openai-codex" || summary.data.model !== "gpt-6-astra" || summary.data.llmStreamCall !== true || events[position + 1] !== end || source.sourceCommandId !== start.data.sourceCommandId || summary.data.sourceCommandId !== start.data.sourceCommandId || end.data.sourceCommandId !== start.data.sourceCommandId) fail();
	...
	if (!isDeepStrictEqual(range.shadowedSeqs, seqs) || !isDeepStrictEqual(summary.data.shadowedSeqs, seqs) || summary.data.shadowedRange.start !== range.start || summary.data.shadowedRange.end !== range.end || !isDeepStrictEqual(event.sourceEventSeqs, [
		start.seq,
		summary.seq,
		...seqs
	])) fail();
	...
	for (const node of shadowed) {
		...
		const index = node.message === null ? void 0 : noticeIndices.get(node.message.id);
		if (index !== void 0) {
			if (index !== count) fail();
			count += 1;
		}
...
function journal(session) {
	const events = session.snapshotEvents();
	const notices = events.flatMap((event) => event.type === "user/message" && readReasoningUpdate(event.data) !== void 0 ? [event.data] : []);
	const first = notices[0] === void 0 ? void 0 : readReasoningUpdate(notices[0]);
	if (first === void 0) return void 0;
	planReasoningUpdates({
		provider: "openai-codex",
		model: "gpt-6-astra",
		sessionId: session.id,
		reasoningEffort: ReasoningEffortId(first.baseEffort),
		messages: notices
	});
	...
	for (const event of events) {
		...
		if (start !== (nodes[0]?.eventType === "system/message" ? 1 : 0) || end < start || message === null) reasoningUpdateError(`Think refuses an unsupported surface replacement (type=${event.type}; start=${start}; end=${end}; head=${nodes[0]?.eventType}; message=${message === null ? "none" : message.role}).`);
	...
	for (const node of nodes) {
		...
		const index = node.message === null ? void 0 : indices.get(node.message.id);
		if (index !== void 0) {
			if (index !== count) fail();
			count += 1;
		}
	...
	const visible = new Map(nodes.flatMap((node) => node.message !== null && checkpoints.has(node.message.id) ? [[node.message.id, checkpoints.get(node.message.id)]] : []));
	return {
		base: first.baseEffort,
		notices,
		nodes,
		checkpoints: visible
	};
}
function plan(options, state, messages, compaction) {
	if (options.provider !== "openai-codex" || options.model !== "gpt-6-astra" || options.purpose !== void 0 && !compaction) reasoningUpdateError("Think checkpoint state requires the original Astra route; other auxiliary or delegated requests are unsupported.");
	...
	for (const message of messages) {
		...
		if (absorbed !== void 0) {
			if (message.source.kind !== "plugin" || message.source.plugin !== "compact" || !isDeepStrictEqual(state.nodes.find((node) => node.message?.id === message.id)?.message, message)) fail();
			const nativeItems = decodeNativeCompactionCheckpoint(message);
			const hidden = absorbed === 0 ? void 0 : readReasoningUpdate(state.notices[absorbed - 1]);
			if (hidden !== void 0) {
				previous = effective;
				effective = hidden.effort;
			}
			checkpoints.push({
				userIndex: userCount,
				content: structuredClone(message.content),
				...nativeItems === void 0 ? {} : { nativeItems: structuredClone(nativeItems) },
				...hidden === void 0 ? {} : { effort: effective }
			});
		}
		const update = readReasoningUpdate(message);
		if (update !== void 0) {
			if (update.sessionId !== options.sessionId || update.baseEffort !== state.base || update.previousEffort !== effective || update.effort === effective) fail();
			previous = effective;
	...
	if (!isAstraReasoningEffort(requestEffort) || ![
		state.base,
		previous,
		effective
	].includes(requestEffort)) reasoningUpdateError("Think checkpoint effort does not match the admitted state.");
	const leading = messages[0];
	const leadingSystemText = options.system === void 0 && leading?.role === "system" ? leading.content.filter((block) => block.type === "text").map((block) => block.text).join("") : void 0;
	return Object.freeze({
		baseEffort: state.base,
...
function prepareCheckpointReasoningReplay(options, session) {
	...
	if (options.purpose === "compaction") {
		...
		if (lifecycle?.type !== "compaction/start" || prefix.length === 0 || prefix.length >= surface.length || !isDeepStrictEqual(prefix, surface.slice(0, prefix.length)) || instruction?.role !== "user" || instruction.source.kind !== "plugin" || instruction.source.plugin !== "dsh-compaction-basic" || instruction.content.length !== 1 || instruction.content[0]?.type !== "text" || instruction.content[0].text.trim() === "") fail();
		return plan(options, state, options.messages, true);
	}
	if (options.purpose !== void 0 || !isDeepStrictEqual(options.messages, surface)) fail();
...
function planCheckpointReasoningAdmission(options, session, additions) {
	const state = journal(session);
	if (state === void 0) return planReasoningUpdates(options);
	if (options.sessionId !== session.id || options.purpose !== void 0 || !isDeepStrictEqual(options.messages, [...session.deriveMessages(), ...additions])) fail();
	return plan(options, state, options.messages, false);
}
//#endregion
//#region src/reasoning-update-history.ts
...
*/
function prepareReasoningReplay(options, session) {
	const requested = options.messages.filter((message) => readReasoningUpdate(message) !== void 0);
	const recorded = session?.snapshotEvents().flatMap((event) => event.type === "user/message" && readReasoningUpdate(event.data) !== void 0 ? [event.data] : []) ?? [];
	if (requested.length === 0 && recorded.length === 0) return void 0;
	if (session === void 0 || options.sessionId !== session.id) reasoningUpdateError("Think replay requires the original host-owned session.");
	if (requested.length > 0 && recorded.length === 0) reasoningUpdateError("An unrecorded Think notice cannot enter a compaction or replaced-surface request.");
	if (options.purpose === "compaction" || session.surface.replaceGeneration !== 0) return prepareCheckpointReasoningReplay(options, session);
	if (options.purpose !== void 0) reasoningUpdateError("Auxiliary requests are not supported by this Think replay mechanism.");
	const surface = session.deriveMessages();
	const visible = surface.filter((message) => readReasoningUpdate(message) !== void 0);
	if (requested.length !== recorded.length || visible.length !== recorded.length) reasoningUpdateError("A confirmed Think update is missing or unrecorded; no model request was started.");
	for (const [index, message] of requested.entries()) {
		const event = recorded[index];
		if (message.id !== event.id || visible[index]?.id !== event.id || !isDeepStrictEqual(message.source, event.source) || !isDeepStrictEqual(message.content, event.content)) reasoningUpdateError("Think request state differs from its recorded notice or order.");
	}
	if (!isDeepStrictEqual(options.messages, surface)) reasoningUpdateError("Think replay requires the complete unchanged session surface, not moved notices or edited background.");
	return planReasoningUpdates(options);
}
//#endregion
//#region src/reasoning-update-provider.ts
...
*/
function projectReasoningPlan(plan, context) {
	const userCount = context.messages.filter((message) => message.role === "user").length;
	if (userCount === plan.userCount) return plan;
	if (plan.leadingSystemText === void 0 || userCount !== plan.userCount - 1 || context.systemPrompt !== (plan.leadingSystemText || void 0)) reasoningUpdateError("The Astra context conversion did not preserve its user-message positions or leading system prompt.");
	return {
		...plan,
		userCount,
		updates: plan.updates.map((update) => ({
			...update,
			userIndex: update.userIndex - 1
		})),
		...plan.checkpoints === void 0 ? {} : { checkpoints: plan.checkpoints.map((checkpoint) => ({
			...checkpoint,
			userIndex: checkpoint.userIndex - 1
		})) }
	};
}
/** Captures each stream independently, including prepared calls and concurrent sessions. */
var AstraReasoningRequestScope = class {
	...
	wrapProvider(provider) {
		...
		return {
			...
			streamSimple(model, context, options) {
				...
				if (captured === void 0) return streamSimple.call(provider, model, context, options);
				if (provider.id !== "openai-codex" || model.provider !== "openai-codex" || model.id !== "gpt-6-astra") reasoningUpdateError("Think replay cannot be applied to another provider or model.");
				...
				return streamSimple.call(provider, model, context, {
					...
					async onPayload(payload, payloadModel) {
						...
						return applyReasoningUpdates(replaced === void 0 ? payload : replaced, plan);
					}
				});
			}
		};
	...
	async *stream(options, delegate, session) {
		const plan = prepareReasoningReplay(options, session);
		const iterator = this.current.run(plan, () => delegate()[Symbol.asyncIterator]());
		let completed = false;
		try {
			while (true) {
...
function invalid(message) {
	throw new TypeError(`Adaptive decision: ${message}`);
}
function object(value) {
	return typeof value === "object" && value !== null && !Array.isArray(value);
}
function appendBounded(items, item) {
	items.push(item);
	if (items.length > 64) items.shift();
}
/** Project only the four non-content fields needed to invalidate an obsolete recommendation. */
function observeAdaptiveState(state) {
	if (!isAstraReasoningEffort(state.baseEffort) || !isAstraReasoningEffort(state.effectiveEffort) || !Number.isSafeInteger(state.revision) || state.revision < -1 || !Number.isSafeInteger(state.generation) || state.generation < 0) invalid("invalid host observation");
	return Object.freeze({
		baseEffort: state.baseEffort,
		effectiveEffort: state.effectiveEffort,
		revision: state.revision,
		generation: state.generation
	});
}
function sameObservation(a, b) {
	return a.baseEffort === b.baseEffort && a.effectiveEffort === b.effectiveEffort && a.revision === b.revision && a.generation === b.generation;
}
function actionFor(value, observation) {
	...
	if (value.kind !== "reasoning" || keys !== "effort,kind" || !isAstraReasoningEffort(value.effort)) invalid("unsupported action or authority-bearing extra fields");
	return value.effort === observation.effectiveEffort ? Object.freeze({ kind: "keep" }) : Object.freeze({
		kind: "reasoning",
		effort: value.effort
	});
}
...
var AdaptiveDecisionFlow = class {
	...
	discard(recommendation, phase) {
		const previous = this.phases.get(recommendation);
		if (previous === void 0) invalid("unknown recommendation");
	...
	settleDelegation(recommendation, outcome) {
		...
		if (phase === void 0) invalid("unknown recommendation");
	...
	async *measure(descriptor, stream) {
		...
		try {
			...
			appendBounded(this.requests, Object.freeze({
				ordinal,
...
//#endregion
//#region src/reasoning-update-host.ts
/** Think host integration: saved opt-in permits proposals, never automatic consent. */
const ASTRA_REASONING_TOOL_NAME = "codex_connect_set_reasoning_effort";
function selectionRevision(session) {
	return session.snapshotEvents().findLast((event) => event.type === "model/selection")?.seq ?? -1;
}
function pendingSelection(session) {
	const events = session.snapshotEvents();
	...
	const admitted = events.some((event) => event.seq > selection.seq && event.type === "user/message" && readReasoningUpdate(event.data) !== void 0 && readReasoningSelectionOrdinal(event.data) === ordinal);
	const consumed = events.some((event) => event.seq > selection.seq && event.type === "request/header" && event.data.header.config.provider === selection.data.provider && event.data.header.config.model === selection.data.model && event.data.header.config.reasoningEffort === selection.data.reasoningEffort);
	return admitted || consumed ? void 0 : {
		...selection.data,
		ordinal
	};
}
...
*/
function registerThinkHostIntegration(ctx) {
	const scope = new AstraReasoningRequestScope();
	const flows = /* @__PURE__ */ new WeakMap();
	const flowFor = (agent) => {
		let flow = flows.get(agent);
		if (flow === void 0) {
			flow = new AdaptiveDecisionFlow();
			flows.set(agent, flow);
		}
		return flow;
	};
	const pending = /* @__PURE__ */ new Map();
	const invalidAdmissions = /* @__PURE__ */ new WeakSet();
	...
	function assertOwner(agent) {
		...
		if (agents?.get(agent.id) !== agent || !agents.roots().includes(agent) || ctx.get("sessions")?.get(agent.id) !== agent.session) reasoningUpdateError("Think changes require the exact live root Agent and its host-owned Session.");
	}
	function discardPending(agent, phase) {
		const queued = pending.get(agent);
		if (queued !== void 0) flows.get(agent)?.discard(queued.recommendation, phase);
	...
	ctx.on("agent/pre-step", async ({ agent, turn, step, signal }, next) => {
		...
		const queued = pending.get(agent);
		if (base === void 0 && queued === void 0) return decision;
		assertOwner(agent);
		const additions = [...decision.messages];
		if (queued !== void 0) {
			if (enabled && !queued.signal.aborted && queued.revision === selectionRevision(agent.session) && queued.generation === agent.session.surface.replaceGeneration) additions.push(queued.notice);
			else discardPending(agent, queued.signal.aborted || !enabled ? "cancelled" : "stale");
		}
		const selected = pendingSelection(agent.session);
		if (selected !== void 0 && base !== void 0) {
			if (selected.provider !== "openai-codex" || selected.model !== "gpt-6-astra" || !isAstraReasoningEffort(selected.reasoningEffort)) reasoningUpdateError("Think history requires the original Astra route and an explicit effort.");
			const previous = planCheckpointReasoningAdmission({
				provider: "openai-codex",
				model: "gpt-6-astra",
				sessionId: agent.id,
				reasoningEffort: ReasoningEffortId(base),
			...
			if (selected.reasoningEffort !== previous.effectiveEffort) additions.push(createReasoningUpdateMessage({
				...
				sessionId: agent.id,
				baseEffort: base,
				previousEffort: previous.effectiveEffort,
		...
		prepared.set(agent, {
			...
			step,
			signal,
			messages: structuredClone(additions),
			revision: selectionRevision(agent.session),
			...active === void 0 ? {} : { pending: active }
		});
		return {
			...decision,
			messages: additions
	...
	ctx.on("agent/request", async ({ agent, turn, step, signal }, next) => {
		...
		const current = captured?.turn === turn && captured.step === step && captured.signal === signal ? captured : void 0;
		...
		const base = recordedReasoningBase(agent.session) ?? messages.map(readReasoningUpdate).find((value) => value !== void 0)?.baseEffort;
		if (base === void 0) return config;
		assertOwner(agent);
		if (stopped) reasoningUpdateError("Think admission is unavailable for this surface.");
		if (current !== void 0 && current.revision !== selectionRevision(agent.session)) reasoningUpdateError("Manual selection changed during request admission; no new effort was published.");
		if (current?.pending !== void 0 && !ids.has(current.pending.notice.id) && (!enabled || current.pending.signal.aborted || pending.get(agent) !== current.pending || current.pending.generation !== agent.session.surface.replaceGeneration)) reasoningUpdateError("The pending Think approval was canceled before admission.");
		...
		const plan = planCheckpointReasoningAdmission({
			...config,
			reasoningEffort: ReasoningEffortId(base),
			sessionId: agent.id,
			messages
		...
		const selected = pendingSelection(agent.session);
		if (selected !== void 0 && (selected.provider !== config.provider || selected.model !== config.model || selected.reasoningEffort !== plan.effectiveEffort)) reasoningUpdateError("The latest manual selection was not preserved by Think admission.");
		return {
			...config,
			reasoningEffort: ReasoningEffortId(plan.effectiveEffort)
		};
	}, { prepend: true });
	function replaySession(options) {
		const session = options.sessionId === void 0 ? void 0 : ctx.get("sessions")?.get(options.sessionId);
		if (session !== void 0 && invalidAdmissions.has(session)) reasoningUpdateError("Think admission integrity failed; this live session cannot dispatch further requests.");
		const agent = options.sessionId === void 0 ? void 0 : ctx.get("agents")?.get(options.sessionId);
		const captured = agent === void 0 ? void 0 : prepared.get(agent);
		if (options.purpose === void 0 && captured !== void 0 && !captured.signal.aborted) for (const expected of captured.messages.filter((message) => readReasoningUpdate(message) !== void 0)) {
			const actual = options.messages.find((message) => message.id === expected.id);
			if (actual === void 0 || !isDeepStrictEqual(actual.source, expected.source) || !isDeepStrictEqual(actual.content, expected.content)) {
				if (session !== void 0) invalidAdmissions.add(session);
				reasoningUpdateError("The approved Think notice was removed or changed before model dispatch.");
			}
		}
		const plan = prepareReasoningReplay(options, session);
		if (plan !== void 0) {
			if (agent === void 0) reasoningUpdateError("Think replay requires a live root Agent.");
			assertOwner(agent);
			if (stopped) reasoningUpdateError("Think integration was disposed before dispatch.");
			const queued = pending.get(agent);
			if (queued !== void 0 && options.purpose === void 0 && session?.deriveMessages().some((message) => message.id === queued.notice.id) && session.requestHeader()?.config.reasoningEffort === plan.effectiveEffort) {
	...
	const forget = ({ agent }) => {
		discardPending(agent, "cancelled");
		prepared.delete(agent);
	};
	...
	ctx.on("agent/disposed", (event) => {
		forget(event);
		flows.delete(event.agent);
	});
	function selection(agent, requireThinkEnabled = true) {
		assertOwner(agent);
		...
		if (config?.provider !== "openai-codex" || config.model !== "gpt-6-astra" || !isAstraReasoningEffort(config.reasoningEffort) || header?.adapterDefaults?.reasoningEffort === true) reasoningUpdateError("Select Astra and an explicit initial effort before proposing a change.");
		const selected = pendingSelection(agent.session);
		if (selected !== void 0 && (selected.provider !== config.provider || selected.model !== config.model || selected.reasoningEffort !== config.reasoningEffort)) reasoningUpdateError("A newer manual selection must enter a request first.");
		const plan = prepareReasoningReplay({
			...config,
			sessionId: agent.id,
			messages: agent.session.deriveMessages()
		}, agent.session);
		return {
			baseEffort: plan?.baseEffort ?? config.reasoningEffort,
			effectiveEffort: plan?.effectiveEffort ?? config.reasoningEffort,
			revision: selectionRevision(agent.session),
			generation: agent.session.surface.replaceGeneration
		};
	}
	async function change(agent, effort, reason, signal) {
		if (effort !== "keep" && !isAstraReasoningEffort(effort) || reason.trim().length === 0 || reason.length > 2e3) reasoningUpdateError("Provide keep or a supported effort and a reason of 1–2000 characters.");
		const combined = signal === void 0 ? questionLifetime.signal : AbortSignal.any([signal, questionLifetime.signal]);
		...
		const recommendation = flow.recommend(before, effort === "keep" ? { kind: "keep" } : {
			kind: "reasoning",
			effort
		});
		if (recommendation.action.kind === "keep") return {
			status: "unchanged",
			effort: before.effectiveEffort,
			message: "Keep the current effort. No confirmation or configuration change is needed."
		};
		...
		try {
			...
			if (questions === void 0) reasoningUpdateError("No native human-question service is available.");
			...
			const answer = await questions.ask({
				...
				questions: [{
					...
					question: `Change this conversation from ${before.effectiveEffort} to ${target}?`,
					...
					options: [{
						label: approve,
						description: "Approve this exact change."
					}, {
						label: "Keep current effort",
						description: "Continue without changing effort."
					}],
					multiSelect: false
				}]
			});
			combined.throwIfAborted();
			if (answer.answers.length !== 1 || answer.answers[0]?.id !== "astra-reasoning-effort" || answer.answers[0].selected.length !== 1 || answer.answers[0].selected[0] !== approve || answer.answers[0].custom !== void 0 && answer.answers[0].custom.trim() !== "") {
				flow.discard(recommendation, "declined");
				return {
					status: "declined",
					effort: before.effectiveEffort,
					message: "No reasoning change was approved."
				};
			}
			...
			pending.set(agent, {
				...
				signal: combined,
				notice: createReasoningUpdateMessage({
					version: 1,
					sessionId: agent.id,
					baseEffort: before.baseEffort,
					previousEffort: before.effectiveEffort,
					effort: target
				})
			});
			return {
				status: "queued",
				effort: target,
				message: `The user approved ${target}. It becomes effective only when the next request is recorded; cancellation or a later manual choice can discard this pending change.`
			};
		} catch (error) {
	...
	function registerTool(toolCtx) {
		toolCtx.tools.register(defineTool({
			...
			parameters: {
				effort: {
					type: "string",
					enum: ["keep", ...ASTRA_REASONING_EFFORTS],
					required: true,
					description: "Keep current effort or recommend one supported level."
				},
				reason: {
					type: "string",
					required: true,
					description: "Reason for the next unit of work."
				}
			},
			output: {
				schema: {
			...
			isConcurrencySafe: () => false,
			execute(args, exec) {
				if (exec.agent === void 0) reasoningUpdateError("Think requires a live root Agent.");
	...
	const integration = {
		adapterReplay: {
			...
			stream: (options, delegate) => {
				...
				const agent = options.sessionId === void 0 ? void 0 : ctx.get("agents")?.get(options.sessionId);
				if (agent === void 0 || stopped || !enabled && !flows.has(agent) || options.provider !== "openai-codex" || options.model !== "gpt-6-astra" || !ctx.get("agents")?.roots().includes(agent) || agent.session !== session) return stream;
				return flowFor(agent).measure({
					purpose: options.purpose === void 0 ? "task" : options.purpose === "compaction" ? "compaction" : "auxiliary",
					effort: options.reasoningEffort,
					signal: options.signal
				}, stream);
			}
		},
		adaptiveSnapshot(agent) {
			assertOwner(agent);
			return flows.get(agent)?.snapshot();
		},
		...
		setEnabled(value) {
			...
			if (!enabled) {
				questionLifetime.abort(/* @__PURE__ */ new Error("Think proposals disabled"));
	...
	ctx.effect(() => async () => {
		stopped = true;
		lifecycle.abort(/* @__PURE__ */ new Error("Adaptive runtime disposed"));
...
function portableTaskMessages(session, current, target) {
	...
	const add = (message, depth = 0) => {
		...
		if (message.source.kind === "plugin" && message.source.plugin === "compact") {
			...
			if (typeof source.compactionId !== "string" || summary?.type !== "compaction/summary" || ended?.type !== "compaction/end" || ended.data.error !== void 0 || ended.seq <= event.seq || summary.seq >= event.seq) taskFailure("TASK_HANDOFF_HISTORY_UNSUPPORTED");
...
var AdaptiveTaskRuntime = class {
	...
	constructor(ctx, options) {
		...
		ctx.on("agent/request", async ({ agent, signal }, next) => {
			...
			if (this.options.legacyThinkEnabled()) taskFailure("TASK_LEGACY_THINK_ACTIVE");
	...
	async state(sessionId, principal) {
		...
		return {
			...
			canStart: task === void 0 && isFresh(agent) && !this.options.legacyThinkEnabled() && allowsTaskRoute(capabilities, ADAPTIVE_TASK_START),
			...this.options.legacyThinkEnabled() ? { unavailable: "TASK_LEGACY_THINK_ACTIVE" } : task?.mode === "stopped" && agent.status !== "idle" ? { unavailable: "TASK_STOPPING" } : {}
		};
	}
	async command(command, principal) {
		...
		const apply = async () => {
			await this.options.store.update(agent.id, (existing) => {
				if (existing !== void 0) {
					...
					if (command.action === "resume") {
						if (agent.status !== "idle" || value.mode !== "interrupted" || value.reserved >= value.maximumRequests || !allowsTaskRoute(capabilities, value.route) || this.options.legacyThinkEnabled()) taskFailure("TASK_RESUME_UNAVAILABLE");
				...
				if (command.action !== "start" || command.revision !== 0 || !isFresh(agent) || this.options.legacyThinkEnabled()) taskFailure("TASK_NEW_SESSION_REQUIRED");
				const allowed = capabilities.filter((item) => command.models?.includes(item.model));
				if (!allowsTaskRoute(allowed, ADAPTIVE_TASK_START) || allowed.length !== command.models?.length) taskFailure("TASK_MODEL_UNAVAILABLE");
				return {
					version: 1,
					sessionId: agent.id,
			...
			if (command.action === "start" && !hasTaskMarker(agent)) {
...
function apply(ctx, config) {
	...
	taskRuntime = new AdaptiveTaskRuntime(ctx, {
		...
		models: async () => {
			...
			return Promise.all(models.filter((model) => [
				"gpt-5.6-sol",
				"gpt-5.6-terra",
				"gpt-5.6-luna",
				"gpt-6-astra"
			].includes(model.id)).map((model) => ctx.llm.resolveModelInfo(OPENAI_CODEX_PROVIDER, model.id)));
		}
	});
	...
	const scheduleCapabilities = () => {
		think.setEnabled(!stopped && resolveOpenAICodexSettings(current()).enableReasoningUpdates).catch((error) => {
