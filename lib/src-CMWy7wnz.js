	...
	wrapProvider(provider) {
		...
		return {
			...
			streamSimple(model, context, options) {
				...
				if (plan === void 0) return streamSimple.call(provider, model, context, options);
...
function assertReasoningUpdateSession(options, session, pending) {
...
function registerReasoningUpdateGuard(ctx) {
	...
	ctx.on("llm/stream", (options, next) => {
		...
		assertReasoningUpdateSession(options, session, agent !== void 0 && pendingUpdates(agent));
		return next();
	}, { prepend: true });
}
