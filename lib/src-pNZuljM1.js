...
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxy) {
	...
	const profiles = /* @__PURE__ */ new Map([[OPENAI_CODEX_PROVIDER, createOpenAICodexProfile(provider, fastMode, proxy)]]);
	...
	return new OpenAICodexAdapter({
		profiles: () => profiles,
		resolveApiKey: async () => withOpenAICodexProxy(proxy, async () => (await models.getAuth(OPENAI_CODEX_PROVIDER))?.auth.apiKey),
		auth: {
			credentials,
		...
		const routes = [
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					json$3(res, 200, await auth.status());
				}
			}),
			ctx.webServer.register({
				kind: "exact",
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$3(res, 200, await auth.signIn());
					} catch (error) {
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					if (req.method !== "GET" && req.method !== "POST") return json$3(res, 405, { error: "method not allowed" });
					if (!await authorize(req, res)) return;
					if (req.method === "GET") {
						const sessionId = fastModeSessionIdFromQuery(req);
						if (sessionId === void 0) return json$3(res, 400, { error: "invalid input" });
						return json$3(res, 200, { enabled: fastMode.isEnabled(sessionId) });
					}
					...
					try {
						...
						if (body === void 0) return json$3(res, 400, { error: "invalid input" });
						fastMode.set(body.sessionId, body.enabled);
						return json$3(res, 200, { enabled: fastMode.isEnabled(body.sessionId) });
					} catch (error) {
						return json$3(res, error instanceof RangeError ? 413 : 400, { error: error instanceof RangeError ? "request body too large" : "invalid input" });
					}
				}
			})
		];
...
function registerOpenAICodexProxyRoutes(ctx, trustedOrigins, options = {}) {
	...
	ctx.effect(() => {
		...
		const authorize = async (req, res) => {
			const decision = await trustedRequestDecision(req, trustedOrigins);
			if (decision.trusted) return true;
			json$2(res, 403, { error: decision.error });
			return false;
		};
		const routes = [
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$2(res, 200, await connectivity());
					} catch {
						json$2(res, 503, { error: "connectivity check failed" });
					}
				}
			}),
			ctx.webServer.register({
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (type === void 0 || !/^application\/json(?:\s*;|$)/iu.test(type.trim())) return json$2(res, 415, { error: "unsupported content type" });
					try {
						const proxyUrl = proxyUrlBody(await readJson(req));
						if (proxyUrl === void 0) return json$2(res, 400, { error: "invalid proxy URL" });
						json$2(res, 200, await testProxy(proxyUrl));
					} catch (error) {
						json$2(res, error instanceof RangeError ? 413 : 400, { error: error instanceof RangeError ? "request body too large" : "invalid input" });
					}
				}
			})
		];
		...
		const disposeRuntime = ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				try {
					...
					return json$1(res, 200, currentDshVersion === void 0 ? {} : { currentDshVersion });
				} catch {
...
//#endregion
export { OPENAI_CODEX_PROXY_ENV_KEYS as $, openAICodexConflictMessage as A, OPENAI_CODEX_PROXY_TEST_URL as At, DSH_PLUGIN_API_PACKAGES as B, OpenAICodexTransport as C, loginOpenAICodex as Ct, IMAGE_GENERATE_TOOL_NAME as D, OPENAI_CODEX_CONNECTIVITY_TIMEOUT_MS as Dt, VIEW_IMAGE_TOOL_NAME as E, OPENAI_CODEX_CONNECTIVITY_TARGETS as Et, parseOpenAICodexUpdateResult as F, OPENAI_CODEX_PROVIDER as Ft, assessCompatibility as G, SUPPORTED_DSH_PLUGIN_API_VERSION as H, parseOpenAICodexVersion as I, OpenAICodexCredentialStore as It, OPENAI_CODEX_CONNECTIVITY_PATH as J, detectCompatibility as K, COMPATIBILITY_CONTRACT as L, openAICodexAuthPath as Lt, OPENAI_CODEX_UPDATE_PATH as M, checkOpenAICodexConnectivity as Mt, checkForOpenAICodexUpdate as N, testOpenAICodexProxy as Nt, assertNoOpenAICodexProviderConflict as O, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as Ot, compareOpenAICodexVersions as P, OPENAI_CODEX_AUTH_FILENAME as Pt, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as Q, COMPATIBILITY_PACKAGES as R, OPENAI_CODEX_TRANSPORT_SERVICE as S, readOpenAICodexRateLimits as St, isOpenAICodexTransportError as T, openAICodexAuthStatus as Tt, SUPPORTED_NODE_RANGE as U, PI_AI_PACKAGE as V, SUPPORTED_PI_AI_VERSION as W, OPENAI_CODEX_PROXY_TEST_PATH as X, OPENAI_CODEX_PROXY_DETECT_PATH as Y, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as Z, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as _, isFastModeSessionId as _t, name as a, DEFAULT_OPENAI_CODEX_SEARCH_MODE as at, OPENAI_CODEX_TRANSPORT_API_VERSION as b, OPENAI_CODEX_USAGE_URL as bt, migrateOpenAICodexSearchHistory as c, OPENAI_CODEX_SETTINGS_NAMESPACE as ct, OPENAI_CODEX_SEARCH_URL as d, resolveOpenAICodexProxyUrl as dt, detectOpenAICodexProxyEnvironment as et, OpenAICodexSearchProvider as f, resolveOpenAICodexSettings as ft, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as g, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as gt, OPENAI_CODEX_IMAGE_MAX_COUNT as h, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as ht, inject as i, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as it, CODEX_CONNECT_VERSION as j, OpenAICodexProxyController as jt, diagnoseOpenAICodex as k, OPENAI_CODEX_PROXY_TEST_TIMEOUT_MS as kt, OPENAI_CODEX_BASE_URL as l, decodeOpenAICodexSettings as lt, OPENAI_CODEX_IMAGE_GENERATION_URL as m, FastModeRegistry as mt, OPENAI_CODEX_SETTINGS_NS as n, DEFAULT_OPENAI_CODEX_PROXY_URL as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as ot, mapOpenAICodexSearchResponse as p, OPENAI_CODEX_FAST_MODE_PATH as pt, evaluateCompatibility as q, apply as r, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, DEFAULT_OPENAI_CODEX_SETTINGS as st, Config as t, listOpenAICodexProxyCandidates as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, isValidOpenAICodexProxyUrl as ut, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as v, OpenAICodexTrustedOriginsStore as vt, OpenAICodexTransportError as w, logoutOpenAICodex as wt, OPENAI_CODEX_TRANSPORT_ERROR_CODES as x, parseOpenAICodexUsage as xt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as y, normalizeTrustedOrigin as yt, COMPATIBILITY_SCHEMA_VERSION as z };
