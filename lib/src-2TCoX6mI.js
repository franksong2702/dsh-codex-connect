...
const SUPPORTED_DSH_PLUGIN_API_VERSION = "0.1.2-alpha.5";
...
function evaluateOpenAICodexDshCompatibility(currentVersion, latestPluginVersion, currentDshVersion, catalog) {
	...
	const recommendedPluginVersion = catalog.pluginVersions.filter((plugin) => plugin.verifiedDshVersions.includes(currentDshVersion) && compareOpenAICodexVersions(plugin.version, latestPluginVersion) <= 0).map((plugin) => plugin.version).reduce((best, version) => best === void 0 || compareOpenAICodexVersions(version, best) > 0 ? version : best, void 0);
	if (recommendedPluginVersion !== void 0) return {
