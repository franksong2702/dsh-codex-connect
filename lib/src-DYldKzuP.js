...
import { LlmError, ReasoningEffortId, createUserMessage, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
function createReasoningUpdateMessage(update) {
	const source = {
		kind: "plugin",
...
function readReasoningUpdate(message) {
	...
	if (source.kind !== "plugin" || source.plugin !== "dsh-codex-connect" || !("reasoningUpdate" in source)) return void 0;
	const value = source.reasoningUpdate;
	...
	const update = value;
	if (message.role !== "user" || source.form !== "notice" || message.content.length !== 1 || message.content[0]?.type !== "text" || message.content[0].text !== reasoningUpdateText(update)) reasoningUpdateError("The saved Astra reasoning notice does not match its confirmed update.");
	return update;
}
/**
...
function applyReasoningUpdates(payload, plan) {
	if (!record$3(payload) || payload.model !== "gpt-6-astra" || !Array.isArray(payload.input) || !record$3(payload.reasoning) || payload.reasoning.effort !== plan.requestEffort) reasoningUpdateError("The Astra request does not preserve its original reasoning level.");
	if (payload.context_management !== void 0 || payload.truncation !== void 0 && payload.truncation !== "disabled" || payload.previous_response_id !== void 0 || payload.agents !== void 0 || payload.agent !== void 0 || payload.input.some((item) => record$3(item) && [
	...
	for (const item of payload.input) {
		if (record$3(item) && item.role === "user") {
			const update = plan.updates[updateIndex];
			if (update?.userIndex === userIndex) {
				if (!Array.isArray(item.content) || item.content.length !== 1 || !record$3(item.content[0]) || item.content[0].type !== "input_text" || item.content[0].text !== update.text) reasoningUpdateError("The Astra reasoning notice changed position during request conversion.");
				input.push({
...
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides) {
	...
	class OpenAICodexAdapter extends PiAiAdapter {
		async prepareCall(providerId, modelId, signal) {
			const prepared = await super.prepareCall(providerId, modelId, signal);
			return {
				...prepared,
				stream: (options) => reasoningScope.stream(options, () => prepared.stream(options))
			};
		}
		stream(options) {
...
const SUPPORTED_DSH_PLUGIN_API_VERSIONS = [SUPPORTED_DSH_PLUGIN_API_VERSION, "0.1.5-alpha.1"];
...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.33";
//#endregion
...
//#endregion
//#region src/reasoning-update-tool.ts
...
function pendingUpdates(agent) {
	return [...agent.inbox.nextStep, ...agent.inbox.nextTurn].some((message) => readReasoningUpdate(message) !== void 0);
}
...
function pendingSelection(session) {
	...
	if (events.some((event) => event.seq > selection.seq && event.type === "user/message" && readReasoningUpdate(event.data) !== void 0 && event.data.source.kind === "plugin" && "reasoningSelectionSeq" in event.data.source && event.data.source.reasoningSelectionSeq === selection.seq)) return void 0;
	return events.some((event) => event.seq > selection.seq && event.type === "request/header" && event.data.header.config.provider === selection.data.provider && event.data.header.config.model === selection.data.model && event.data.header.config.reasoningEffort === selection.data.reasoningEffort) ? void 0 : {
		...selection.data,
		seq: selection.seq
	};
}
/** Reject dropped updates and auxiliary/model-switch requests even outside the Codex adapter. */
function assertReasoningUpdateSession(options, session, pending, proposed = []) {
	const recorded = session?.snapshotEvents().filter((event) => event.type === "user/message" && readReasoningUpdate(event.data) !== void 0) ?? [];
	const proposedUpdates = proposed.filter((message) => readReasoningUpdate(message) !== void 0);
	if (recorded.length === 0 && proposedUpdates.length === 0 && !pending) {
		planReasoningUpdates(options);
		return;
	}
	if (options.purpose !== void 0 || session !== void 0 && session.surface.replaceGeneration !== 0) reasoningUpdateError("Compaction and auxiliary requests are not supported in conversations with confirmed Astra reasoning updates. Start a new conversation when compaction is needed.");
	const ids = new Set(options.messages.filter((message) => readReasoningUpdate(message) !== void 0).map((message) => message.id));
	for (const event of recorded) if (event.type === "user/message" && !ids.has(event.data.id)) reasoningUpdateError("A confirmed Astra reasoning update is missing from this request history. Keep the original history or start a new conversation.");
	for (const message of proposedUpdates) if (!ids.has(message.id)) reasoningUpdateError("A confirmed Astra reasoning update was removed before request admission. No model request was sent.");
	planReasoningUpdates(options);
}
/** Guards remain installed when new proposals are disabled, so existing approvals still replay. */
function registerReasoningUpdateGuard(ctx) {
	const steps = /* @__PURE__ */ new WeakMap();
	ctx.on("agent/pre-step", async ({ agent, turn, step, signal }, next) => {
		steps.delete(agent);
		const decision = await next();
		if (decision.kind === "enter" && !signal.aborted) steps.set(agent, {
			turn,
			step,
			signal,
	...
	ctx.on("agent/request", async ({ agent, turn, step, signal }, next) => {
		...
		const base = messages.map(readReasoningUpdate).find((update) => update !== void 0)?.baseEffort;
		const options = {
			...config,
			...base === void 0 ? {} : { reasoningEffort: ReasoningEffortId(base) },
			sessionId: agent.session.id,
			messages
		};
		assertReasoningUpdateSession(options, agent.session, pendingUpdates(agent), proposed);
		const plan = planReasoningUpdates(options);
		if (plan === void 0) return config;
		const pending = pendingSelection(agent.session);
		if (pending !== void 0 && (pending.provider !== config.provider || pending.model !== config.model || pending.reasoningEffort !== plan.effectiveEffort)) reasoningUpdateError("The model selection changed outside the confirmed Astra update. Start a new conversation or restore the confirmed level.");
		return {
			...config,
	...
	ctx.on("llm/stream", (options, next) => {
		...
		const prepared = agent === void 0 ? void 0 : steps.get(agent);
		assertReasoningUpdateSession(options, session, agent !== void 0 && pendingUpdates(agent), prepared?.signal.aborted === false ? prepared.messages : []);
		return next();
	}, { prepend: true });
...
function registerReasoningUpdateTool(ctx, enabled) {
	const lifetime = new AbortController();
	const busy = /* @__PURE__ */ new WeakSet();
	const operations = /* @__PURE__ */ new Set();
	ctx.on("agent/pre-step", async ({ agent }, next) => {
		const decision = await next();
		if (decision.kind !== "enter" || lifetime.signal.aborted || !ctx.get("agents")?.roots().includes(agent)) return decision;
		...
		const base = messages.map(readReasoningUpdate).find((update) => update !== void 0)?.baseEffort;
		if (!enabled() && base === void 0) return decision;
		const additions = [...decision.messages];
		if (selected !== void 0 && base !== void 0 && isAstraReasoningEffort(selected.reasoningEffort)) {
			const previous = planReasoningUpdates({
			...
			if (selected.reasoningEffort !== previous.effectiveEffort) {
				const approved = createReasoningUpdateMessage({
					version: 1,
					sessionId: agent.id,
					baseEffort: base,
				...
				const notice = {
					...
					source: {
						...approved.source,
						reasoningSelectionSeq: selected.seq
					}
				};
		...
		const explicit = isAstraReasoningEffort(config.reasoningEffort) && !(selected === void 0 && header?.adapterDefaults?.reasoningEffort === true);
		const plan = explicit ? planReasoningUpdates({
			provider: config.provider,
			model: config.model,
			reasoningEffort: ReasoningEffortId(base ?? config.reasoningEffort),
			messages,
			sessionId: agent.id
		}) : void 0;
		const text = explicit ? `Astra reasoning state: original request-level effort ${plan?.baseEffort ?? config.reasoningEffort}; effective effort for this request ${plan?.effectiveEffort ?? config.reasoningEffort}. The model selector follows the effective request level. At meaningful task transitions, assess the next phase's uncertainty and failure cost, including whether a lower effort is now sufficient. Follow codex_connect_set_reasoning_effort's guidance; the user decides.` : "Astra reasoning state: no explicit initial effort is selected. Do not propose a reasoning update until the user explicitly selects an initial Astra effort; do not infer it from Default.";
		const previous = history.findLast((message) => message.source.kind === "plugin" && message.source.plugin === "dsh-codex-connect" && message.source.form === "notice" && message.source.summary === "Astra reasoning state");
		if (previous?.content[0]?.type === "text" && previous.content[0].text === text) return {
			...decision,
			messages: additions
		};
		return {
			...decision,
			messages: [...additions, createUserMessage({
				source: {
					kind: "plugin",
					plugin: "dsh-codex-connect",
					form: "notice",
					summary: "Astra reasoning state"
				},
				content: [{
					type: "text",
					text
	...
	function selection(agent) {
		...
		if (agents?.get(agent.id) !== agent || !agents.roots().includes(agent)) reasoningUpdateError("Only the live main agent can ask the user to change this conversation's reasoning effort.");
		if (pendingUpdates(agent)) reasoningUpdateError("A confirmed reasoning update is already queued for the next request.");
		if (agent.session.surface.replaceGeneration !== 0) reasoningUpdateError("This first version requires an uncompacted conversation. Start a new Astra conversation before changing effort.");
		const header = agent.session.requestHeader();
		const config = header?.config;
		if (config?.provider !== "openai-codex" || config.model !== "gpt-6-astra" || !isAstraReasoningEffort(config.reasoningEffort) || header?.adapterDefaults?.reasoningEffort === true) reasoningUpdateError("Select GPT-6 Astra and an explicit reasoning level before requesting a change; provider Default is not supported by this tool.");
		const latestSelection = pendingSelection(agent.session);
		if (latestSelection !== void 0) {
			const selected = latestSelection;
			if (selected.provider !== config.provider || selected.model !== config.model || selected.reasoningEffort !== config.reasoningEffort) reasoningUpdateError("The model selection changed after the last request. Wait for the selected model to handle the next turn before changing effort.");
		}
		const plan = planReasoningUpdates({
			...config,
	...
	async function change(agent, effort, reason, signal) {
		if (!isAstraReasoningEffort(effort) || reason.trim().length === 0 || reason.length > 2e3) reasoningUpdateError("Choose a supported reasoning level and give a reason of 1–2000 characters.");
		const combined = signal === void 0 ? lifetime.signal : AbortSignal.any([signal, lifetime.signal]);
		combined.throwIfAborted();
		const before = selection(agent);
		if (before.effectiveEffort === effort) return {
			status: "unchanged",
			effort,
			message: `Astra reasoning is already ${effort}.`
		};
		if (busy.has(agent)) reasoningUpdateError("A reasoning-change question is already awaiting this user's answer.");
		const questions = ctx.get("userQuestions");
		if (questions === void 0) reasoningUpdateError("The current DSH profile has no human-question service. No reasoning change was made.");
		busy.add(agent);
		try {
			...
			const answer = await questions.ask({
				...
				questions: [{
					...
					options: [{
						label: approve,
						description: "Approve this exact change for subsequent requests."
					}, {
						label: "Keep current effort",
						description: "Continue without changing the reasoning level."
					}],
					multiSelect: false
				}]
			});
			...
			if (chosen.length !== 1 || chosen[0]?.id !== "astra-reasoning-effort" || chosen[0].selected.length !== 1 || chosen[0].selected[0] !== approve || chosen[0].custom !== void 0 && chosen[0].custom.trim() !== "") return {
				status: "declined",
				effort: before.effectiveEffort,
				message: "No reasoning change was approved."
			};
			const after = selection(agent);
			if (after.baseEffort !== before.baseEffort || after.effectiveEffort !== before.effectiveEffort || after.revision !== before.revision) reasoningUpdateError("The conversation changed while the user was deciding. The approval was not applied; request a new confirmation.");
			...
			return {
				status: "queued",
				effort,
				message: `The user approved ${effort}. It becomes effective when the queued notice enters the next request; cancellation may discard pending context. The model selector updates with that request.`
			};
		} finally {
			busy.delete(agent);
		}
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			effort: {
				...
				required: true,
				description: "Requested reasoning level."
			},
			reason: {
				type: "string",
				required: true,
				description: "Briefly name the upcoming work, the changed uncertainty or risk, and why the requested effort is appropriate."
			}
		},
		output: {
			schema: {
		...
		execute(args, exec) {
			...
			if (agent === void 0) reasoningUpdateError("Astra reasoning changes require a live main-agent conversation.");
	...
	ctx.effect(() => async () => {
		lifetime.abort(/* @__PURE__ */ new Error("Astra reasoning tool was unloaded"));
...
function apply(ctx, config) {
	...
	const reconcileReasoningTool = async () => {
		if (stopped) return;
		const enabled = current().enableReasoningUpdates === true;
		if (enabled === (reasoningFiber !== void 0)) return;
		const previous = reasoningFiber;
		reasoningFiber = void 0;
		if (previous !== void 0) await previous.dispose();
		if (stopped || !enabled) return;
		...
		Promise.resolve(fiber).catch((error) => {
			if (reasoningFiber === fiber) reasoningFiber = void 0;
			ctx.logger.error("dsh-codex-connect: reasoning update tool failed to activate");
			ctx.logger.error(error);
		});
	};
	const scheduleCapabilities = () => {
		reasoningTail = reasoningTail.then(reconcileReasoningTool, reconcileReasoningTool).catch((error) => {
...
//#endregion
export { isSupportedDshPluginApiVersion as $, IMAGE_GENERATE_TOOL_NAME as A, OPENAI_CODEX_USAGE_URL as At, COMPATIBILITY_CONTRACT as B, OPENAI_CODEX_AUTH_FILENAME as Bt, OPENAI_CODEX_TRANSPORT_API_VERSION as C, OPENAI_CODEX_FAST_MODE_PATH as Ct, OpenAICodexTransportError as D, isFastModeSessionId as Dt, OpenAICodexTransport as E, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as Et, OPENAI_CODEX_UPDATE_PATH as F, logoutOpenAICodex as Ft, SUPPORTED_DSH_PLUGIN_API_RANGE as G, COMPATIBILITY_SCHEMA_VERSION as H, OPENAI_CODEX_PROVIDER as Ht, checkForOpenAICodexUpdate as I, openAICodexAuthStatus as It, SUPPORTED_NODE_RANGE as J, SUPPORTED_DSH_PLUGIN_API_VERSION as K, compareOpenAICodexVersions as L, publicAuthError as Lt, diagnoseOpenAICodex as M, readOpenAICodexRateLimits as Mt, openAICodexConflictMessage as N, openAICodexModelCatalog as Nt, isOpenAICodexTransportError as O, OpenAICodexTrustedOriginsStore as Ot, CODEX_CONNECT_VERSION as P, loginOpenAICodex as Pt, evaluateCompatibility as Q, parseOpenAICodexUpdateResult as R, OPENAI_CODEX_ACCOUNT_LIMIT as Rt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as S, resolveOpenAICodexSettings as St, OPENAI_CODEX_TRANSPORT_SERVICE as T, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as Tt, DSH_PLUGIN_API_PACKAGES as U, OpenAICodexCredentialStore as Ut, COMPATIBILITY_PACKAGES as V, OPENAI_CODEX_AUTH_V1_BACKUP_SUFFIX as Vt, PI_AI_PACKAGE as W, openAICodexAuthPath as Wt, assessCompatibility as X, SUPPORTED_PI_AI_RANGE as Y, detectCompatibility as Z, OPENAI_CODEX_IMAGE_GENERATION_URL as _, decodeOpenAICodexSettings as _t, name as a, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as at, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as b, normalizeOpenAICodexProxyUrl as bt, migrateOpenAICodexSearchHistory as c, detectOpenAICodexProxies as ct, probeCodexAutoReview as d, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as dt, readInstalledPackageVersion as et, OPENAI_CODEX_BASE_URL as f, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as ft, mapOpenAICodexSearchResponse as g, OPENAI_CODEX_SETTINGS_NAMESPACE as gt, OpenAICodexSearchProvider as h, DEFAULT_OPENAI_CODEX_SETTINGS as ht, inject as i, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as it, assertNoOpenAICodexProviderConflict as j, parseOpenAICodexUsage as jt, VIEW_IMAGE_TOOL_NAME as k, normalizeTrustedOrigin as kt, ASTRA_REASONING_TOOL_NAME as l, listOpenAICodexProxyCandidates as lt, OPENAI_CODEX_SEARCH_URL as m, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as mt, OPENAI_CODEX_SETTINGS_NS as n, OPENAI_CODEX_PROXY_TEST_PATH as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, OPENAI_CODEX_PROXY_PROBE_URL as ot, OPENAI_CODEX_SEARCH_PROVIDER as p, DEFAULT_OPENAI_CODEX_SEARCH_MODE as pt, SUPPORTED_DSH_PLUGIN_API_VERSIONS as q, apply as r, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, OpenAICodexProxyManager as st, Config as t, OPENAI_CODEX_PROXY_DETECT_PATH as tt, CODEX_AUTO_REVIEW_MODEL as u, DEFAULT_OPENAI_CODEX_PROXY_URL as ut, OPENAI_CODEX_IMAGE_MAX_COUNT as v, isValidOpenAICodexContextWindowOverrides as vt, OPENAI_CODEX_TRANSPORT_ERROR_CODES as w, FastModeRegistry as wt, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as x, resolveOpenAICodexProxyUrl as xt, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as y, isValidOpenAICodexProxyUrl as yt, parseOpenAICodexVersion as z, OPENAI_CODEX_AUTH_DOCUMENT_LIMIT as zt };
