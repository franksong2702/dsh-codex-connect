...
function ensureScopedDispatcher() {
...
*/
function withOpenAICodexProxy(proxyUrl, operation) {
...
async function closeOpenAICodexProxyAgents() {
...
function requestProvider(provider, fastMode, resolveProxyUrl) {
	...
	return {
		...
		streamSimple(model, context, options) {
			...
			return withOpenAICodexProxy(proxyUrl, () => streamSimple.call(configured, model, context, proxyUrl === void 0 ? options : {
				...options,
				transport: "sse"
			}));
		},
...
function createOpenAICodexProfile(provider, fastMode, resolveProxyUrl) {
	return {
		...
		piProvider: requestProvider(provider, fastMode, resolveProxyUrl)
	};
}
/**
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, resolveProxyUrl) {
	const provider = openaiCodexProvider();
	const profiles = /* @__PURE__ */ new Map([[OPENAI_CODEX_PROVIDER, createOpenAICodexProfile(provider, fastMode, resolveProxyUrl)]]);
	...
	return new OpenAICodexAdapter({
		profiles: () => profiles,
		resolveApiKey: async () => withOpenAICodexProxy(resolveProxyUrl?.(), async () => (await models.getAuth(OPENAI_CODEX_PROVIDER))?.auth.apiKey),
		auth: {
			credentials,
...
*/
async function loginOpenAICodex(interaction, store = new OpenAICodexCredentialStore(), proxyUrl) {
	const models = createModels({ credentials: store });
	models.setProvider(openaiCodexProvider());
	await withOpenAICodexProxy(proxyUrl, () => models.login(OPENAI_CODEX_PROVIDER, "oauth", interaction));
}
/**
...
*/
async function readOpenAICodexRateLimits(store, proxyUrl) {
...
var OpenAICodexWebAuth = class {
	...
	start() {
		...
		}, this.store, this.resolveProxyUrl()).then(async () => {
...
function registerOpenAICodexAuthRoutes(ctx, store, trustedOriginsOverride, fastModeOverride, resolveProxyUrl) {
	const auth = new OpenAICodexWebAuth(store, { ...resolveProxyUrl === void 0 ? {} : { resolveProxyUrl } });
		...
		const routes = [
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					...
					json$2(res, 200, await auth.status());
				}
			}),
			ctx.webServer.register({
				kind: "exact",
				...
				handler: async (req, res) => {
					...
					if (!await authorize(req, res)) return;
					try {
						json$2(res, 200, await auth.signIn());
					} catch (error) {
						json$2(res, 500, { error: safeMessage(error) });
					}
				}
			}),
			ctx.webServer.register({
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					if (req.method !== "GET" && req.method !== "POST") return json$2(res, 405, { error: "method not allowed" });
					if (!await authorize(req, res)) return;
					if (req.method === "GET") {
						const sessionId = fastModeSessionIdFromQuery(req);
						if (sessionId === void 0) return json$2(res, 400, { error: "invalid input" });
						return json$2(res, 200, { enabled: fastMode.isEnabled(sessionId) });
					}
					...
					try {
						...
						if (body === void 0) return json$2(res, 400, { error: "invalid input" });
						fastMode.set(body.sessionId, body.enabled);
						return json$2(res, 200, { enabled: fastMode.isEnabled(body.sessionId) });
					} catch (error) {
						return json$2(res, error instanceof RangeError ? 413 : 400, { error: error instanceof RangeError ? "request body too large" : "invalid input" });
					}
				}
			})
		];
...
var OpenAICodexTransport = class extends Service {
	...
	apiVersion = 1;
	models;
	constructor(ctx, credentials, resolveProxyUrl = () => void 0) {
	...
	async generateImages(input, context) {
		...
		let auth;
		try {
			auth = await withOpenAICodexProxy(this.resolveProxyUrl(), () => this.models.getAuth(OPENAI_CODEX_PROVIDER));
		} catch {
...
function decodeOpenAICodexSettings(value) {
	...
	if ((enableProxy ?? true) && proxyUrl !== void 0 && !isValidOpenAICodexProxyUrl(proxyUrl)) return void 0;
...
var OpenAICodexSearchProvider = class {
	...
	*/
	...
	async search(request, signal) {
		...
		let auth;
		try {
			auth = await abortable(withOpenAICodexProxy(this.options.resolveProxyUrl?.(), () => this.models.getAuth(OPENAI_CODEX_PROVIDER)), signal);
		} catch (error) {
			throwIfSearchAborted(signal);
...
function apply(ctx, config) {
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).models, resolveProviderProxyUrl));
...
//#endregion
export { SUPPORTED_DSH_PLUGIN_API_VERSION as $, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as A, diagnoseOpenAICodex as B, resolveOpenAICodexProxyUrl as C, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as D, OPENAI_CODEX_IMAGE_MAX_COUNT as E, OpenAICodexTransportError as F, compareOpenAICodexVersions as G, CODEX_CONNECT_VERSION as H, isOpenAICodexTransportError as I, COMPATIBILITY_CONTRACT as J, parseOpenAICodexUpdateResult as K, VIEW_IMAGE_TOOL_NAME as L, OPENAI_CODEX_TRANSPORT_ERROR_CODES as M, OPENAI_CODEX_TRANSPORT_SERVICE as N, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as O, OpenAICodexTransport as P, PI_AI_PACKAGE as Q, IMAGE_GENERATE_TOOL_NAME as R, isValidOpenAICodexProxyUrl as S, OPENAI_CODEX_IMAGE_GENERATION_URL as T, OPENAI_CODEX_UPDATE_PATH as U, openAICodexConflictMessage as V, checkForOpenAICodexUpdate as W, COMPATIBILITY_SCHEMA_VERSION as X, COMPATIBILITY_PACKAGES as Y, DSH_PLUGIN_API_PACKAGES as Z, DEFAULT_OPENAI_CODEX_SEARCH_MODE as _, openAICodexAuthStatus as _t, name as a, OPENAI_CODEX_FAST_MODE_PATH as at, OPENAI_CODEX_SETTINGS_NAMESPACE as b, OpenAICodexCredentialStore as bt, migrateOpenAICodexSearchHistory as c, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as ct, OPENAI_CODEX_SEARCH_URL as d, normalizeTrustedOrigin as dt, SUPPORTED_NODE_RANGE as et, OpenAICodexSearchProvider as f, OPENAI_CODEX_USAGE_URL as ft, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as g, logoutOpenAICodex as gt, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as h, loginOpenAICodex as ht, inject as i, evaluateCompatibility as it, OPENAI_CODEX_TRANSPORT_API_VERSION as j, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as k, OPENAI_CODEX_BASE_URL as l, isFastModeSessionId as lt, DEFAULT_OPENAI_CODEX_PROXY_URL as m, readOpenAICodexRateLimits as mt, OPENAI_CODEX_SETTINGS_NS as n, assessCompatibility as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, FastModeRegistry as ot, mapOpenAICodexSearchResponse as p, parseOpenAICodexUsage as pt, parseOpenAICodexVersion as q, apply as r, detectCompatibility as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as st, Config as t, SUPPORTED_PI_AI_VERSION as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, OpenAICodexTrustedOriginsStore as ut, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as v, OPENAI_CODEX_AUTH_FILENAME as vt, resolveOpenAICodexSettings as w, decodeOpenAICodexSettings as x, openAICodexAuthPath as xt, DEFAULT_OPENAI_CODEX_SETTINGS as y, OPENAI_CODEX_PROVIDER as yt, assertNoOpenAICodexProviderConflict as z };
