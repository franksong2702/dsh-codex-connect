...
function planReasoningUpdates(options) {
	if (!options.messages.some((message) => readReasoningUpdate(message) !== void 0)) return void 0;
	...
	return {
		baseEffort: options.reasoningEffort,
		effectiveEffort,
		userCount,
		updates
	};
...
function applyReasoningUpdates(payload, plan) {
	if (!record$3(payload) || payload.model !== "gpt-6-astra" || !Array.isArray(payload.input) || !record$3(payload.reasoning) || payload.reasoning.effort !== plan.baseEffort) reasoningUpdateError("The Astra request does not preserve its original reasoning level.");
...
function registerReasoningUpdateTool(ctx, enabled) {
	...
	ctx.on("agent/pre-step", async ({ agent }, next) => {
		...
		if (decision.kind !== "enter" || !enabled() || lifetime.signal.aborted || !ctx.get("agents")?.roots().includes(agent)) return decision;
		...
		const text = explicit ? `Astra reasoning state: original request-level effort ${config.reasoningEffort}; effective effort for this request ${plan?.effectiveEffort ?? config.reasoningEffort}. The selector retains the original level. Assess whether the upcoming work warrants a change and use codex_connect_set_reasoning_effort to propose it when justified; the user decides.` : "Astra reasoning state: no explicit initial effort is selected. Do not propose a reasoning update until the user explicitly selects an initial Astra effort; do not infer it from Default.";
	...
	function selection(agent) {
		...
		return {
			baseEffort: config.reasoningEffort,
	...
	async function change(agent, effort, reason, signal) {
		...
		try {
			...
			return {
				status: "queued",
				effort,
				message: `The user approved ${effort}. It becomes effective when the queued notice enters the next request; cancellation may discard pending context. The original model-selector level remains ${before.baseEffort}.`
			};
		} finally {
			busy.delete(agent);
		}
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			reason: {
				type: "string",
				required: true,
				description: "Brief task-specific reason for increasing or decreasing reasoning effort."
			}
		},
		output: {
			schema: {
