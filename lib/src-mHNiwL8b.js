...
function createOpenAICodexProfile(provider, fastMode, proxy) {
	return {
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxy) {
...
function resolveOpenAICodexProxyUrl(value) {
	...
	if (!settings.enableProxy) return void 0;
	const proxyUrl = settings.proxyUrl.trim();
	if (!isValidOpenAICodexProxyUrl(proxyUrl)) throw new TypeError("OpenAI Codex proxy URL must be a credential-free HTTP(S) origin");
	return proxyUrl;
}
...
function apply(ctx, config) {
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).models, proxy));
	...
	installSettingsSection(ctx, OPENAI_CODEX_SETTINGS_NS, Config, config, {
		validate(value) {
			resolveOpenAICodexProxyUrl(value);
		},
		setSource(source) {
			current = source;
		},
...
//#endregion
export { OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as $, openAICodexConflictMessage as A, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as At, DSH_PLUGIN_API_PACKAGES as B, OpenAICodexTransport as C, readOpenAICodexRateLimits as Ct, IMAGE_GENERATE_TOOL_NAME as D, openAICodexModelCatalog as Dt, VIEW_IMAGE_TOOL_NAME as E, openAICodexAuthStatus as Et, parseOpenAICodexUpdateResult as F, testOpenAICodexProxy as Ft, assessCompatibility as G, SUPPORTED_DSH_PLUGIN_API_VERSION as H, parseOpenAICodexVersion as I, OPENAI_CODEX_AUTH_FILENAME as It, readInstalledPackageVersion as J, detectCompatibility as K, COMPATIBILITY_CONTRACT as L, OPENAI_CODEX_PROVIDER as Lt, OPENAI_CODEX_UPDATE_PATH as M, OPENAI_CODEX_PROXY_TEST_URL as Mt, checkForOpenAICodexUpdate as N, OpenAICodexProxyController as Nt, assertNoOpenAICodexProviderConflict as O, OPENAI_CODEX_CONNECTIVITY_TARGETS as Ot, compareOpenAICodexVersions as P, checkOpenAICodexConnectivity as Pt, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as Q, COMPATIBILITY_PACKAGES as R, OpenAICodexCredentialStore as Rt, OPENAI_CODEX_TRANSPORT_SERVICE as S, parseOpenAICodexUsage as St, isOpenAICodexTransportError as T, logoutOpenAICodex as Tt, SUPPORTED_NODE_RANGE as U, PI_AI_PACKAGE as V, SUPPORTED_PI_AI_VERSION as W, OPENAI_CODEX_PROXY_DETECT_PATH as X, OPENAI_CODEX_CONNECTIVITY_PATH as Y, OPENAI_CODEX_PROXY_TEST_PATH as Z, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as _, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as _t, name as a, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as at, OPENAI_CODEX_TRANSPORT_API_VERSION as b, normalizeTrustedOrigin as bt, migrateOpenAICodexSearchHistory as c, DEFAULT_OPENAI_CODEX_SETTINGS as ct, OPENAI_CODEX_SEARCH_URL as d, isValidOpenAICodexProxyUrl as dt, OPENAI_CODEX_PROXY_ENV_KEYS as et, OpenAICodexSearchProvider as f, resolveOpenAICodexProxyUrl as ft, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as g, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as gt, OPENAI_CODEX_IMAGE_MAX_COUNT as h, FastModeRegistry as ht, inject as i, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as it, CODEX_CONNECT_VERSION as j, OPENAI_CODEX_PROXY_TEST_TIMEOUT_MS as jt, diagnoseOpenAICodex as k, OPENAI_CODEX_CONNECTIVITY_TIMEOUT_MS as kt, OPENAI_CODEX_BASE_URL as l, OPENAI_CODEX_SETTINGS_NAMESPACE as lt, OPENAI_CODEX_IMAGE_GENERATION_URL as m, OPENAI_CODEX_FAST_MODE_PATH as mt, OPENAI_CODEX_SETTINGS_NS as n, listOpenAICodexProxyCandidates as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, DEFAULT_OPENAI_CODEX_SEARCH_MODE as ot, mapOpenAICodexSearchResponse as p, resolveOpenAICodexSettings as pt, evaluateCompatibility as q, apply as r, DEFAULT_OPENAI_CODEX_PROXY_URL as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as st, Config as t, detectOpenAICodexProxyEnvironment as tt, OPENAI_CODEX_SEARCH_PROVIDER as u, decodeOpenAICodexSettings as ut, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as v, isFastModeSessionId as vt, OpenAICodexTransportError as w, loginOpenAICodex as wt, OPENAI_CODEX_TRANSPORT_ERROR_CODES as x, OPENAI_CODEX_USAGE_URL as xt, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as y, OpenAICodexTrustedOriginsStore as yt, COMPATIBILITY_SCHEMA_VERSION as z, openAICodexAuthPath as zt };
