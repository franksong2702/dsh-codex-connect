		...
		streamSimple(model, context, options) {
			...
			const bounded = {
				...
				async onPayload(payload, payloadModel) {
					...
					if (!record$3(next) || next["model"] !== "gpt-5.6-luna" || next["service_tier"] !== void 0 || next["stream"] !== true || next["store"] !== false || !record$3(next["reasoning"]) || next["reasoning"]["effort"] !== "low" || !Array.isArray(next["tools"]) || next["tools"].some((tool) => !record$3(tool) || tool["type"] !== "function" || !["split_read_evidence", "structured_output"].includes(String(tool["name"]))) || Buffer.byteLength(JSON.stringify(next)) > 512e3) throw new Error("SPLIT_PAYLOAD_DENIED");
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits, nativeCompactionEnabled) {
