#!/usr/bin/env node
import { B as DSH_PLUGIN_API_PACKAGES, Bt as openAICodexAuthPath, Dt as openAICodexAuthStatus, Et as logoutOpenAICodex, J as readInstalledPackageVersion, Ot as openAICodexModelCatalog, Rt as OPENAI_CODEX_PROVIDER, Tt as loginOpenAICodex, bt as OpenAICodexTrustedOriginsStore, c as migrateOpenAICodexSearchHistory, ft as isValidOpenAICodexProxyUrl, j as CODEX_CONNECT_VERSION, k as diagnoseOpenAICodex, l as OPENAI_CODEX_BASE_URL, q as evaluateCompatibility, xt as normalizeTrustedOrigin, zt as OpenAICodexCredentialStore } from "./src-3ofbQx2g.js";
...
var CodexCapabilityDiagnostics = class {
	...
	*/
	async inspect(request) {
		...
		const runtime = DSH_PLUGIN_API_PACKAGES.some((name) => versions[name] !== null && versions[name] !== "0.1.1-rc.2") || compatibility.status === "incompatible" ? result("rejected", "declared-version-mismatch", "Use DSH API 0.1.1-rc.2 and pi-ai 0.82.1 together. DSH 0.1.0-rc.7 requires Codex Connect 0.1.0-alpha.4.14.") : missing || compatibility.status === "unknown" ? result("unknown", "version-metadata-unavailable", "Run this command from the plugin installation in the intended profile.") : result("supported", "declared-host-versions-match", "Host package versions satisfy the declared requirements; this is not a live profile or browser compatibility test.");
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (flag === "--json") json = true;
			...
			if (flag === "--proxy") {
				...
				if (!isValidOpenAICodexProxyUrl(proxyUrl)) return invalid();
