#!/usr/bin/env node
import { B as CODEX_CONNECT_VERSION, R as diagnoseOpenAICodex, ct as loginOpenAICodex, it as normalizeTrustedOrigin, l as migrateOpenAICodexSearchHistory, lt as logoutOpenAICodex, mt as openAICodexAuthPath, rt as OpenAICodexTrustedOriginsStore, ut as openAICodexAuthStatus } from "./src-DUWbLQpF.js";
...
function printHelp() {
	process.stdout.write([
		...
		"       dsh-codex-connect migrate-history [--apply] [--root <path>] [--json]",
...
async function run(argv) {
	...
	if (action === "migrate-history") for (let index = 0; index < optionFlags.length; index += 1) {
		...
		if (flag === "--apply") migrationApply = true;
		else if (flag === "--json") continue;
		else if (flag === "--root" && migrationRoot === void 0 && optionFlags[index + 1] !== void 0 && !optionFlags[index + 1]?.startsWith("--")) {
	...
	if (unknown.length > 0 || deviceCode && action !== "login" || jsonOutput && (action === "login" || action === "logout" || deviceCode) || (action === "trust-origin" || action === "untrust-origin") && (originArgument === void 0 || optionFlags.length !== 0)) {
		...
		switch (action) {
			...
			case "migrate-history": {
				...
				if (jsonOutput) printJson({
					...
					if (result.mode === "dry-run" && result.changedEvents > 0) process.stdout.write("Stop DSH, then run again with --apply to create backups and repair these histories.\n");
				}
				return 0;
			}
			case "status": {
