...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.42";
//#endregion
...
var AdaptiveTaskDelegation = class {
	...
	async reconcile(parent, requireAll = false) {
		...
		for (const run of doc.delegation.runs) {
			...
			if (result.data.turn !== call.data.turn || result.data.step !== call.data.step || result.data.error !== void 0 || block.toolCallId !== call.data.callId || block.isError === true || block.content.length !== 1 || block.content[0]?.type !== "text" || block.content[0].text !== expected) {
				missing();
				continue;
			}
...
*/
function liveSettingsOf(settings) {
	if (typeof settings !== "object" || settings === null) return void 0;
	const candidate = settings;
	if (typeof candidate.register !== "function" || typeof candidate.installSection === "function") return void 0;
	return settings;
}
//#endregion
