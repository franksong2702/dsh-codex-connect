	...
	factory: (require) => {
		...
		*/
		...
		const en = {
			...
			reasoningUpdatesRisk: "Use a new, uncompacted Astra main conversation with an explicit initial effort, not Default. After the first approved adjustment, this experiment cannot compact that conversation or switch its model. Remember and Split composition are not yet supported; their settings are not changed.",
		...
		const zh = {
			...
			reasoningUpdatesRisk: "请使用新的、未压缩的 Astra 主会话，并明确选择初始档位，不要使用 Default。首次批准调整后，当前实验不支持该会话的上下文压缩或切换模型；尚未支持与 Remember、Split 联动，也不会改写它们的开关。",
