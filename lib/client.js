	...
	factory: (require) => {
		...
		*/
		...
		const en = {
			...
			enableNativeCompaction: "Experimental native compaction",
			enableNativeCompactionHelp: "Let DSH compaction use Codex Responses V2 when available. Default off; failures fall back to the normal Harness summary path, while existing native checkpoints remain readable after disabling it.",
		...
		const zh = {
			...
			enableNativeCompaction: "实验性原生上下文压缩",
			enableNativeCompactionHelp: "让 DSH 的压缩流程在可用时尝试 Codex Responses V2。默认关闭；失败时回退到 Harness 原有摘要，关闭后仍可读取已经写入的原生 checkpoint。",
