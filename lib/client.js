	...
	factory: (require) => {
		...
		*/
		...
		var OpenAICodexAccountStore = class {
			...
			schedule() {
				clearTimeout(this.timer);
				const interval = this.snapshot.operation.kind === "waiting-authorization" || this.snapshot.status.status === "signing-in" ? 1e3 : this.snapshot.status.status === "signed-in" ? 6e4 : this.snapshot.status.status === "error" && this.snapshot.accounts.length > 0 ? 5e3 : void 0;
		...
		function AccountManager({ t, store, snapshot, quotaExpanded, onToggleQuota, compact = false }) {
