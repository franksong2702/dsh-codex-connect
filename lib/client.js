window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function decodeOpenAICodexSettings(value) {
			...
			if (proxyUrl !== void 0 && typeof proxyUrl !== "string") return void 0;
			if (enableAccountFallback !== void 0 && typeof enableAccountFallback !== "boolean") return void 0;
			if ((enableProxy ?? false) && proxyUrl !== void 0 && !isValidOpenAICodexProxyUrl(proxyUrl)) return void 0;
			...
			return {
				...
				proxyUrl: proxyUrl === void 0 ? DEFAULT_OPENAI_CODEX_PROXY_URL : enableProxy ?? false ? normalizeOpenAICodexProxyUrl(proxyUrl) : proxyUrl,
		...
		function OpenAICodexConfiguration({ scope, t }) {
			...
			const getSnapshot = (0, react.useCallback)(() => scope?.getSnapshot() ?? UNAVAILABLE_SNAPSHOT$1, [scope]);
		...
		const OPENAI_CODEX_PROXY_DETECT_PATH = "/api/dsh-codex-connect/proxy/detect";
		const OPENAI_CODEX_PROXY_TEST_PATH = "/api/dsh-codex-connect/proxy/test";
		const OPENAI_CODEX_CONNECTIVITY_PATH = "/api/dsh-codex-connect/proxy/connectivity";
		//#endregion
		//#region src/client/OpenAICodexProxyConfiguration.tsx
		...
		const primaryButtonStyle$2 = {
			...
			background: "var(--dsw-alias-brand-primary)",
			color: "white",
			boxShadow: "0 4px 12px color-mix(in srgb, var(--dsw-alias-brand-primary) 24%, transparent)"
		};
		const errorStyle$2 = {
			...bodyStyle$2,
		...
		function environmentCandidate(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			const record = value;
			if (record["detected"] === false) return { detected: false };
			if (record["detected"] !== true || typeof record["source"] !== "string" || typeof record["valid"] !== "boolean") return void 0;
			if (record["valid"] === true && typeof record["proxyUrl"] === "string") return {
				detected: true,
			...
			return record["valid"] === false && record["reason"] === "invalid-or-credentialed" ? {
				...
				source: record["source"],
				valid: false,
				reason: "invalid-or-credentialed"
			} : void 0;
		}
		function detectedCandidate(value) {
			...
			if (!Array.isArray(record["candidates"])) return void 0;
			for (const candidate of record["candidates"]) {
				if (typeof candidate !== "object" || candidate === null || Array.isArray(candidate)) continue;
				const item = candidate;
				if (item["ok"] === true && typeof item["source"] === "string" && typeof item["proxyUrl"] === "string" && isValidOpenAICodexProxyUrl(item["proxyUrl"])) return {
					detected: true,
					source: item["source"],
					valid: true,
		...
		function proxyTestResult(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			const record = value;
			if (record["ok"] === true) return typeof record["statusCode"] === "number" ? {
				ok: true,
		...
		function OpenAICodexProxyConfiguration({ scope, t, compact = false }) {
			const snapshot = (0, react.useSyncExternalStore)((listener) => scope?.subscribe(listener) ?? (() => {}), () => scope?.getSnapshot() ?? UNAVAILABLE_SNAPSHOT, () => scope?.getSnapshot() ?? UNAVAILABLE_SNAPSHOT);
			const active = snapshot.value?.enableProxy === true;
			const savedUrl = snapshot.value?.proxyUrl ?? "http://127.0.0.1:7890";
			const [draft, setDraft] = (0, react.useState)(savedUrl);
			const [candidate, setCandidate] = (0, react.useState)();
			const [testedUrl, setTestedUrl] = (0, react.useState)();
			const [feedback, setFeedback] = (0, react.useState)("idle");
			const [busy, setBusy] = (0, react.useState)();
			(0, react.useEffect)(() => {
				if (busy === void 0 && feedback !== "detected" && feedback !== "test-ok") setDraft(savedUrl);
			...
			const detect = async () => {
				...
				try {
					...
					const next = response.ok ? detectedCandidate(await response.json().catch(() => void 0)) : void 0;
					if (next === void 0) throw new Error("invalid detect response");
					setCandidate(next);
			...
			const test = async () => {
				...
				try {
					...
					if ((response.ok ? proxyTestResult(await response.json().catch(() => void 0)) : void 0)?.ok !== true) {
			...
			const activate = async () => {
				if (scope === void 0 || !writable || testedUrl !== normalized) return;
				setBusy("activate");
				setFeedback("idle");
				try {
			...
			const disable = async () => {
				if (scope === void 0 || !writable) return;
				setBusy("disable");
				setFeedback("idle");
				try {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				style: compact ? compactSectionStyle : sectionStyle$1,
      ...
      ` }),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						style: { paddingRight: compact ? 18 : 0 },
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
								alignItems: "center",
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
										d: "M4 6.5h12M6.5 10h7M8.5 13.5h3",
										strokeLinecap: "round"
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						style: {
							...active ? successStyle : errorStyle$2,
							display: "inline-flex",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						style: labelStyle,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("proxyUrl") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							style: inputStyle,
							type: "url",
							value: draft,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								children: busy === "detect" ? t("proxyDetecting") : t("proxyDetect")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								style: buttonStyle$3,
								disabled: !writable || busy !== void 0 || !valid,
								onClick: () => {
									test();
								},
								children: busy === "test" ? t("proxyTesting") : t("proxyTest")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								"data-variant": "primary",
								style: primaryButtonStyle$2,
								disabled: !writable || busy !== void 0 || testedUrl !== normalized,
								onClick: () => {
									activate();
								},
								children: busy === "activate" ? t("proxyActivating") : t("proxyActivate")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								"data-variant": "danger",
								style: buttonStyle$3,
								disabled: !writable || busy !== void 0 || !active,
								onClick: () => {
									disable();
								},
								children: busy === "disable" ? t("proxyDisabling") : t("proxyDisable")
							})
						]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						"aria-live": "polite",
						style: { minHeight: feedback === "idle" ? 0 : 18 },
						children: [
							feedback === "detected" && candidate?.detected === true && candidate.valid ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: successStyle,
								children: t("proxyEnvironmentDetected", {
									source: candidate.source,
									url: candidate.proxyUrl
								})
							}) : null,
							feedback === "none" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: bodyStyle$2,
								children: t("proxyEnvironmentNone")
							}) : null,
							feedback === "invalid-env" && candidate?.detected === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: errorStyle$2,
								children: t("proxyEnvironmentInvalid", { source: candidate.source })
							}) : null,
							feedback === "test-ok" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: successStyle,
								children: t("proxyTestSucceeded")
							}) : null,
							feedback === "test-failed" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: errorStyle$2,
								children: t("proxyTestFailed")
							}) : null,
							feedback === "activated" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: successStyle,
								children: t("proxyActivated")
							}) : null,
							feedback === "disabled" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: bodyStyle$2,
								children: t("proxyDisabled")
							}) : null,
							feedback === "failed" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: errorStyle$2,
								children: t("proxyActionFailed")
							}) : null
						]
					})
				]
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [embedded ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(OpenAICodexProxyConfiguration, {
							t,
		...
		function OpenAICodexQuotaIndicator({ directory, t }) {
			...
			const quotas = [];
			if (fiveHour !== void 0) quotas.push({
				kind: "five-hour",
    ...
    ` }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				...
				}), tooltipVisible ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
					id: tooltipId,
					role: "tooltip",
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							style: {
								...
								borderTop: index === 0 ? void 0 : "1px solid var(--dsw-alias-border-l2)"
							},
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								"aria-hidden": "true",
								style: {
									...
									boxShadow: `0 0 0 3px color-mix(in srgb, ${progressColor.value} 13%, transparent)`
								}
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: quota.summary })]
						}, quota.kind);
					})]
				}) : null]
			})] });
		...
		*/
		...
		//#endregion
		//#region src/client/OpenAICodexProxyIndicator.tsx
		/** Draggable session-local Codex proxy signal and water-drop configuration panel. */
		const OPENAI_CODEX_PROXY_CLOSE_DELAY_MS = 1e3;
		const BALL_SIZE = 54;
		const INDICATOR_HEIGHT = BALL_SIZE;
		const ACCOUNT_REFRESH_INTERVAL_MS = 3e3;
		const EDGE_GAP = 16;
		...
		function accountsResponse(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			...
			for (const item of raw) {
				...
				const record = item;
				if (typeof record["accountId"] !== "string" || typeof record["active"] !== "boolean" || typeof record["expires"] !== "number") return void 0;
				const displayName = typeof record["displayName"] === "string" && record["displayName"].trim().length > 0 ? record["displayName"].trim() : `Account ${String(accounts.length + 1)}`;
				const email = typeof record["email"] === "string" && record["email"].trim().length > 0 ? record["email"].trim() : void 0;
				const profileSource = record["profileSource"] === "file" || record["profileSource"] === "oauth" || record["profileSource"] === "local" || record["profileSource"] === "generated" ? record["profileSource"] : "generated";
				accounts.push({
		...
		function accountInitials(account) {
			...
			return (words.length > 1 ? [words[0]?.[0], words[1]?.[0]] : Array.from(account.displayName).slice(0, 2)).filter((value) => value !== void 0).join("").toUpperCase();
		}
		function accountProfileSourceKey(source) {
		...
		function targetResult(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			const record = value;
			if (typeof record["id"] !== "string" || typeof record["hostname"] !== "string" || typeof record["reachable"] !== "boolean" || typeof record["latencyMs"] !== "number") return void 0;
			if (record["statusCode"] !== void 0 && typeof record["statusCode"] !== "number") return void 0;
			if (record["error"] !== void 0 && typeof record["error"] !== "string") return void 0;
			return {
				id: record["id"],
		...
		function connectivityReport(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			const record = value;
			if (typeof record["checkedAt"] !== "number" || record["mode"] !== "direct" && record["mode"] !== "proxy" || !Array.isArray(record["targets"])) return void 0;
		...
		function overallSignal(report, requestError) {
			if (requestError !== void 0) return "red";
		...
		function readRatio(sessionKey) {
			if (typeof localStorage === "undefined") return {
				x: 1,
				y: 0
			};
			try {
				const value = JSON.parse(localStorage.getItem(storageKey(sessionKey)) ?? "null");
				if (typeof value !== "object" || value === null) return {
					x: 1,
					y: 0
				};
				const record = value;
				return typeof record["x"] === "number" && typeof record["y"] === "number" && record["x"] >= 0 && record["x"] <= 1 && record["y"] >= 0 && record["y"] <= 1 ? {
					x: record["x"],
					y: record["y"]
				} : {
					x: 1,
		...
		function pointFromRatio(ratio, bounds) {
			const width = Math.max(0, bounds.width - BALL_SIZE - 32);
			const height = Math.max(0, bounds.height - INDICATOR_HEIGHT - 32);
			return clampPoint({
				x: bounds.left + EDGE_GAP + ratio.x * width,
				y: bounds.top + EDGE_GAP + ratio.y * height
			}, bounds);
		}
		function ratioFromPoint(point, bounds) {
			const width = Math.max(1, bounds.width - BALL_SIZE - 32);
			const height = Math.max(1, bounds.height - INDICATOR_HEIGHT - 32);
			return {
		...
		function OpenAICodexProxyIndicator({ directory, configScope, sessionKey, t }) {
			const model = (0, react.useSyncExternalStore)((listener) => directory.subscribe(listener), () => directory.getSnapshot(), () => directory.getSnapshot());
			const settings = (0, react.useSyncExternalStore)((listener) => configScope.subscribe(listener), () => configScope.getSnapshot(), () => configScope.getSnapshot());
			...
			const closeTimer = (0, react.useRef)();
			const drag = (0, react.useRef)();
			const positionRef = (0, react.useRef)({
				x: EDGE_GAP,
				y: EDGE_GAP
			});
			const ratioRef = (0, react.useRef)(readRatio(sessionKey));
			const boundsRef = (0, react.useRef)();
			const connectivityController = (0, react.useRef)();
			const [position, setPosition] = (0, react.useState)(positionRef.current);
			const [bounds, setBounds] = (0, react.useState)();
			const [expanded, setExpanded] = (0, react.useState)(false);
			const [dragging, setDragging] = (0, react.useState)(false);
			const [hoveredTarget, setHoveredTarget] = (0, react.useState)();
			const [report, setReport] = (0, react.useState)();
			const [connectivityError, setConnectivityError] = (0, react.useState)();
			...
			const [fallbackBusy, setFallbackBusy] = (0, react.useState)(false);
			const [accountError, setAccountError] = (0, react.useState)();
			const [accountNotice, setAccountNotice] = (0, react.useState)();
			const popupId = (0, react.useId)();
			const updatePosition = (next) => {
				const currentBounds = boundsRef.current;
				const clamped = currentBounds === void 0 ? next : clampPoint(next, currentBounds);
			...
			(0, react.useEffect)(() => {
				...
				const resolveRoot = () => {
					...
					return Array.from(document.querySelectorAll("[data-openai-codex-proxy-indicator]")).find((candidate) => candidate.dataset["openaiCodexProxySession"] === sessionKey) ?? null;
				};
				const sync = () => {
				...
				const resolveBoundary = () => {
					...
					if (boundary === null) return;
					sync();
					resizeObserver = typeof ResizeObserver === "undefined" ? void 0 : new ResizeObserver(sync);
					resizeObserver?.observe(boundary);
				};
				resolveBoundary();
				const placementObserver = typeof MutationObserver === "undefined" ? void 0 : new MutationObserver(resolveBoundary);
			...
			(0, react.useEffect)(() => {
				...
				connectivityController.current = void 0;
				setReport(void 0);
				setConnectivityError(void 0);
				setChecking(false);
			}, [
				selected,
				active,
			...
			(0, react.useEffect)(() => {
				...
				const refreshAccounts = async () => {
					...
					try {
						...
						const next = response.ok ? accountsResponse(await response.json().catch(() => void 0)) : void 0;
						if (next === void 0) throw new Error(`HTTP ${String(response.status)}`);
						if (!stopped) {
							setAccounts(next.accounts);
							setAccountError(void 0);
						}
					} catch (error) {
						if (!stopped && controller.signal.aborted !== true) setAccountError(error instanceof Error ? error.message : String(error));
					}
				};
				refreshAccounts();
			...
			const testConnectivity = async () => {
				...
				try {
					...
					const next = response.ok ? connectivityReport(await response.json().catch(() => void 0)) : void 0;
					if (next === void 0) throw new Error(`HTTP ${String(response.status)}`);
					if (!controller.signal.aborted && connectivityController.current === controller) setReport(next);
				} catch (error) {
					if (!controller.signal.aborted && connectivityController.current === controller) setConnectivityError(error instanceof Error ? error.message : String(error));
				} finally {
			...
			const toggleAccountFallback = async () => {
				...
				try {
					...
					if (configScope.getSnapshot().value?.enableAccountFallback === true !== next) throw new Error("setting was not confirmed");
			...
			const addAccount = async () => {
				...
				try {
					...
					const value = await response.json().catch(() => void 0);
					const url = typeof value === "object" && value !== null && typeof value["url"] === "string" ? value["url"] : void 0;
					if (!response.ok || url === void 0) throw new Error(`HTTP ${String(response.status)}`);
			...
			const signalText = t(signal === "green" ? "connectivitySignalGreen" : signal === "red" ? "connectivitySignalRed" : "connectivitySignalYellow");
			const detailTarget = report?.targets.find((target) => target.id === hoveredTarget);
			const panelWidth = Math.max(260, Math.min(420, (bounds?.width ?? 452) - 32));
			const panelHeight = Math.max(220, Math.min(600, (bounds?.height ?? 632) - 32));
			const openRight = bounds === void 0 || position.x + panelWidth <= bounds.right - EDGE_GAP;
			const openDown = bounds === void 0 || position.y + INDICATOR_HEIGHT + panelHeight <= bounds.bottom - EDGE_GAP;
			const openPanel = () => {
				if (closeTimer.current !== void 0) clearTimeout(closeTimer.current);
			...
			const pointerMove = (event) => {
				...
				if (current === void 0 || current.pointerId !== event.pointerId) return;
			...
			const pointerUp = (event) => {
				...
				setDragging(false);
				const currentBounds = boundsRef.current;
				if (current.moved && currentBounds !== void 0) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				"data-openai-codex-proxy-dragging": dragging || void 0,
				style: {
					display: "flex",
					flexDirection: "column",
					alignItems: "center",
      ...
      ` }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						"aria-describedby": expanded ? popupId : void 0,
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									style: {
										...
										inset: 0,
										borderRadius: "inherit",
										background: "radial-gradient(circle at 29% 20%, rgb(255 255 255 / 82%) 0 3%, rgb(255 255 255 / 20%) 12%, transparent 31%), radial-gradient(circle at 72% 78%, rgb(167 139 250 / 56%) 0%, transparent 52%)"
									}
								})]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								style: {
									...
									borderRadius: 7,
									background: "rgb(5 15 38 / 25%)",
									boxShadow: "0 2px 7px rgb(3 7 24 / 20%)",
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
									...
									strokeWidth: "1.8",
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
										d: "M8.1 12.1 6.6 13.6a2.8 2.8 0 0 1-4-4l2.3-2.3a2.8 2.8 0 0 1 4 0M11.9 7.9l1.5-1.5a2.8 2.8 0 1 1 4 4l-2.3 2.3a2.8 2.8 0 0 1-4 0M7.2 10h5.6",
										strokeLinecap: "round",
										strokeLinejoin: "round"
									})
								})
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: {
									...
									letterSpacing: ".08em",
									textAlign: "center",
									textShadow: "0 1px 2px rgb(0 0 0 / 42%)",
									transform: "translateX(-50%)"
								},
								children: "PROXY"
							})
					...
					expanded ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						id: popupId,
						role: "dialog",
						"aria-label": t("proxyHeaderPopup"),
						"data-openai-codex-proxy-popup": "status-and-settings",
						style: popupStyle,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								"aria-hidden": "true",
								style: {
									...
									width: 9,
									height: 9,
									borderTop: openDown ? "1px solid var(--dsw-alias-border-l2)" : void 0,
									borderLeft: openRight ? "1px solid var(--dsw-alias-border-l2)" : void 0,
									borderRight: !openRight ? "1px solid var(--dsw-alias-border-l2)" : void 0,
									borderBottom: !openDown ? "1px solid var(--dsw-alias-border-l2)" : void 0,
									background: "var(--dsw-alias-bg-layer-1, #fff)",
									transform: "rotate(45deg)"
								}
							}),
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										...
										style: {
											...
											height: 34,
											borderRadius: 11,
											background: "linear-gradient(145deg, color-mix(in srgb, var(--dsw-alias-brand-primary) 24%, transparent), color-mix(in srgb, #38bdf8 13%, transparent))",
											color: "var(--dsw-alias-brand-primary)"
										},
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
											viewBox: "0 0 24 24",
											...
											"aria-hidden": "true",
											children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
												d: "M7 8.5h10M7 15.5h10M9.5 5.5 7 8.5l2.5 3M14.5 12.5l2.5 3-2.5 3",
												strokeLinecap: "round",
												strokeLinejoin: "round"
											})
										})
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
											...
											children: t("connectivityHeading")
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											style: {
												display: "flex",
												alignItems: "center",
											...
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												...
												style: {
													...
													boxShadow: `0 0 0 3px color-mix(in srgb, ${signalColors[signal]} 14%, transparent)`
												}
											}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
												style: {
													overflow: "hidden",
												...
												children: [
													signalText,
													" · ",
													active ? t("proxyHeaderActive") : t("proxyHeaderDirect")
												]
											})]
										})]
									})]
									...
									type: "button",
									"data-openai-codex-connectivity-test": "manual",
									style: {
										...buttonStyle,
										display: "inline-flex",
										alignItems: "center",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
								...
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
												...
												children: t("accountSwitcherHeading")
											}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												style: {
													display: "block",
													marginTop: 2,
												...
												children: t("accountSwitcherHelp")
											})]
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											type: "button",
											...
											children: accountBusy ? t("accountWorking") : t("accountAdd")
										})]
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										style: {
											...
											marginTop: 10,
											padding: "9px 10px",
											border: "1px solid color-mix(in srgb, var(--dsw-alias-brand-primary) 18%, var(--dsw-alias-border-l2))",
											borderRadius: 11,
											background: "color-mix(in srgb, var(--dsw-alias-brand-primary) 5%, var(--dsw-alias-bg-layer-1))"
										},
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
											htmlFor: `${popupId}-fallback`,
											style: {
												minWidth: 0,
												cursor: fallbackBusy ? "wait" : "pointer"
											},
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												style: {
													display: "block",
												...
												children: t("accountFallbackHelp")
											})]
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
											...
											children: [
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													...
													style: {
														position: "absolute",
														inset: 0,
														border: "1px solid color-mix(in srgb, var(--dsw-alias-brand-primary) 36%, var(--dsw-alias-border-l2))",
														borderRadius: 999,
														background: fallbackEnabled ? "var(--dsw-alias-brand-primary)" : "var(--dsw-alias-bg-layer-3, #94a3b8)",
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										...
										children: accounts.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											style: {
												padding: "12px 10px",
											...
											disabled: accountBusy,
											onClick: () => {
												if (!account.active) switchAccount(account.accountId);
											},
											"data-openai-codex-account-active": account.active || void 0,
											style: {
												display: "flex",
												alignItems: "center",
												gap: 9,
											...
											children: [
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													...
													style: {
														...
														height: 32,
														borderRadius: 10,
														background: account.active ? "linear-gradient(145deg, #2563eb, #4f46e5)" : "var(--dsw-alias-bg-layer-3, #e2e8f0)",
														color: account.active ? "#fff" : "var(--dsw-alias-label-secondary)",
														fontSize: 11,
														fontWeight: 750,
														letterSpacing: ".03em"
													},
													children: accountInitials(account)
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
													style: {
														display: "block",
													...
													children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
														...
														children: account.email ?? t(accountProfileSourceKey(account.profileSource))
													})]
												}),
												account.active ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
													style: {
														display: "inline-flex",
														flex: "0 0 auto",
														alignItems: "center",
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										...
										children: t("accountProfileFileHelp")
									}),
									accountNotice !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										role: "status",
										style: {
											marginTop: 6,
										...
										children: accountNotice
									}) : null,
									accountError !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										role: "alert",
										style: {
											marginTop: 6,
											color: "var(--dsw-alias-state-error-primary, #d92d20)",
										...
										children: t("accountActionFailed", { error: accountError })
									}) : null
								]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									report === void 0 && connectivityError === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										style: {
											padding: "7px 9px",
											color: "var(--dsw-alias-label-tertiary)",
											fontSize: 10,
										...
										children: t("connectivityManualHint")
									}) : null,
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										role: "list",
										style: {
										...
										children: report?.targets.map((target) => {
											...
											return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
												...
												onMouseLeave: () => {
													setHoveredTarget((current) => current === target.id ? void 0 : current);
												},
												onFocus: () => {
													setHoveredTarget(target.id);
												},
												...
												children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
													...
													children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
														...
														style: {
															...
															height: 8,
															borderRadius: "50%",
															background: signalColors[targetState],
															boxShadow: `0 0 0 3px color-mix(in srgb, ${signalColors[targetState]} 14%, transparent)`
														}
													}), target.hostname]
												}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													"aria-hidden": "true",
													style: {
														...
														height: 5,
														borderRadius: 999,
														background: `linear-gradient(90deg, color-mix(in srgb, ${signalColors[targetState]} 25%, transparent), ${signalColors[targetState]})`,
														boxShadow: `0 0 7px color-mix(in srgb, ${signalColors[targetState]} 48%, transparent)`
													}
												})]
											}, target.id);
										})
									}),
									detailTarget !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										...
										children: detailTarget.reachable ? t("connectivityReachable", { latency: detailTarget.latencyMs }) : detailTarget.error ?? t("connectivityUnreachable", { latency: detailTarget.latencyMs })
									}) : null,
									connectivityError !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										role: "alert",
										style: {
											marginTop: 8,
											padding: "8px 9px",
											borderRadius: 9,
											background: "color-mix(in srgb, var(--dsw-alias-state-error-primary, #d92d20) 9%, transparent)",
											color: "var(--dsw-alias-state-error-primary, #d92d20)",
											fontSize: 11,
											lineHeight: "16px",
											overflowWrap: "anywhere"
										},
										children: t("connectivityRequestFailed", { error: connectivityError })
									}) : null
								]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(OpenAICodexProxyConfiguration, {
		...
		const en = {
			...
			accountSwitcherHeading: "Account & fallback",
			accountSwitcherHelp: "Choose the identity used by new Codex requests and control quota failover.",
			accountSelectLabel: "Active Codex subaccount",
			accountOption: "Subaccount {index} · …{suffix}",
			accountActive: "Active",
			accountFallback: "Automatic fallback",
			accountFallbackHelp: "On quota exhaustion, continue the same safe turn with the next saved account.",
			accountFallbackEnabled: "Automatic account fallback enabled.",
			accountFallbackDisabled: "Automatic account fallback disabled.",
			accountProfileOauth: "ChatGPT profile",
			accountProfileFile: "Local alias",
			accountProfileLocal: "Local username",
			accountProfileGenerated: "Saved account",
			accountProfileFileHelp: "Names come from the ChatGPT profile or .openai-codex-account-profiles.json; account IDs stay hidden.",
			accountAdd: "Add account",
			accountWorking: "Working…",
			accountUnavailable: "No saved account",
			accountSwitched: "Account switched. New Codex requests use this account.",
			accountAuthorizationOpened: "Authorization opened. The new account will appear after sign-in completes.",
			accountPopupBlocked: "The browser blocked the authorization window.",
			accountActionFailed: "Account action failed: {error}",
			...
			proxyHeading: "Provider proxy",
			proxyIntro: "Route only OpenAI Codex provider traffic through an HTTP CONNECT proxy.",
			enableProxy: "Use proxy for Codex provider requests",
			enableProxyHelp: "Applies to model requests, OAuth, usage, search, and image generation without changing other providers.",
			proxyUrl: "Proxy URL",
			proxyUrlHelp: "Use a credential-free HTTP or HTTPS proxy origin, for example http://127.0.0.1:7890.",
			proxyWorkflowHelp: "Detect reads standard local proxy environment variables, Test probes only this draft, and Activate is the only action that saves and enables it.",
			proxyStatusActive: "Proxy active: {url}",
			proxyStatusDirect: "Direct connection. No proxy is active for Codex.",
			proxyDetect: "Detect",
			proxyDetecting: "Detecting…",
			proxyTest: "Test",
			proxyTesting: "Testing…",
			proxyActivate: "Activate",
			proxyActivating: "Activating…",
			proxyDisable: "Disable proxy",
			proxyDisabling: "Disabling…",
			proxyEnvironmentDetected: "Detected {source}: {url}. The address is only a draft until Activate.",
			proxyEnvironmentNone: "No HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY candidate was found.",
			proxyEnvironmentInvalid: "{source} exists but is not a credential-free HTTP/HTTPS proxy origin.",
			proxyTestSucceeded: "Proxy test succeeded. Activate is now available.",
			proxyTestFailed: "Proxy test failed. Settings were not changed.",
			proxyActivated: "Proxy saved and activated for this Codex provider instance.",
			proxyDisabled: "Proxy disabled. New Codex requests use the direct dispatcher.",
			proxyActionFailed: "The proxy action failed. Host settings were not confirmed.",
			proxyHeaderLabel: "Proxy",
			proxyHeaderPopup: "Codex proxy status",
			proxyHeaderActive: "Codex proxy is active",
			proxyHeaderDirect: "Codex uses a direct connection",
			proxyHeaderDirectHelp: "No proxy is selected for this plugin instance. Use Plugin configuration to Detect, Test, and Activate one.",
			connectivityHeading: "Codex connectivity",
			connectivityChecking: "Checking connections…",
			connectivitySummary: "{reachable}/{total} domains reachable",
			connectivityUnavailable: "Connection status unavailable",
			connectivityBallLabel: "Codex connectivity: {summary}",
			connectivitySignalGreen: "All Codex endpoints are reachable",
			connectivitySignalYellow: "Checking Codex endpoint reachability",
			connectivitySignalRed: "One or more Codex endpoints are unreachable",
			connectivityRefresh: "Test connections",
			connectivityRefreshing: "Testing…",
			connectivityManualHint: "No background checks. Use Test connections above when you need a fresh result.",
			connectivityReachable: "Reachable · {latency} ms",
			connectivityUnreachable: "Failed · {latency} ms",
			connectivityRequestFailed: "Connectivity check failed: {error}",
			...
			invalidProxyUrl: "Enter a credential-free HTTP or HTTPS proxy origin.",
			save: "Save changes",
			saving: "Saving…",
			discard: "Discard",
			settingsSaved: "Saved",
		...
		const zh = {
			...
			accountSwitcherHeading: "账户与 Fallback",
			accountSwitcherHelp: "选择新 Codex 请求使用的身份，并控制额度耗尽后的自动接续。",
			accountSelectLabel: "当前 Codex 子账户",
			accountOption: "子账户 {index} · …{suffix}",
			accountActive: "当前",
			accountFallback: "自动 Fallback",
			accountFallbackHelp: "额度耗尽时，使用下一个已保存账户安全接续当前 turn。",
			accountFallbackEnabled: "已开启自动账户 Fallback。",
			accountFallbackDisabled: "已关闭自动账户 Fallback。",
			accountProfileOauth: "ChatGPT 账户资料",
			accountProfileFile: "本地别名",
			accountProfileLocal: "本机用户名",
			accountProfileGenerated: "已保存账户",
			accountProfileFileHelp: "名称来自 ChatGPT 资料或 .openai-codex-account-profiles.json；界面不会显示账户 ID。",
			accountAdd: "添加账户",
			accountWorking: "处理中…",
			accountUnavailable: "暂无已保存账户",
			accountSwitched: "账户已切换，新的 Codex 请求将使用此账户。",
			accountAuthorizationOpened: "已打开授权页，登录完成后新账户会自动出现。",
			accountPopupBlocked: "浏览器拦截了授权窗口。",
			accountActionFailed: "账户操作失败：{error}",
			expand: "展开设置",
			collapse: "折叠设置",
			...
			proxyHeading: "Provider 代理",
			proxyIntro: "仅让 OpenAI Codex provider 流量通过 HTTP CONNECT 代理。",
			enableProxy: "为 Codex provider 请求启用代理",
			enableProxyHelp: "覆盖模型请求、OAuth、额度查询、搜索和图片生成，不影响其他 provider。",
			proxyUrl: "代理地址",
			proxyUrlHelp: "请输入不含凭据的 HTTP 或 HTTPS 代理 origin，例如 http://127.0.0.1:7890。",
			proxyWorkflowHelp: "Detect 仅读取本地标准代理环境变量；Test 只测试当前草稿；只有 Activate 会保存并启用代理。",
			proxyStatusActive: "代理已启用：{url}",
			proxyStatusDirect: "当前为直连，Codex 未启用代理。",
			proxyDetect: "Detect 检测",
			proxyDetecting: "正在检测…",
			proxyTest: "Test 测试",
			proxyTesting: "正在测试…",
			proxyActivate: "Activate 启用",
			proxyActivating: "正在启用…",
			proxyDisable: "Disable proxy",
			proxyDisabling: "正在关闭…",
			proxyEnvironmentDetected: "检测到 {source}：{url}。点击 Activate 前它只保留在草稿中。",
			proxyEnvironmentNone: "未发现 HTTPS_PROXY、HTTP_PROXY 或 ALL_PROXY 候选。",
			proxyEnvironmentInvalid: "检测到 {source}，但它不是不含凭据的 HTTP/HTTPS 代理 origin。",
			proxyTestSucceeded: "代理测试成功，现在可以点击 Activate。",
			proxyTestFailed: "代理测试失败，设置没有发生变化。",
			proxyActivated: "代理已保存，并对当前 Codex provider 实例启用。",
			proxyDisabled: "代理已关闭，新的 Codex 请求将动态切回直连 dispatcher。",
			proxyActionFailed: "代理操作失败，Host 设置未确认成功。",
			proxyHeaderLabel: "代理",
			proxyHeaderPopup: "Codex 代理状态",
			proxyHeaderActive: "Codex 代理已启用",
			proxyHeaderDirect: "Codex 当前使用直连",
			proxyHeaderDirectHelp: "此插件实例没有启用代理。请到插件配置中依次执行 Detect、Test 和 Activate。",
			connectivityHeading: "Codex 连接状态",
			connectivityChecking: "正在检查连接…",
			connectivitySummary: "{reachable}/{total} 个域名可达",
			connectivityUnavailable: "暂时无法获取连接状态",
			connectivityBallLabel: "Codex 连接状态：{summary}",
			connectivitySignalGreen: "Codex 相关站点连接正常",
			connectivitySignalYellow: "正在检查 Codex 相关站点可达性",
			connectivitySignalRed: "一个或多个 Codex 相关站点无法访问",
			connectivityRefresh: "手动测试连接",
			connectivityRefreshing: "正在测试…",
			connectivityManualHint: "不会在后台持续测速；需要时请点击上方“手动测试连接”。",
			connectivityReachable: "可达 · {latency} ms",
			connectivityUnreachable: "连接失败 · {latency} ms",
			connectivityRequestFailed: "连接检查失败：{error}",
			...
			invalidProxyUrl: "请输入不含凭据的 HTTP 或 HTTPS 代理 origin。",
			save: "保存更改",
			saving: "正在保存…",
			discard: "放弃更改",
			settingsSaved: "已保存",
