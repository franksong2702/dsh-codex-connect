window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/search-route.ts
		...
		const OPENAI_CODEX_SEARCH_ROUTE_PROVIDER = "openai-codex";
		/**
		...
		*/
		function decodeOpenAICodexSearchRouteConfig(value) {
			if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
			const searchProvider = value["searchProvider"];
			if (searchProvider !== void 0 && (typeof searchProvider !== "string" || searchProvider.length === 0)) return void 0;
			return searchProvider === void 0 ? {} : { searchProvider };
		}
		//#endregion
		...
		function OpenAICodexConfiguration({ scope, searchRouteScope, t, activeModule, panelIdPrefix }) {
			...
			const save = async () => {
				...
				try {
					...
					if ((enablingSearch && routeProviderBefore !== "openai-codex" || disablingSearch && routeProviderBefore === "openai-codex") && (routeBefore?.status !== "ready" || !routeBefore.writable || routeBefore.revision === void 0)) throw new Error("Host search route is not writable");
					if (disablingSearch && routeProviderBefore === "openai-codex") {
						const userRoute = routeBefore?.user;
						if (!(typeof userRoute === "object" && userRoute !== null && Object.hasOwn(userRoute, "searchProvider")) || searchRouteScope === void 0 || routeBefore?.revision === void 0) throw new Error("Codex is selected by the profile composition");
						await searchRouteScope.mutate([{
							op: "unset",
							path: ["searchProvider"]
						}], routeBefore.revision);
						if (searchRouteScope.getSnapshot().value?.searchProvider === "openai-codex") throw new Error("Host kept Codex as the search route");
					}
					...
					if (enablingSearch && routeProviderBefore !== "openai-codex") {
						if (searchRouteScope === void 0 || routeBefore?.revision === void 0) throw new Error("Host search route is unavailable");
						await searchRouteScope.mutate([{
							op: "set",
							path: ["searchProvider"],
							value: OPENAI_CODEX_SEARCH_ROUTE_PROVIDER
						}], routeBefore.revision);
						if (searchRouteScope.getSnapshot().value?.searchProvider !== "openai-codex") throw new Error("Host refused the Codex search route");
					}
					setDraft(accepted);
					setDirty(false);
					setFeedback("saved");
		...
		function OpenAICodexSettings({ t, configScope, searchRouteScope, updater, account, embedded = false, accountOnly = false }) {
				...
				children: [embedded ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
					...
					children: [
						...
						accountOnly ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(OpenAICodexConfiguration, {
							...
							...searchRouteScope === void 0 ? {} : { searchRouteScope }
						})
					]
				})]
			});
		...
		*/
		...
		const en = {
			...
			enableSearchHelp: "Makes OpenAI Codex available and selects it for profile-wide web search when saved.",
			...
			updateHighlightSearchRoute: "Saving the Codex Search switch now also selects or clears the profile-wide search route.",
		...
		const zh = {
			...
			enableSearchHelp: "让 OpenAI Codex 可用，并在保存时将它选为整个 profile 的 Web 搜索提供方。",
			searchModel: "搜索模型",
			searchMode: "联网方式",
			modeCached: "缓存",
			modeIndexed: "索引",
			...
			updateHighlightSearchRoute: "保存 Codex 搜索开关时，现在会同时选择或清除整个 profile 的搜索路由。",
		...
		*/
		...
		function ConfigurationDialog({ t, configScope, searchRouteScope, onClose }) {
			const dialog = (0, react.useRef)(null);
			const titleId = (0, react.useId)();
			(0, react.useEffect)(() => {
				const element = dialog.current;
		...
		function OpenAICodexModelsCard({ t, account, configScope, searchRouteScope }) {
			const [expanded, setExpanded] = (0, react.useState)(false);
