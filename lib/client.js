window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const OPENAI_CODEX_COMPATIBILITY_RECHECK_MS = 3e5;
		function storage() {
			try {
		...
		var OpenAICodexUpdateStore = class {
			...
			readCached() {
				try {
					...
					if (result === void 0 || result.status === "unavailable" || result.compatibility.status !== "compatible") return void 0;
					return {
						result,
			...
			acceptResult(result) {
				...
				if (result.status === "unavailable" || result.compatibility.status !== "compatible") this.recheckTimer = setTimeout(() => {
					this.refresh(true);
				}, OPENAI_CODEX_COMPATIBILITY_RECHECK_MS);
			}
			...
			async refresh(force = false) {
				...
				try {
					...
					if (!force) {
						...
						if (cached !== void 0 && cached.result.currentVersion === this.currentVersion && cached.result.currentDshVersion === currentDshVersion) {
							this.setSnapshot({
		...
		function UpdateContents({ updater, t, overlay }) {
			...
			if (overlay && !pendingRecheck && (!compatibilityWarning && snapshot.status !== "update-available" || noticeKey === void 0 || snapshot.dismissedNotice === noticeKey)) return null;
		...
		*/
		...
		const en = {
			...
			enableReasoningUpdates: "Experimental Astra reasoning changes",
			enableReasoningUpdatesHelp: "Let the agent propose a reasoning change in this conversation. Every change requires your answer; Auto-review cannot approve it. Use an explicit initial Astra level. The selector follows the effective level when the next request is recorded. Compaction and model switching are unsupported.",
			...
			updateCheckUnavailable: "Update and compatibility information cannot be confirmed right now. Check again; this is not evidence of incompatibility.",
		...
		const zh = {
			...
			enableReasoningUpdates: "实验性 Astra 推理档位调整",
			enableReasoningUpdatesHelp: "允许代理提议调整当前会话的推理档位，每次都需要你回答，自动审查不能代为确认。须显式选择 Astra 初始档位；下一次请求记录后，选择器同步显示生效档位。暂不支持压缩和切换模型。",
			...
			updateCheckUnavailable: "暂时无法确认更新与兼容性信息，请重新检查；这不代表当前组合不兼容。",
