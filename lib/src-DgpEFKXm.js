...
import { ReasoningEffortId, createUserMessage, isAgentLoopRequest, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
function parseWindow(value) {
	...
	if (!isRecord$4(value)) throw new Error("OpenAI Codex returned a malformed rate-limit window");
...
function parseLimit(id, name, value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$4(value)) throw new Error("OpenAI Codex returned malformed rate-limit details");
...
function parseCredits(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$4(value) || typeof value["has_credits"] !== "boolean" || typeof value["unlimited"] !== "boolean") throw new Error("OpenAI Codex returned malformed credit details");
...
function parseIndividualLimit(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isRecord$4(value)) throw new Error("OpenAI Codex returned malformed spend-control details");
	const individual = value["individual_limit"];
	if (individual === void 0 || individual === null) return void 0;
	if (!isRecord$4(individual)) throw new Error("OpenAI Codex returned a malformed individual limit");
...
function parseOpenAICodexUsage(value) {
	if (!isRecord$4(value)) throw new Error("OpenAI Codex returned a malformed usage response");
	const limits = [];
	...
	for (const item of additional ?? []) {
		if (!isRecord$4(item)) throw new Error("OpenAI Codex returned a malformed additional rate limit");
...
const OPENAI_CODEX_RESERVE_USAGE_TIMEOUT_MS = 15e3;
/** Maximum usage JSON accepted for an automatic routing decision. */
const OPENAI_CODEX_RESERVE_USAGE_MAX_BYTES = 131072;
...
function reserveIdentity(access) {
	...
	if (!isRecord$2(decoded)) return void 0;
	const claims = decoded["https://api.openai.com/auth"];
	if (!isRecord$2(claims)) return void 0;
...
function parseReserveUsage(value, identity) {
	...
	const hasCredits = isRecord$2(credits) && (credits["has_credits"] === true || credits["unlimited"] === true);
	if (isRecord$2(rateLimit) && typeof rateLimit["allowed"] === "boolean" && (rateLimit["allowed"] || hasCredits) && (spendControl === void 0 || spendControl === null || isRecord$2(spendControl) && spendControl["reached"] === false) && (value["rate_limit_reached_type"] === void 0 || value["rate_limit_reached_type"] === null)) return { kind: "ordinary" };
	return { kind: "unavailable" };
}
/** Query the fixed usage endpoint with the exact token whose identity will authorize dispatch. */
async function readReserveUsage(access, identity, signal) {
...
*/
function createOpenAICodexAdapter(credentials, resolveAttachments, fastMode, visibleModelIds, proxyManager, resolveProxyUrl, contextWindowOverrides, reservePermits) {
...
function registerReserveRouting(ctx, options) {
	...
	const route = async (agent, signal, next) => {
		...
		const result = await options.proxyManager.run(proxyUrl, async () => {
			...
			if (identity === void 0 || identity.accountId !== auth.accountId) return void 0;
			return {
				identity,
				decision: await readReserveUsage(auth.access, identity, requestSignal)
			};
		}).catch(() => {
			requestSignal.throwIfAborted();
		});
		requestSignal.throwIfAborted();
		if (result === void 0 || result.decision.kind === "unavailable") {
	...
	ctx.on("agent/error", ({ agent }) => {
		options.permits.revoke(String(agent.session.id));
	});
...
function apply(ctx, config) {
	...
	ctx.llm.registerAdapter([OPENAI_CODEX_PROVIDER], createOpenAICodexAdapter(credentials, () => ctx.get("attachments"), fastMode, () => resolveOpenAICodexSettings(current()).models, proxyManager, resolveProviderProxyUrl, () => resolveOpenAICodexSettings(current()).contextWindowOverrides, reservePermits));
...
//#endregion
export { OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as $, DSH_PLUGIN_API_PACKAGES as A, resolveOpenAICodexProxyUrl as At, isSupportedDshPluginApiVersion as B, OPENAI_CODEX_AUTH_DOCUMENT_LIMIT as Bt, checkForOpenAICodexUpdate as C, DEFAULT_OPENAI_CODEX_SETTINGS as Ct, COMPATIBILITY_CONTRACT as D, isValidOpenAICodexImageModelHint as Dt, parseOpenAICodexVersion as E, isValidOpenAICodexContextWindowOverrides as Et, SUPPORTED_NODE_RANGE as F, loginOpenAICodex as Ft, OPENAI_CODEX_PROXY_CANDIDATE_LIMIT as G, openAICodexAuthPath as Gt, OPENAI_CODEX_PROXY_DETECT_PATH as H, OPENAI_CODEX_AUTH_V1_BACKUP_SUFFIX as Ht, SUPPORTED_PI_AI_RANGE as I, logoutOpenAICodex as It, OpenAICodexProxyManager as J, OPENAI_CODEX_PROXY_PROBE_TIMEOUT_MS as K, assessCompatibility as L, openAICodexAuthStatus as Lt, SUPPORTED_DSH_PLUGIN_API_RANGE as M, OPENAI_CODEX_USAGE_URL as Mt, SUPPORTED_DSH_PLUGIN_API_VERSION as N, parseOpenAICodexUsage as Nt, COMPATIBILITY_PACKAGES as O, isValidOpenAICodexProxyUrl as Ot, SUPPORTED_DSH_PLUGIN_API_VERSIONS as P, readOpenAICodexRateLimits as Pt, FastModeRegistry as Q, detectCompatibility as R, publicAuthError as Rt, OPENAI_CODEX_UPDATE_PATH as S, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as St, parseOpenAICodexUpdateResult as T, decodeOpenAICodexSettings as Tt, OPENAI_CODEX_PROXY_TEST_PATH as U, OPENAI_CODEX_PROVIDER as Ut, readInstalledPackageVersion as V, OPENAI_CODEX_AUTH_FILENAME as Vt, OPENAI_CODEX_LOCAL_PROXY_CANDIDATES as W, OpenAICodexCredentialStore as Wt, listOpenAICodexProxyCandidates as X, detectOpenAICodexProxies as Y, OPENAI_CODEX_FAST_MODE_PATH as Z, IMAGE_GENERATE_TOOL_NAME as _, DEFAULT_OPENAI_CODEX_IMAGE_MODEL_HINT as _t, name as a, OPENAI_CODEX_IMAGE_GENERATION_URL as at, openAICodexConflictMessage as b, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as bt, migrateOpenAICodexSearchHistory as c, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as ct, OPENAI_CODEX_BASE_URL as d, OPENAI_CODEX_TRANSPORT_API_VERSION as dt, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as et, OPENAI_CODEX_SEARCH_PROVIDER as f, OPENAI_CODEX_TRANSPORT_ERROR_CODES as ft, VIEW_IMAGE_TOOL_NAME as g, isOpenAICodexTransportError as gt, mapOpenAICodexSearchResponse as h, OpenAICodexTransportError as ht, inject as i, openAICodexModelCatalog as it, PI_AI_PACKAGE as j, resolveOpenAICodexSettings as jt, COMPATIBILITY_SCHEMA_VERSION as k, normalizeOpenAICodexProxyUrl as kt, CODEX_AUTO_REVIEW_MODEL as l, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as lt, OpenAICodexSearchProvider as m, OpenAICodexTransport as mt, OPENAI_CODEX_SETTINGS_NS as n, OpenAICodexTrustedOriginsStore as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, OPENAI_CODEX_IMAGE_MAX_COUNT as ot, OPENAI_CODEX_SEARCH_URL as p, OPENAI_CODEX_TRANSPORT_SERVICE as pt, OPENAI_CODEX_PROXY_PROBE_URL as q, apply as r, normalizeTrustedOrigin as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as st, Config as t, isFastModeSessionId as tt, probeCodexAutoReview as u, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as ut, assertNoOpenAICodexProviderConflict as v, DEFAULT_OPENAI_CODEX_PROXY_URL as vt, compareOpenAICodexVersions as w, OPENAI_CODEX_SETTINGS_NAMESPACE as wt, CODEX_CONNECT_VERSION as x, DEFAULT_OPENAI_CODEX_SEARCH_MODE as xt, diagnoseOpenAICodex as y, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as yt, evaluateCompatibility as z, OPENAI_CODEX_ACCOUNT_LIMIT as zt };
