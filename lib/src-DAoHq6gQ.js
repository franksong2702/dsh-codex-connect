...
function isValidOpenAICodexContextWindowOverrides(value) {
	...
	return entries.every(([modelId, window]) => modelId.length > 0 && modelId.trim() === modelId && typeof window === "number" && Number.isSafeInteger(window) && window > 0);
}
...
function resolveOpenAICodexContextWindowOverrides(value) {
	if (value === void 0 || value === null) return void 0;
	if (!isValidOpenAICodexContextWindowOverrides(value)) throw new TypeError("OpenAI Codex contextWindowOverrides must contain at most 256 nonempty model ids with positive safe-integer token budgets");
	return { ...value };
}
...
const Config = z.object({
	...
	contextWindowOverrides: z.transform(z.union([z.const(void 0), z.dict(z.number())]), resolveOpenAICodexContextWindowOverrides),
...
function apply(ctx, config) {
	...
	const validateSettings = (value) => {
		assertOpenAICodexContextWindowModelIds(resolveOpenAICodexSettings(value).contextWindowOverrides, catalog);
	};
