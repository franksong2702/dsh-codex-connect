...
const CODEX_CONNECT_VERSION = "0.1.0-alpha.4.11";
//#endregion
...
const ERROR_MESSAGES = {
	...
	OPENAI_CODEX_TIMEOUT: "The Codex image request timed out and may still consume quota",
	OPENAI_CODEX_CANCELED: "The Codex image request was canceled and may still consume quota",
...
* @deprecated The provider now records searches through standard Tool events.
*/
function recordOpenAICodexSearchRequest(_ctx, _request) {}
...
async function migrateArtifact(path, apply) {
	...
	try {
		...
		if (!(await readFile(backupPath)).equals(original)) throw new Error(`migration backup already exists with different content: ${backupPath}`);
	}
...
async function migrateOpenAICodexSearchHistory(options = {}) {
	...
	for await (const path of sessionArtifacts(root)) try {
		...
		const partial = options.apply === true && files.length > 0 ? `; ${files.length} earlier file(s) were already repaired and remain safely backed up` : "";
...
//#endregion
export { FastModeRegistry as $, OPENAI_CODEX_TRANSPORT_API_VERSION as A, CODEX_CONNECT_VERSION as B, resolveOpenAICodexSettings as C, OPENAI_CODEX_IMAGE_MAX_RESPONSE_BYTES as D, OPENAI_CODEX_IMAGE_MAX_ERROR_BYTES as E, isOpenAICodexTransportError as F, PI_AI_PACKAGE as G, COMPATIBILITY_PACKAGES as H, VIEW_IMAGE_TOOL_NAME as I, SUPPORTED_PI_AI_VERSION as J, SUPPORTED_DSH_PLUGIN_API_VERSION as K, assertNoOpenAICodexProviderConflict as L, OPENAI_CODEX_TRANSPORT_SERVICE as M, OpenAICodexTransport as N, OPENAI_CODEX_IMAGE_PROMPT_MAX_LENGTH as O, OpenAICodexTransportError as P, OPENAI_CODEX_FAST_MODE_PATH as Q, diagnoseOpenAICodex as R, decodeOpenAICodexSettings as S, OPENAI_CODEX_IMAGE_MAX_COUNT as T, COMPATIBILITY_SCHEMA_VERSION as U, COMPATIBILITY_CONTRACT as V, DSH_PLUGIN_API_PACKAGES as W, detectCompatibility as X, assessCompatibility as Y, evaluateCompatibility as Z, DEFAULT_OPENAI_CODEX_SEARCH_MAX_OUTPUT_TOKENS as _, name as a, OPENAI_CODEX_USAGE_URL as at, DEFAULT_OPENAI_CODEX_SETTINGS as b, installOpenAICodexSearchEvent as c, loginOpenAICodex as ct, OPENAI_CODEX_BASE_URL as d, OPENAI_CODEX_AUTH_FILENAME as dt, OPENAI_CODEX_FAST_MODE_MAX_SESSIONS as et, OPENAI_CODEX_SEARCH_PROVIDER as f, OPENAI_CODEX_PROVIDER as ft, DEFAULT_OPENAI_CODEX_SEARCH_CONTEXT_SIZE as g, mapOpenAICodexSearchResponse as h, inject as i, normalizeTrustedOrigin as it, OPENAI_CODEX_TRANSPORT_ERROR_CODES as j, OPENAI_CODEX_IMAGE_REQUEST_TIMEOUT_MS as k, migrateOpenAICodexSearchHistory as l, logoutOpenAICodex as lt, OpenAICodexSearchProvider as m, openAICodexAuthPath as mt, OPENAI_CODEX_SETTINGS_NS as n, isFastModeSessionId as nt, OPENAI_CODEX_HISTORY_BACKUP_SUFFIX as o, parseOpenAICodexUsage as ot, OPENAI_CODEX_SEARCH_URL as p, OpenAICodexCredentialStore as pt, SUPPORTED_NODE_RANGE as q, apply as r, OpenAICodexTrustedOriginsStore as rt, OPENAI_CODEX_SEARCH_MODEL_REQUEST_EVENT as s, readOpenAICodexRateLimits as st, Config as t, OPENAI_CODEX_FAST_MODE_MAX_SESSION_ID_LENGTH as tt, recordOpenAICodexSearchRequest as u, openAICodexAuthStatus as ut, DEFAULT_OPENAI_CODEX_SEARCH_MODE as v, OPENAI_CODEX_IMAGE_GENERATION_URL as w, OPENAI_CODEX_SETTINGS_NAMESPACE as x, DEFAULT_OPENAI_CODEX_SEARCH_MODEL as y, openAICodexConflictMessage as z };
