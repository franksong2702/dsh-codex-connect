...
function registerThinkHostIntegration(ctx) {
	...
	ctx.on("agent/request", async ({ agent, turn, step, signal }, next) => {
		...
		if (current?.pending !== void 0 && ids.has(current.pending.notice.id)) pending.delete(agent);
		return {
			...config,
	...
	function replaySession(options) {
		...
		if (prepareReasoningReplay(options, session) !== void 0) {
			...
			const queued = pending.get(agent);
			if (queued !== void 0 && session?.deriveMessages().some((message) => message.id === queued.notice.id)) pending.delete(agent);
		}
		return session;
	}
	...
	async function change(agent, effort, reason, signal) {
		if (!isAstraReasoningEffort(effort) || reason.trim().length === 0 || reason.length > 2e3) reasoningUpdateError("Provide a supported effort and a reason of 1–2000 characters.");
		...
		if (effort === before.effectiveEffort) return {
			status: "unchanged",
			effort,
			message: "The selected effort is already effective."
		};
		...
		try {
			...
			combined.throwIfAborted();
			if (answer.answers.length !== 1 || answer.answers[0]?.id !== "astra-reasoning-effort" || answer.answers[0].selected.length !== 1 || answer.answers[0].selected[0] !== approve || answer.answers[0].custom !== void 0 && answer.answers[0].custom.trim() !== "") return {
				status: "declined",
				effort: before.effectiveEffort,
				message: "No reasoning change was approved."
			};
			const after = selection(agent);
			if (after.baseEffort !== before.baseEffort || after.effectiveEffort !== before.effectiveEffort || after.revision !== before.revision || after.generation !== before.generation) reasoningUpdateError("The conversation changed during the decision; this approval was not applied.");
			pending.set(agent, {
			...
			return {
				status: "queued",
				effort,
				message: `The user approved ${effort}. It becomes effective only when the next request is recorded; cancellation or a later manual choice can discard this pending change.`
			};
		} finally {
			busy.delete(agent);
		}
	...
	function registerTool(toolCtx) {
		toolCtx.tools.register(defineTool({
			...
			parameters: {
				effort: {
					type: "string",
					enum: ASTRA_REASONING_EFFORTS,
					required: true,
					description: "Requested effort."
				},
				reason: {
					type: "string",
					required: true,
