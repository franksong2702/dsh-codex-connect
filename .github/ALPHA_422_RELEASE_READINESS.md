...
Alpha 4.22 is the Codex Connect release candidate for DSH `0.1.2-alpha.1` and `@earendil-works/pi-ai` `0.84.2`. It combines the DSH client API migration, the optional Models account card, interrupted OAuth recovery, shared settings, and bounded per-model context budgets.

## Completed before registry availability
...
## Required after the official npm packages are available

1. Confirm every exact DSH dependency declared in `package.json` resolves from the public npm registry at `0.1.2-alpha.1`.
...
5. Only after those gates pass, add the exact Alpha 4.22 and DSH `0.1.2-alpha.1` pair to `verified-compatibility.json` and the released-version table in `INSTALL.md`, then rerun the affected checks.
...
As of 2026-08-29, the public npm registry returns `E404` for all 31 exact `@deepseek-ai/dsh-*` packages at `0.1.2-alpha.1` declared by this candidate. Source-linked validation cannot replace the registry and installed-artifact gates above.
