...
The Alpha 4.22 release candidate pins Harness `0.1.2-alpha.2` development dependencies and follows its pi-ai range `^0.84.2`; the release lockfile currently selects pi-ai `0.84.4`. Supported Node.js remains `^22.19.0 || >=24.0.0`. The keyed `settings.plugin.item` integration remains, while client types come from their Session Controller, Settings, Store, and Renderer owners instead of the removed client-runtime package. The target is not added to the published compatibility record until normal npm installation and full Web/OAuth validation pass. Backend eligibility, quotas, models, service-side context capacity, and protocol details remain controlled upstream. Tests use temporary OAuth documents and mocked network responses; CI does not perform real authentication.
