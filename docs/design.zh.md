...
Alpha 4.22 发布候选固定使用 Harness `0.1.2-alpha.1` 开发依赖和 pi-ai `0.84.2`；Node.js 支持范围仍为 `^22.19.0 || >=24.0.0`。keyed `settings.plugin.item` 集成保持不变，客户端类型改从 Session Controller、Settings、Store 和 Renderer 的所属包导入，不再依赖已移除的 client-runtime 包。在正常 npm 安装及完整 Web/OAuth 验证通过前，不会把这一目标加入已发布兼容性记录。资格、额度、模型、服务端上下文容量和后端协议仍由上游控制。测试仅使用临时 OAuth 文档和模拟网络响应，CI 不执行真实认证。
