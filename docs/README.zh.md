...
如需精确复现这个版本，使用 `dsh plugin --profile web add dsh-codex-connect@0.1.0-alpha.4.16`。对应 GitHub prerelease 已创建但 npm 不可用时，可使用 `dsh plugin --profile web add 'github:franksong2702/dsh-codex-connect#v0.1.0-alpha.4.16'`。本地 checkout 可安装为 `link:/absolute/path/to/dsh-codex-connect`。

### 版本更新提醒

Codex Connect 会通过 DSH Web 服务定期检查公开的 npm 包信息，以及本仓库维护的 `verified-compatibility.json`。同一张卡片会读取本机实际加载的 DSH 软件包版本，与本项目记录的最新 DSH 版本并列显示，并在不重复相同版本号的前提下比较当前与最新插件版本。本机版本检测只使用插件已经能够取得的软件包元数据，不要求 DSH Core 作出任何修改。

兼容性清单按插件版本和 DSH 版本精确记录，不假设后续版本天然兼容。维护者确认新的 DSH 版本后，只需更新仓库文件，不必重新发布插件。绿色表示当前插件已经通过验证；黄色表示最新插件已经通过验证，应先升级插件；红色表示当前插件和最新发布的插件都尚未通过验证；灰色表示暂时无法检查清单。红色状态会提供预填内容的 GitHub Issue 链接，让用户无需自行组织报告就能提醒维护者。
...
- 当前唯一已验证的兼容组合是 DSH 插件 API packages `0.1.1-rc.2`、`@earendil-works/pi-ai` `0.82.1` 和 Node.js `^22.19.0 || >=24.0.0`；详见 [compatibility.json](../compatibility.json)。Alpha 4.16 使用 rc.2 的 keyed 插件配置 slot；旧版 DSH API packages 用户应升级到 rc.2 API packages。
