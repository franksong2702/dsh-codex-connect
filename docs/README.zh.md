...
兼容性清单按插件版本和 DSH 版本精确记录，不假设后续版本天然兼容。维护者确认新的 DSH 版本后，只需更新仓库文件，不必重新发布插件。绿色表示当前安装组合已经通过验证；黄色表示应安装已与当前 DSH 版本完成验证的最新插件，或清单中适配该 DSH 的最高历史插件版本；红色表示这个已知 DSH 版本没有任何已发布插件的匹配记录；灰色表示清单尚未记录当前 DSH 版本，或暂时无法检查公开清单。红色状态会按当前安装的 DSH 版本提供预填内容的 GitHub Issue 链接，让用户无需自行组织报告就能提醒维护者。
