...
此未发布的 DSH `0.1.2-alpha.1` 分支还在 **设置 → 模型** 中提供 **Openai-Codex** 账户卡，辅助文案标注“由 Codex Connect 插件提供支持。”，用于 ChatGPT 登录、重新授权、退出和查看额度。两个页面共用同一份内存账户状态及轮询；代理、模型显示、搜索和图片配置仍在插件页管理。模型页底部入口是可选增强：没有该设置分区的 profile 仍保留原插件入口。这不代表已发布的 Alpha 4.21 新增了兼容性承诺。

模型页入口采用紧凑的提供方卡片样式，点击 **管理** 才展开账户操作。浏览器授权中断后，可以点击 **重新打开授权** 继续原登录，或点击 **取消登录** 后，从任一设置页或另一个受信任浏览器重试。取消不会退出已有账户。未完成的授权默认在 10 分钟后到期；插件配置 `oauthTimeoutMs` 可设为 1,000–1,800,000 毫秒，在插件加载时生效。获取初始授权链接仍有独立的 30 秒等待上限。取消和到期都不需要重启 DSH。
...
- 已发布 Alpha 4.21 的已验证组合仍是 DSH 插件 API packages `0.1.1-rc.2`、`@earendil-works/pi-ai` `0.82.1` 和 Node.js `^22.19.0 || >=24.0.0`。此未发布分支面向 DSH `0.1.2-alpha.1` 与 pi-ai `0.84.2`；[compatibility.json](../compatibility.json) 描述的是开发目标，不代表 Alpha 4.21 增加了已验证组合。发布前必须通过正常 npm 安装及完整 Web/OAuth 验证。已发布版本的对应关系见 [INSTALL.md](../INSTALL.md)。
