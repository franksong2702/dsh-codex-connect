...
此未发布的 DSH `0.1.2-alpha.1` 分支还在 **设置 → 模型** 中提供 Codex Connect 账户卡，用于 ChatGPT 登录、重新授权、退出和查看额度。两个页面共用同一份内存账户状态及轮询；代理、模型显示、搜索和图片配置仍在插件页管理。模型页底部入口是可选增强：没有该设置分区的 profile 仍保留原插件入口。这不代表已发布的 Alpha 4.21 新增了兼容性承诺。
