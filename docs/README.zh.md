...
Codex Connect 会通过 DSH Web 服务定期检查公开的 npm 包信息，以及本仓库维护的 `verified-compatibility.json`。同一张卡片会显示当前插件版本、最新插件版本、本项目记录的最新 DSH 版本，以及这个精确组合是否通过验证。它不会读取当前运行中的 DSH 版本，也不要求 DSH Core 作出任何修改。
