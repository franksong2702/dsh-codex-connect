...
模型页紧凑卡片在未登录时显示 **授权**；登录后显示当前账户、**查看额度** 和 **管理账户**。添加新账户期间，当前账户及其额度仍可使用。切换账户会改变后续 Codex 请求使用的本机凭据，但不会自动迁移已有对话。移除当前账户时必须确认由哪个剩余账户接替；移除最后一个账户会退出登录。**退出全部账户** 会删除本机保存的全部 Codex 凭据。浏览器授权中断后，可以点击模型页的 **继续授权**（插件页为 **重新打开授权**）继续原登录，或点击 **取消登录** 后，从任一设置页或另一个受信任浏览器重试。未完成的授权默认在 10 分钟后到期；插件配置 `oauthTimeoutMs` 可设为 1,000–1,800,000 毫秒，在插件加载时生效。获取初始授权链接仍有独立的 30 秒等待上限。取消和到期都会保留已有账户，也都不需要重启 DSH。
