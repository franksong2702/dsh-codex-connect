...
**Current browser continuation: local four-host Chromium gate passed.** The private shell now supplies 18 frozen runtime pins across store/slots/primitives, including the additionally discovered Zustand/Immer edges. Current code passed 54 focused cases/typecheck, the full 113-file / 1,228-test check, and four exact hosts with 232 worker/HTTP executions plus 16 full Chromium flows (`wc_job_QFD3sWUsobHRmy5t`). The complete report is `docs/experiments/split-browser-final-host-acceptance.json`; every browser flow recovered after reload without duplicate admission and confirmed child cleanup. Final-commit remote CI must still be read from #200 after delivery. Older interrupted/failed runs below remain historical, not unresolved evidence for unchanged earlier source. Human experience, real usefulness and persistent authority/budget reconstruction remain unaccepted. No production source/dependency/default, service, real credential or model request changed.

Next gate: [exact-host Chromium](../experiments/split-readonly-worker.md#exact-host-chromium-gate--2026-09-19) extends the already-passing 58-scenario HTTP/worker matrix with actual browser flows on each selected host. The new runner verifies the selected frontend package identities instead of borrowing the development baseline's client bundles. Record new exact-head CI separately; no production entry, persistence or live inference is enabled.

This branch is the internal Split implementation in PR #200, now based on refreshed design #199 at `cc979bc` and post-publication main `e5772cd8a5c47f30b5ab14fe73d2901914348463`. Read [the latest Conversation compatibility gate](../experiments/split-readonly-worker.md#post-437-conversation-compatibility-gate--2026-09-19) before relying on the historical checkpoints below. The expanded matrix now requires all six actual Conversation HTTP/controller cases on each exact host, with real file-upload admission dependencies composed on newer hosts. Final-code local checks passed 112 files / 1,197 tests, 38 Chromium tests, both browser lifecycle checks and 232 exact-host scenarios. New-head remote CI remains separate. No full four-host Chromium, persistent worker or live-provider acceptance is implied.
