...
插件不自动删除已发布的原文件：关闭能力或卸载插件后历史结果仍可下载，数据保留策略由设备所有者管理。
...
- **下载原文件**使用插件注册的同源 GET 路由。路由必须复用可信 origin 判定，同时校验不透明 asset id、请求 session 与创建时 owner session 一致，并在读取后复核文件权限、SHA-256、格式、尺寸和字节数；不接受路径作为输入。
...
7. 下载路由拒绝跨 session、非法 asset id、非 GET 方法和不可信远端请求；损坏文件不会作为原图返回。
