...
Codex 目录来自已安装的 `@earendil-works/pi-ai` 包，不是实时查询账户可用模型的结果。DSH `0.1.2-rc.1` 使用 pi-ai `^0.84.2`，其中尚无 `gpt-6-astra`，因此由 Codex Connect 补充定义。Alpha 4.33 候选还以 DSH `0.1.5-alpha.1` 与 pi-ai `0.85.1` 为目标；混装的宿主包以及其他 DSH/pi-ai 组合仍属未验证。遇到原生 Astra 条目时，插件保留其元数据，并维持 Low、Medium、High、Xhigh 和 Max 推理选择，不修改已安装的目录。用户无需单独升级 pi-ai 即可选择 Astra。依赖声明表示预期支持，不代表发布验收已完成；候选通过验证前，公开安装推荐仍使用已发布、已验证的组合。两种来源的目录条目都不能证明账户具有调用权限。
