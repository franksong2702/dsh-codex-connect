...
import { applyReasoningUpdates, reasoningUpdateError } from './reasoning-update.ts'
...
export class AstraReasoningRequestScope {
  ...
  wrapProvider(provider: Provider): Provider {
    ...
    return {
      ...
      streamSimple(model, context, options) {
        ...
        if (provider.id !== 'openai-codex' || model.provider !== 'openai-codex' || model.id !== 'gpt-6-astra') {
          reasoningUpdateError('Think replay cannot be applied to another provider or model.')
        }
  ...
  async *stream(options: GenerateOptions, delegate: () => AsyncIterable<StreamChunk>, session?: Session): AsyncIterable<StreamChunk> {
    const plan = prepareReasoningReplay(options, session)
    // Some adapters construct the provider stream eagerly, before the first next().
