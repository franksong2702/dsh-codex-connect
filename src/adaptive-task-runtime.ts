...
import { ADAPTIVE_TASK_MODELS, ADAPTIVE_TASK_START, ADAPTIVE_TASK_REQUEST_LIMIT, ADAPTIVE_TASK_TOOL,
  allowsTaskRoute, taskRecord, taskRoute } from './adaptive-task-contract.ts'
...
export interface TaskRuntimeOptions {
  ...
  readonly legacyThinkEnabled: () => boolean
}
  ...
  async state(sessionId: string, principal: string): Promise<AdaptiveTaskState> {
    ...
    return { mode: task?.mode ?? 'off', revision: task?.revision ?? 0,
      ...
      capabilities: task?.capabilities ?? capabilities, eligibility: 'not-probed', canStart: task === undefined && isFresh(agent)
        && !this.options.legacyThinkEnabled() && allowsTaskRoute(capabilities, ADAPTIVE_TASK_START),
      ...(this.options.legacyThinkEnabled() ? { unavailable: 'TASK_LEGACY_THINK_ACTIVE' }
  ...
  async command(command: AdaptiveTaskCommand, principal: string): Promise<AdaptiveTaskState> {
    ...
    const apply = async () => {
      await this.options.store.update(agent.id, existing => {
        if (existing !== undefined) {
          ...
          if (command.action === 'resume') {
            if (agent.status !== 'idle' || value.mode !== 'interrupted' || value.reserved >= value.maximumRequests
              || !allowsTaskRoute(capabilities, value.route) || this.options.legacyThinkEnabled()) taskFailure('TASK_RESUME_UNAVAILABLE')
            value.runtime = this.epoch; value.mode = 'auto'; value.selectionSeq = selectionSeq(agent)
          } else {
        ...
        if (command.action !== 'start' || command.revision !== 0 || !isFresh(agent)
          || this.options.legacyThinkEnabled()) taskFailure('TASK_NEW_SESSION_REQUIRED')
        ...
        return { version: 1, sessionId: agent.id, sessionKey: key(agent), owner: principal,
          runtime: this.epoch, revision: 1, mode: 'auto', route: ADAPTIVE_TASK_START,
