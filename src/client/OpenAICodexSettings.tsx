...
export function OpenAICodexSettings({ t, configScope, updater, account, embedded = false, accountOnly = false }: OpenAICodexSettingsProps) {
  ...
  const moduleSummary = (module: OpenAICodexSettingsModule): string => {
    ...
    const count = config === undefined ? 0 : [config.enableSearch, config.enableReserveFallback, config.enableNativeCompaction, config.enableReasoningUpdates, config.enableImageTool, config.enableImageGeneration, config.enableAutoReview].filter(Boolean).length
