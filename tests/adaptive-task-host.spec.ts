...
async function setup(options: { persistence?: 'none' | 'zstd' } = {}) {
  ...
  const adapter = createOpenAICodexAdapter(credentials, () => undefined, undefined, undefined,
    undefined, undefined, undefined, undefined, undefined, governor, undefined, dispatch)
  ...
  let feature = await ctx.plugin(function TaskHost(host: Context) {
    runtime = new AdaptiveTaskRuntime(host, { store, models: async () => catalog, legacyThinkEnabled: () => legacy })
  })
  ...
  const start = (maximumRequests = 40) => command('start', { maximumRequests, models: [...ADAPTIVE_TASK_MODELS] })
  ...
  return { ctx, agent, handle, create, wires, fetch, store, state, command, start, send, execute, governor,
    runtime: () => runtime, setReply(value: typeof reply) { reply = value }, setLegacy(value: boolean) { legacy = value },
...
it('requires the same authenticated browser and exact revision, with duplicate operation receipts', async () => {
  ...
  const command: AdaptiveTaskCommand = { action: 'start', operationId: randomUUID(), revision: 0,
    sessionId: f.agent.id, models: [...ADAPTIVE_TASK_MODELS], maximumRequests: 5 }
...
it('refuses old sessions, legacy Think and starting with unavailable Sol', async () => {
  const f = await setup(); f.setLegacy(true)
  await expect(f.start()).rejects.toThrow('TASK_NEW_SESSION_REQUIRED')
  f.setLegacy(false); f.setCatalog(f.catalog().filter(model => model.id !== 'gpt-5.6-sol'))
