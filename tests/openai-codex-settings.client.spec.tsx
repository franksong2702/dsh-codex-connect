...
describe('OpenAI Codex Plugin configuration card', () => {
  ...
  it('stages, discards, and saves optional capability settings in the same card', async () => {
    ...
    expect(en.enableImageGenerationHelp).toBe('Generate or edit conversation images with GPT Image. Editing sends only the selected target and reference images to the image service.')
    expect(zh.enableImageGeneration).toBe('启用 GPT Image 图片生成')
    expect(zh.enableImageGenerationHelp).toBe('在对话中生成或编辑图片。编辑时，仅将本次选定的主图和参考图发送给图片服务。')
