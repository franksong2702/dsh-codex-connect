...
describe('strict image payload validation', () => {
  it('accepts canonical base64 and rejects non-canonical variants', () => {
    expect(estimateBase64Bytes('TQ==')).toBe(1)
    expect(decodeStrictBase64('TQ==')).toEqual(Uint8Array.from([0x4d]))
    for (const value of ['', 'TQ=', 'TR==', 'TWF=', 'TQ==\n', 'data:image/png;base64,TQ==', 'TQ-_']) {
      expect(estimateBase64Bytes(value)).toBeUndefined()
      expect(decodeStrictBase64(value)).toBeUndefined()
    }
  })

  it('derives dimensions from PNG, JPEG, and WebP', () => {
  ...
  it('rejects unsupported and truncated payloads without throwing', () => {
    ...
    for (const data of [new Uint8Array(), Uint8Array.from([0xff, 0xff, 0xff]), new TextEncoder().encode('GIF89a'), truncatedPng, overrunJpeg, badWebp]) {
