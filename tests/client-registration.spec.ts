...
describe('OpenAI Codex browser contribution', () => {
  it('adds an optional Models footer using the same account owner as Plugins', async () => {
    ...
    expect(client).toContain('({ t, configScope, searchRouteScope, updater, account })')
    expect(client).toContain('inject: () => ({ t, account, configScope, searchRouteScope })')
  ...
  it('registers as a Plugin configuration card instead of adding a tab or section', async () => {
    ...
    expect(client).toContain("namespace: 'web'")
    expect(client).toContain('decodeOpenAICodexSearchRouteConfig')
