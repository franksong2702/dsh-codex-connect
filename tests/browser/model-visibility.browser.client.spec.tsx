...
describe('Codex model visibility in Chromium', () => {
  it.each([['en', 390, en], ['zh', 390, zh], ['en', 960, en], ['zh', 960, zh]] as const)('stages and saves the Think opt-in at peer level in %s at %i pixels', async (_language, width, copy) => {
    await page.viewport(width, 844); host.style.width = '100%'
    ...
    const render = () => root.render(createElement(OpenAICodexConfiguration, { scope, t: key => copy[key], activeModule: 'capabilities' }))
    render()
    const toggle = page.getByRole('checkbox', { name: copy.enableReasoningUpdates, exact: true })
    await expect.element(toggle).not.toBeChecked()
    await expect.element(page.getByText(copy.reasoningUpdatesConsent, { exact: true })).toBeVisible()
    await expect.element(page.getByText(copy.reasoningUpdatesRisk, { exact: true })).toBeVisible()
    await toggle.click()
    await expect.element(page.getByText(copy.reasoningUpdatesPending, { exact: true })).toBeVisible()
    ...
    await expect.element(toggle).not.toBeChecked()
    await toggle.click(); await page.getByRole('button', { name: copy.save, exact: true }).click()
    await expect.element(page.getByText(copy.reasoningUpdatesSavedOn, { exact: true })).toBeVisible()
    expect(mutate).toHaveBeenCalledExactlyOnceWith([{ op: 'set', path: ['enableReasoningUpdates'], value: true }], 0)
    expect(scope.getSnapshot().value).toEqual({ ...DEFAULT_OPENAI_CODEX_SETTINGS, enableReasoningUpdates: true })
    root.unmount(); root = createRoot(host); render()
    await expect.element(toggle).toBeChecked()
    await page.getByText(copy.reasoningUpdatesDetails, { exact: true }).click()
    await expect.element(page.getByText(copy.reasoningUpdatesDisableHelp, { exact: true })).toBeVisible()
    const group = page.getByRole('group', { name: copy.enableReasoningUpdates, exact: true }).element()
    expect(getComputedStyle(group).borderTopWidth).toBe('0px')
    expect(getComputedStyle(group).paddingLeft).toBe('0px')
    const peer = page.getByRole('checkbox', { name: copy.enableNativeCompaction, exact: true }).element()
    expect(Math.abs(toggle.element().getBoundingClientRect().left - peer.getBoundingClientRect().left)).toBeLessThan(1)
    expect(group.scrollWidth).toBeLessThanOrEqual(group.clientWidth)
    await toggle.click(); await page.getByRole('button', { name: copy.save, exact: true }).click()
    await expect.element(page.getByText(copy.reasoningUpdatesSavedOff, { exact: true })).toBeVisible()
    expect(scope.getSnapshot().value).toEqual(DEFAULT_OPENAI_CODEX_SETTINGS)
  })

  it.each([['en', en], ['zh', zh]] as const)('keeps the saved Think choice unchanged after a failed save (%s)', async (_language, copy) => {
    vi.stubGlobal('fetch', vi.fn(async () => Response.json(modelCatalogFixture([]))))
    const { scope, mutate } = settingsScopeFixture()
    ...
    const toggle = page.getByRole('checkbox', { name: copy.enableReasoningUpdates, exact: true })
    await toggle.click(); await page.getByRole('button', { name: copy.save, exact: true }).click()
    await expect.element(page.getByText(copy.settingsSaveFailed, { exact: true })).toBeVisible()
    await expect.element(page.getByRole('group', { name: copy.enableReasoningUpdates, exact: true }).getByRole('status')).toHaveTextContent(copy.reasoningUpdatesSavedOff)
    await expect.element(page.getByText(copy.reasoningUpdatesPending, { exact: true })).toBeVisible()
    expect(scope.getSnapshot().value?.enableReasoningUpdates).toBe(false)
    await page.getByRole('button', { name: copy.discard, exact: true }).click()
    await expect.element(toggle).not.toBeChecked()
